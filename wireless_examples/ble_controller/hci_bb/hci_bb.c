...
// List of HCI events generated by hci_bb app using eventcode = 0xFF
#define HCI_VENDOR_SUBEVENT_TRAFFIC_REPORT     (0xF0)
#define HCI_VENDOR_SUBEVENT_FATAL_ERROR        (0xF1)
...
#endif
void HCIBB_SerialWrite(uint8_t *data, uint32_t len);
...
static void HCI_LocalSendVendorFatalErrorToHost(uint8_t segmentNb, uint8_t done, uint32_t lenght, uint8_t *payload)
{
    ...
    if (lenght > 0)
    {
        FLib_MemCpy(pPacketBuffer + 6, payload, lenght);
    }

    HCIBB_SerialWrite(pPacketBuffer, lenght + 6);
    (void)MEM_BufferFree(pPacketBuffer);
}
