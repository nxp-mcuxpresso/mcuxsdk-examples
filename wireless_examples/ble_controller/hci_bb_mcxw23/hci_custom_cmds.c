...
static ble_stat_t PrintString(uint8_t nbOfInParams,
                              ...
                              uint8_t *outParamsLength,
                              hci_return_command_type *returnCommandType);
static ble_stat_t EnableLLGpioDebug(uint8_t nbOfInParams,
                                    uint8_t *inParams,
                                    uint8_t *evntPcktP,
                                    uint8_t *outParamsLength,
                                    hci_return_command_type *returnCommandType);
static ble_stat_t DisableLLGpioDebug(uint8_t nbOfInParams,
                                     uint8_t *inParams,
                                     uint8_t *evntPcktP,
                                     uint8_t *outParamsLength,
                                     hci_return_command_type *returnCommandType);
...
static const CustomHciCmd s_customHCIOperations_Utils[] = {
    {.opcode = GET_OP_CODE(HCI_OCF_PRINT_STRING, HCI_OGF_EXT), .nbOfInParams = HCI_BLACKBOX_INIT_STRING_SIZE + 1, .func = PrintString},
    {.opcode = GET_OP_CODE(HCI_ENABLE_LL_GPIO_DEBUG, HCI_OGF_EXT), .nbOfInParams = 0, .func = EnableLLGpioDebug},
    {.opcode = GET_OP_CODE(HCI_DISABLE_LL_GPIO_DEBUG, HCI_OGF_EXT), .nbOfInParams = 0, .func = DisableLLGpioDebug},
};
...
{
    ble_stat_t status = SUCCESS;
    if (BLEController_EnableGPIODebugging() == kBLEC_Success)
    {
        /* No parameters to return */
        *outParamsLength = 0;
        /* Return HCI_RETURN_COMMAND_TYPE_COMPLETE  */
...
{
    ble_stat_t status = SUCCESS;
    if (BLEController_DisableGPIODebugging() == kBLEC_Success)
    {
        /* No parameters to return */
        *outParamsLength = 0;
        /* Return HCI_RETURN_COMMAND_TYPE_COMPLETE  */
