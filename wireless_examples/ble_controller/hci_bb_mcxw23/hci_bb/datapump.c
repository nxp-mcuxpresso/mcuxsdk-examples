...
{
    static const uint32_t txTimerUs = 100000; /* Check every 100ms for jobs. */
...
static void DATAPUMP_FreeTimer(uint8_t timerId)
{
    ...
    CTIMER_RegisterCallBack(timer, (ctimer_callback_t *)DATAPUMP_TimerCb, kCTIMER_SingleCallback);
...
static void DATAPUMP_TimerInit(datapump_ctimer_id_t ctimerId)
{
    ...
    CTIMER_RegisterCallBack(base, NULL, kCTIMER_SingleCallback);
...
static void DATAPUMP_Transmit(transmit_params_t *transmit)
{
    if (transmit->stopTransmit == false)
    {
        while (transmit->credits > 0)
        {
            ...
            info->sequenceNumber                   = transmit->numPacketsSend;
...
static void DATAPUMP_GenerateReport(uint16_t connHandle, report_status_datapump_t *status)
{
    ...
    {
        status->status       = kDATAPUMP_Status_Success;
        status->numPacketsTx = transmitDatapump->numPacketsSend;
    }
}
...
static bool DATAPUMP_ProcessAclPacket(const acl_packet_t *aclPacket)
{
    ...
    else
    {
        ...
        {
            PRINTF("ERROR expected %u but got %u\n", rxp->expSequenceNumber, Info.sequenceNumber);
        }
...
static void DatapumpTransmit(datapump_transmit_parameters_t *p)
{
    ...
    if (status.status == kDATAPUMP_Status_Success)
    {
        ...
        transmit_params_t *txp = &s_contexts[0].transmit;
        for (i = 0; i < MAX_CONN; i++, txp++)
        {
                ...
                */
                ...
                if (p->frequency == PAYLOAD_FREQUENCY_PERIODIC)
                {
                    if (s_txTimerReferences == 0)
                    {
                        ...
                        {
                            /*                                VVVVVVVVVVVV is wrong since s_ctimerIdTx can be 3. */
                            // CTIMER_RegisterCallBack(s_ctimers[s_ctimerIdTx], (ctimer_callback_t *)DATAPUMP_TimerCb,
                            // kCTIMER_SingleCallback);
                            CTIMER_RegisterCallBack(s_ctimers[s_cClockId], (ctimer_callback_t *)DATAPUMP_TimerCb,
                                                    kCTIMER_SingleCallback);
...
static void DatapumpReceive(datapump_receive_parameters_t *params)
{
    ...
    if (status.status == kDATAPUMP_Status_Success)
    {
        ...
        receive_params_t *rxp = &s_contexts[0].receive;
        for (i = 0; i < MAX_CONN; i++, rxp++)
        {
            if (!s_contexts[i].inUse)
            {
