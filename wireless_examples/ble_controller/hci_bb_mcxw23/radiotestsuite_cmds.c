    ...
    switch (inParams[0])
    {
        ...
        case 1:
            ...
            *returnCommandType = HCI_RETURN_COMMAND_TYPE_COMPLETE;
            ...
            break;
        case 2:
            gPhyMode = kBLEC_PHY_CODED_S8;
            /* Return HCI_RETURN_COMMAND_TYPE_COMPLETE  */
            *returnCommandType = HCI_RETURN_COMMAND_TYPE_COMPLETE;
            status               = SUCCESS;
            break;
        case 3:
            gPhyMode = kBLEC_PHY_CODED;
            /* Return HCI_RETURN_COMMAND_TYPE_COMPLETE  */
            *returnCommandType = HCI_RETURN_COMMAND_TYPE_COMPLETE;
            status               = SUCCESS;
            break;
                                      ...
                                      hci_return_command_type *returnCommandType)
{
    ...
    *returnCommandType = HCI_RETURN_COMMAND_TYPE_COMPLETE;
    ...
    (void)OSA_EventSet((osa_event_handle_t)s_EventHandle, EVENT_STOP_DTM);
...
static ble_stat_t ExecuteDTMTxAndCount(void)
{
    ...
    /* We want to execute packet sending as fast as possible:
     *  - Reschedule this EVENT_EXECUTE_DTM event
     *  - This will cause OSA_ProcessTasks to not return until there is no EVENT_EXECUTE_DTM event anymore
     *  - Full speed packet sending has to stop, once new HCI command is received
     *  - When new HCI command is received, BLEController_EmngrIsIdle will return false
     *  - When this happens, EVENT_STOP_DTM event will be sent which will clear the pending EVENT_EXECUTE_DTM
     *  - Once the EVENT_EXECUTE_DTM event is cleared, the full speed packet sending stops, and normal task execution
     * resumes
     */
    (void)OSA_EventSet((osa_event_handle_t)s_EventHandle, EVENT_EXECUTE_DTM);
    if (!BLEController_EmngrIsIdle())
    {
...
static void RadioTestSuiteTask(void *argument)
{
    ...
    if ((event & EVENT_STOP_DTM) == EVENT_STOP_DTM)
    {
        (void)OSA_EventClear((osa_event_handle_t)s_EventHandle, EVENT_EXECUTE_DTM);
    }
