...
int main(void)
{
    ...
    pflashBlockBase  = armFlashInfo->sector_info->start;
...
/* Erase a sector from destAdrss. */
    ...
    result = Driver_FLASH.EraseSector(destAdrss);
    if (ARM_DRIVER_OK != result)
    {
        error_trap();
    }
    ...
    result = Driver_FLASH.ProgramData(destAdrss, s_buffer, sizeof(s_buffer));
    if (ARM_DRIVER_OK != result)
    {
        error_trap();
    }
    ...
    Driver_FLASH.EraseSector(destAdrss);
