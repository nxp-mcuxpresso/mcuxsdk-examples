...
#define NMT_CONTROL          (CO_NMT_STARTUP_TO_OPERATIONAL | CO_NMT_ERR_FREE_TO_OPERATIONAL | NMT_ERROR)
...
CO_NMT_internalState_t deviceState;
uint8_t changedNMT_HB;
...
void HBChangedNMT_Callback(uint8_t nodeId, uint8_t idx, CO_NMT_internalState_t NMTstate, void *object)
{
    changedNMT_HB = 1;
    deviceState = NMTstate;
}
...
int canopen_app_resetCommunication(void)
{
    ...
    for (uint8_t index = 0; index < OD_CNT_ARR_1016; index++)
    {
...
void canopen_app_process(void)
{
    ...
    {
        changedNMT_HB = 0;
        if (deviceState == CO_NMT_OPERATIONAL)
        {
            PRINTF("Device NMT state %s\r\n", getNMT_string(deviceState));
        }
    }
