...
static OSA_TASK_DEFINE(USB_DeviceTask, PRIORITY_RTOS_TO_OSA(((configMAX_PRIORITIES - CONFIG_NCP_PRIORITY_OFFSET)- 3)), 1, 3072, 0);
...
usb_status_t USB_DeviceCdcVcomCallback(class_handle_t handle, uint32_t event, void *param)
{
    ...
    defined(FSL_FEATURE_USB_KHCI_USB_RAM) && (FSL_FEATURE_USB_KHCI_USB_RAM > 0U)
                ...
                {
#if CONFIG_NCP_USB
                    recv_len = ncp_usb_device_recv(&s_currRecvBuf[0], s_recvSize);
#endif
                }

                /* Schedule buffer for next receive event */
                error = USB_DeviceCdcAcmRecv(handle, USB_CDC_VCOM_BULK_OUT_ENDPOINT, s_currRecvBuf + recv_len,
                                             DATA_BUFF_SIZE - recv_len);
#if defined(FSL_FEATURE_USB_KHCI_KEEP_ALIVE_ENABLED) && (FSL_FEATURE_USB_KHCI_KEEP_ALIVE_ENABLED > 0U) && \
    defined(USB_DEVICE_CONFIG_KEEP_ALIVE_MODE) && (USB_DEVICE_CONFIG_KEEP_ALIVE_MODE > 0U) &&             \
    defined(FSL_FEATURE_USB_KHCI_USB_RAM) && (FSL_FEATURE_USB_KHCI_USB_RAM > 0U)
        ...
        switch (usbPmEvent)
        {
...
#if (!CONFIG_NCP_OT) && (!CONFIG_NCP_BLE) && (CONFIG_NCP_WIFI)
            ...
            case kStatus_StartResume:
                ...
                if (s_cdcVcom.selfWakeup)
                {
                    ...
                    if (s_cdcVcom.remoteWakeup)
                    {
                        /* A device with remote wakeup capability may not generate
                         * resume signaling unless the bus has been continuously
                         * in the Idle state for 5 ms.
                         * Refer to usb 2.0 spec 7.1.7.7 section.
                         */
                        OSA_TimeDelay(50);
