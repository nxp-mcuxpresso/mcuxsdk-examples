...
void host_sleep_post_cfg(int mode)
{
    ...
    if (global_power_config.subscribe_evt && 2 != mode)
    {
        ...
        {
            /* LPM timer has expired, need to exit PM2/PM3 mode.
             * NCP device resumes from WFI, IDLE task notifies SLEEP_EXIT event to NCP host,
             * releases the original PM constrain.
             */
