...
#endif
#if CONFIG_NCP_BLE
extern bool ncp_ble_is_running(void);
#endif
...
status_t powerManager_PmNotify(pm_event_type_t eventType, uint8_t powerState, void *data)
{
     ...
     * When wakeup form by ncp host, it will delay for a while before sending BLE command, the ncp device
     * has already performed the ENTER low power process again before processing the BLE command.
     * This workaround ensures the corresponding EXIT low power process can be completed.
     */
        && (lpm_getHandshakeState() != NCP_LMP_HANDSHAKE_FINISH))
    {
        return kStatus_PMPowerStateNotAllowed;
    }
#endif
...
status_t powerManager_BoardNotify(pm_event_type_t eventType, uint8_t powerState, void *data)
{
    ...
    if (eventType == kPM_EventEnteringSleep)
    {
        if (ncp_intf_pm_enter((int32_t)powerState) != NCP_PM_STATUS_SUCCESS)
        {
            return kStatus_PMPowerStateNotAllowed;
        }
...
#if CONFIG_NCP_SDIO
        else if ((lpm_getHandshakeState() != NCP_LMP_HANDSHAKE_NOT_START) && powerState == PM_LP_STATE_PM1)
...
static void LPM_TimerCallback(TimerHandle_t timer_h)
{
    ...
    if (lpm_getHandshakeState() != NCP_LMP_HANDSHAKE_NOT_START)
    {
        /* Currently in sleep or sleep handshake state, keep the original PM constrain.
         * Before the next IDLE task sends the SLEEP_EXIT event, set the PM constraint to PWR_WFI.
         * This is to ensure that the SLEEP_EXIT event can be sent after waking up.
         */
        lpm_setHandshakeState(NCP_LMP_HANDSHAKE_TIME_WAIT);
        return;
    }
