...
#define PING_SOCK_TASK_PRIO   PRIORITY_RTOS_TO_OSA(((configMAX_PRIORITIES - CONFIG_NCP_PRIORITY_OFFSET) - 3))
static OSA_TASK_HANDLE_DEFINE(ping_sock_thread);
static OSA_TASK_DEFINE(ping_sock_task, PING_SOCK_TASK_PRIO, 1, 2048, 0);

#define NCP_IPERF_TX_TASK_PRIO   PRIORITY_RTOS_TO_OSA(((configMAX_PRIORITIES - CONFIG_NCP_PRIORITY_OFFSET) - 3))
...
#define NCP_IPERF_RX_TASK_PRIO   PRIORITY_RTOS_TO_OSA(((configMAX_PRIORITIES - CONFIG_NCP_PRIORITY_OFFSET) - 3))
...
#define WIFI_NCP_TASK_PRIO   PRIORITY_RTOS_TO_OSA(((configMAX_PRIORITIES - CONFIG_NCP_PRIORITY_OFFSET) - 3))
    ...
    while (1)
    {
        ...
        udp_rate = iperf_msg.iperf_set.iperf_udp_rate;
        {
            ...
            else if (iperf_msg.iperf_set.iperf_type == NCP_IPERF_UDP_TX)
            {
                ...
                server_addr.sin_family = AF_INET;
                server_addr.sin_port = PP_HTONS(5003);
                server_addr.sin_addr.s_addr = dest_addr.s_addr;
            }
        }
