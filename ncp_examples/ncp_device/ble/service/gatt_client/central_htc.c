...
#include "central_htc.h"

/*******************************************************************************
* Definitions
******************************************************************************/
#define HTC_MAX_NOTIF_DATA                  (MIN(BT_L2CAP_RX_MTU, BT_L2CAP_TX_MTU) - 3)
...
static uint8_t htc_ev_buf[sizeof(gatt_notification_ev_t) + HTC_MAX_NOTIF_DATA];
...
static uint8_t notify_func(struct bt_conn *conn,
               ...
               const void *data, uint16_t length)
{
    struct gatt_notification_ev *ev = (void *) htc_ev_buf;
    ...
    length = MIN(length, HTC_MAX_NOTIF_DATA);
                 ...
                 struct bt_gatt_discover_params *params)
{
    ...
    if (bt_uuid_cmp(discover_params.uuid, BT_UUID_HTS) == 0)
    {
        if (host_svc)
        {
            gatt_disc_prim_rp_t *ev = (void *) htc_ev_buf;
            ...
            service.uuid[0] = ((BT_UUID_HTS_VAL) >>  0) & 0xFF;
    ...
    else if (bt_uuid_cmp(discover_params.uuid, BT_UUID_HTS_MEASUREMENT) == 0)
    {
        if (host_svc)
        {
            gatt_disc_chrc_rp_t *ev = (void *) htc_ev_buf;
            ...
            characteristics.uuid[0] = ((BT_UUID_HTS_MEASUREMENT_VAL) >>  0) & 0xFF;
    ...
    else
    {
        if (host_svc)
        {
            gatt_disc_all_desc_rp_t *ev = (void *) htc_ev_buf;
        ...
        else
        {
            ...
            ble_prepare_status(NCP_EVENT_GATT_SUBSCRIPTION, NCP_CMD_RESULT_OK, (uint8_t *) &ev, sizeof(gatt_ncp_ble_svc_subscription_ev_t));
        }
        return BT_GATT_ITER_STOP;
    }
    ...
    switch (data->type)
    {
        ...
        case BT_DATA_UUID16_ALL:
        {
            ...
            for (i = 0; i < data->data_len; i += sizeof(uint16_t))
            {
                ...
                if (bt_uuid_cmp(BT_UUID_DECLARE_16(sys_le16_to_cpu(u16)), BT_UUID_HTS) == 0)
                {
