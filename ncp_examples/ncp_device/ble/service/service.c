...
#define ADV_REPORT_PROCESS_FUN_LEN      (sizeof(p_adv_report) / sizeof(struct service_cb_t))
...
struct service_cb_t svc_cb_connect[] = {
    {PERIPHERAL_HTS_SERVICE_ID, .svc_conn_cb = peripheral_hts_connect},
    {PERIPHERAL_HRS_SERVICE_ID, .svc_conn_cb = peripheral_hrs_connect},
    {BAS_SERVICE_ID, .svc_conn_cb = bas_connect},
...
struct service_cb_t svc_cb_disconnect[] = {
    {PERIPHERAL_HTS_SERVICE_ID, .svc_disconn_cb = peripheral_hts_disconnect},
    {PERIPHERAL_HRS_SERVICE_ID, .svc_disconn_cb = peripheral_hrs_disconnect},
    {BAS_SERVICE_ID, .svc_disconn_cb = bas_disconnect},
...
struct service_cb_t svc_cb_security[] = {
    {PERIPHERAL_HTS_SERVICE_ID, .svc_security_cb = NULL},
    {PERIPHERAL_HRS_SERVICE_ID, .svc_security_cb = NULL},
    {BAS_SERVICE_ID, .svc_security_cb = NULL},
    {CENTRAL_HTC_SERVICE_ID, .svc_security_cb = NULL},
    {CENTRAL_HRC_SERVICE_ID, .svc_security_cb = NULL},
};

struct service_cb_t svc_cb_auth_pass[] = {
    {PERIPHERAL_HTS_SERVICE_ID, .svc_auth_passkey_cb = NULL},
    {PERIPHERAL_HRS_SERVICE_ID, .svc_auth_passkey_cb = NULL},
    {BAS_SERVICE_ID, .svc_auth_passkey_cb = NULL},
};

struct service_cb_t svc_cb_auth_cancel[] = {
    {PERIPHERAL_HTS_SERVICE_ID, .svc_auth_auth_cancel_cb = NULL},
    {PERIPHERAL_HRS_SERVICE_ID, .svc_auth_auth_cancel_cb = NULL},
    {BAS_SERVICE_ID, .svc_auth_auth_cancel_cb = NULL},
};

struct service_cb_t p_adv_report[] = {
    {PERIPHERAL_HTS_SERVICE_ID, .svc_adv_report_cb = NULL},
    {PERIPHERAL_HRS_SERVICE_ID, .svc_adv_report_cb = NULL},
    {BAS_SERVICE_ID, .svc_adv_report_cb = NULL},
    {CENTRAL_HTC_SERVICE_ID, .svc_adv_report_cb = htc_adv_report_processed},
    {CENTRAL_HRC_SERVICE_ID, .svc_adv_report_cb = hrc_adv_report_processed},
};
...
void le_service_connect(struct bt_conn *conn, uint8_t conn_err) {
    ...
    for (size_t i = 0; i < CONNECT_HANDLER_LEN ; i++)
    {
        ...
        {
            if(svc_cb_connect[i].svc_conn_cb == NULL)
                continue;
            svc_cb_connect[i].svc_conn_cb(conn, conn_err);
        }
...
void le_service_disconnect(struct bt_conn *conn, uint8_t reason) {
    ...
    for (size_t i = 0; i < DISCONNECT_HANDLER_LEN ; i++)
    {
        ...
        if(svc_list[i].is_registered)
        {
            if(svc_cb_disconnect[i].svc_disconn_cb == NULL)
                continue;
            svc_cb_disconnect[i].svc_disconn_cb(conn, reason);
        }
    }
}
...
void le_service_security(struct bt_conn *conn, bt_security_t level, enum bt_security_err err) {
    ...
    for (size_t i = 0; i < SECURITY_HANDLER_LEN ; i++)
    {
        ...
        if(svc_list[i].is_registered)
        {
            if(svc_cb_security[i].svc_security_cb == NULL)
                continue;
            svc_cb_security[i].svc_security_cb(conn, level, err);
        }
    }
}
...
void le_service_auth_passkey(struct bt_conn *conn, unsigned int passkey) {
    ...
    for (size_t i = 0; i < AUTH_PASS_HANDLER_LEN ; i++)
    {
        ...
        if(svc_list[i].is_registered)
        {
            if(svc_cb_auth_pass[i].svc_auth_passkey_cb == NULL)
                continue;
            svc_cb_auth_pass[i].svc_auth_passkey_cb(conn, passkey);
        }
    }
}
...
void le_service_auth_cancel(struct bt_conn *conn) {
    ...
    for (size_t i = 0; i < AUTH_CANCEL_HANDLER_LEN ; i++)
    {
        ...
        if(svc_list[i].is_registered)
        {
            if(svc_cb_auth_cancel[i].svc_auth_auth_cancel_cb == NULL)
                continue;
            svc_cb_auth_cancel[i].svc_auth_auth_cancel_cb(conn);
        }
    }
}
        ...
        for (size_t j = 0; j < ADV_REPORT_PROCESS_FUN_LEN ; j++)
        {
            if(svc_list[j].is_registered)
            {
                for(int i = 0; i < ev->eir_data_len; ) {
                    ...
                    if(p_adv_report[j].svc_adv_report_cb == NULL)
                        continue;
                    ...
                    if(!p_adv_report[j].svc_adv_report_cb(&data, (void *)addr)) {
