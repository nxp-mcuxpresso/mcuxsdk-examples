...
void peripheral_ncs_connect(struct bt_conn *conn, uint8_t conn_err)
{
    default_conn = bt_conn_ref(conn);
}

void peripheral_ncs_disconnect(struct bt_conn *conn, uint8_t reason)
{
    if (default_conn) {
...
static void ncs_adv_start(void)
{
    ...
    if(bt_le_adv_start(BT_LE_ADV_CONN_FAST_2, ncs_ad, ARRAY_SIZE(ncs_ad), NULL, 0) < 0) {
