...
struct service_cb_t
{
    ...
    union
    {
        void (*svc_conn_cb)(struct bt_conn *conn, uint8_t conn_err);
        void (*svc_disconn_cb)(struct bt_conn *conn, uint8_t reason);
        void (*svc_security_cb)(struct bt_conn *conn, bt_security_t level, enum bt_security_err err);
        void (*svc_auth_passkey_cb)(struct bt_conn *conn, unsigned int passkey);
        void (*svc_auth_auth_cancel_cb)(struct bt_conn *conn);
        bool (*svc_adv_report_cb)(struct adv_report_data *data, void *user_data);
    };
};
