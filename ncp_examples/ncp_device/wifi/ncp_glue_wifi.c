...
static int scan_cb(unsigned int count)
{
    ...
    for (i = 0, j = 0; i < count && j < CONFIG_MAX_AP_ENTRIES; i++, j++)
    {
        ...
        scan_result->res[i].ap_mfpc        = res.ap_mfpc;
        scan_result->res[i].ap_mfpr        = res.ap_mfpr;
#if CONFIG_NCP_11K
        scan_result->res[i].neighbor_report_supported = res.neighbor_report_supported;
#endif
#if CONFIG_NCP_11V
        scan_result->res[i].bss_transition_supported  = res.bss_transition_supported;
#endif
    }
...
static int wlan_ncp_uap_prov_uapcfg(void *data)
{
    int ret      = -WM_FAIL;
    char ssid[WLAN_NETWORK_NAME_MAX_LENGTH + 1] = {'\0'};
    uint32_t security_type = WLAN_SECURITY_NONE;
    char uapPass[WLAN_PASSWORD_MAX_LENGTH  + 1] = {'\0'};
    
    NCP_CMD_UAP_PROV_SET_UAPCFG *prov_set_uapcfg = (NCP_CMD_UAP_PROV_SET_UAPCFG *)data;
    security_type = prov_set_uapcfg->security_type;
    strncpy(ssid, prov_set_uapcfg->ssid, strlen(prov_set_uapcfg->ssid));
    strncpy(uapPass, prov_set_uapcfg->uapPass, strlen(prov_set_uapcfg->uapPass));
    
    ret = uap_prov_set_uapcfg(ssid, security_type, uapPass);
    if (!ret)
        wlan_ncp_prepare_status(NCP_RSP_WLAN_BASIC_WLAN_UAP_PROV_SET_UAPCFG, NCP_CMD_RESULT_OK);
    else
        wlan_ncp_prepare_status(NCP_RSP_WLAN_BASIC_WLAN_UAP_PROV_SET_UAPCFG, NCP_CMD_RESULT_ERROR);
    return ret;
}
...
static int wlan_ncp_btwt_cfg(void *data)
{
    ...
    cmd_res->params.btwt_cfg.action = tlv->action;
    cmd_res->header.size += sizeof(cmd_res->params.btwt_cfg.action);
    return ret;
}
...
static int wlan_ncp_eu_crypto_gcmp_128(void *data)
{
    ...
    NonceLength       = 12;
    uint8_t AAD[24]   = {0x88, 0x40, 0x0f, 0xd2, 0xe1, 0x28, 0xa5, 0x7c, 0x50, 0x30, 0xf1, 0x84,
...
static int wlan_ncp_eu_crypto_ccmp_256(void *data)
{
    int ret;
    uint16_t Dec_DataLength;
    uint16_t Enc_DataLength;
    ...
    if (ret == WM_SUCCESS)
    {
        ...
        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_CCMP_256, NCP_CMD_RESULT_OK);
    }
    else
    {
        (void)PRINTF("Hostcmd failed error: %d \r\n", ret);

        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_CCMP_256, NCP_CMD_RESULT_ERROR);
    }

    return WM_SUCCESS;
}

static int wlan_ncp_eu_crypto_gcmp_256(void *data)
{
    int ret;
    uint16_t Dec_DataLength;
    uint16_t Dec_DataOnlyLength;
    ...
    if (ret == WM_SUCCESS)
    {
        ...
        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_GCMP_256, NCP_CMD_RESULT_OK);
    }
    else
    {
        (void)PRINTF("Hostcmd failed error: %d \r\n", ret);

        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_GCMP_256, NCP_CMD_RESULT_ERROR);
    }

    return WM_SUCCESS;
}

static int wlan_ncp_eu_crypto_rc4(void *data)
{
    int ret;
    uint16_t Dec_DataLength;
    uint16_t Enc_DataLength;
    uint16_t KeyLength;
    uint16_t KeyIVLength;
    uint8_t DATA[80] = {0};
    uint16_t Length;
    NCP_CMD_EU_CRYPRTO *eu_crypto = (NCP_CMD_EU_CRYPRTO *)data;
    ...
    if (ret == WM_SUCCESS)
    {
        ...
        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_RC4, NCP_CMD_RESULT_OK);
    }
    else
    {
        (void)PRINTF("Hostcmd failed error: %d \r\n", ret);

        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_RC4, NCP_CMD_RESULT_ERROR);
    }

    return WM_SUCCESS;
}

static int wlan_ncp_eu_crypto_aes_wrap(void *data)
{
    int ret;
    uint16_t Dec_DataLength;
    uint16_t Enc_DataLength;
    ...
    if (ret == WM_SUCCESS)
    {
        ...
        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_AES_WRAP, NCP_CMD_RESULT_OK);
    }
    else
    {
        (void)PRINTF("Hostcmd failed error: %d \r\n", ret);

        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_AES_WRAP, NCP_CMD_RESULT_ERROR);
    }

    return WM_SUCCESS;
}

static int wlan_ncp_eu_crypto_aes_ecb(void *data)
{
    int ret;
    uint16_t Dec_DataLength;
    uint16_t Enc_DataLength;
    ...
    if (ret == WM_SUCCESS)
    {
        ...
        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_AES_ECB, NCP_CMD_RESULT_OK);
    }
    else
    {
        (void)PRINTF("Hostcmd failed error: %d \r\n", ret);

        wlan_ncp_prepare_status(NCP_RSP_WLAN_REGULATORY_EU_CRYPTO_AES_ECB, NCP_CMD_RESULT_ERROR);
    }
...
static int wlan_ncp_get_current_network(void *tlv)
{
    int ret = WM_SUCCESS;
    ...
    {
        cmd_res->header.size         += sizeof(NCP_CMD_GET_CURRENT_NETWORK);
    }

    return ret;
}

static int wlan_ncp_remove_all_networks(void *tlv)
{
    int ret = WM_SUCCESS;

    wlan_remove_all_networks();
    wlan_ncp_prepare_status(NCP_RSP_WLAN_NETWORKS_REMOVE_ALL, NCP_CMD_RESULT_OK);
...
struct cmd_t wlan_cmd_basic[] = {
    ...
    {NCP_CMD_WLAN_BASIC_WLAN_UAP_PROV_SET_UAPCFG, "wlan_uap-prov-set-uapcfg", wlan_ncp_uap_prov_uapcfg, CMD_SYNC},
    {NCP_CMD_INVALID, NULL, NULL, NULL},
};
...
struct cmd_t wlan_cmd_regulatory[] = {
    ...
    {NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_CCMP_256, "wlan-eu-crypto-ccmp-256", wlan_ncp_eu_crypto_ccmp_256, CMD_SYNC},
    {NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_GCMP_256, "wlan-eu-crypto-gcmp-256", wlan_ncp_eu_crypto_gcmp_256, CMD_SYNC},
    {NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_RC4, "wlan-eu-crypto-rc4", wlan_ncp_eu_crypto_rc4, CMD_SYNC},
    {NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_AES_WRAP, "wlan-eu-crypto-aes-wrap", wlan_ncp_eu_crypto_aes_wrap, CMD_SYNC},
    {NCP_CMD_WLAN_REGULATORY_EU_CRYPTO_AES_ECB, "wlan-eu-crypto-aes-ecb", wlan_ncp_eu_crypto_aes_ecb, CMD_SYNC},
    {NCP_CMD_INVALID, NULL, NULL, NULL},
};
