...
static shell_status_t shellCmd_query(shell_handle_t shellHandle, int32_t argc, char **argv);
static shell_status_t shellCmd_update(shell_handle_t shellHandle, int32_t argc, char **argv);
static shell_status_t shellCmd_install(shell_handle_t shellHandle, int32_t argc, char **argv);
static shell_status_t shellCmd_accept(shell_handle_t shellHandle, int32_t argc, char **argv);
static shell_status_t shellCmd_cancel(shell_handle_t shellHandle, int32_t argc, char **argv);
static shell_status_t shellCmd_clean(shell_handle_t shellHandle, int32_t argc, char **argv);
static shell_status_t shellCmd_reject(shell_handle_t shellHandle, int32_t argc, char **argv);
...
static shell_status_t shellCmd_query(shell_handle_t shellHandle, int32_t argc, char **argv)
{
    ...
    {
        PRINTF("Component id:%d state: %s\n", component, state_to_str(info.state));
...
{    
    ...
    {
        PRINTF("Unable to mark image as STAGED (is in CANDIDATE state), PSA error %d\n",
                psa_status);
    }
...
static void clean_staging_partition(int image)
{
    ...
    {
        uint32_t buf_erased[BUF_SZ / sizeof(uint32_t)];
        uint32_t buf_flash[BUF_SZ / sizeof(uint32_t)];
