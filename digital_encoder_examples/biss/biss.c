...
void MB4_Init(biss_master_t *master)
{
    ...
    MB4_ConfigureSlave(master, 2);
    PRINTF("reset biss slave 2 \r\n");
    data = 0x1;
    BISS_SLVWriteRegister(master, 2, 0x74, 1, &data);
}
...
{
    PRINTF("SCD: \t0x%08x 0x%08x\r\n",
           master->base->SCDATA1_HIGH, master->base->SCDATA1_LOW);
...
static void BISS_SLVDumpPosition(biss_master_t *master, uint8_t slvID)
{
    ...
    if (status != kStatus_Success)
    {
        PRINTF("\r\n BISS_GetSCData Error!\r\n");
        return;
    }

    PRINTF("Sensor type: Rotary\r\n");
    PRINTF("Multiturn value: %lu Singleturn value: %lu\r\n",
            BISS_SCDGetMtVal(master, slvID, SCData),
...
int main(void)
{
    ...
    PRINTF("BiSS test example.\r\n");
    PRINTF("This example use one board as BiSS master and connect to the BiSS encoder.\r\n");
    ...
    while (1)
    {
        ...
        PRINTF("0: Dump slave position\r\n");
        PRINTF("1: Dump slave information\r\n");
        ...
        if ('0' == inputChar)
        {
           BISS_SLVDumpPosition(master, 1);
        }
        else if ('1' == inputChar)
        {
            BISS_SLVDumpInfo(master, 1);
        ...
        {
            BISS_ChangeTriggerMode(master, BISS_INSTR_TRIGGER);
            BISS_InstrSendandWait(master, BISS_INSTR_CDM_0,
                                  BISS_AGS_DISABLE);
        }
        else if ('4' == inputChar)
        {
