...
/* The location to be used for generation of keys must be defined */
#if !defined(TEST_KEY_LOCATION)
#error "Persistent key location must be defined"
#endif

#if defined(TEST_NXP_WRAPPED_KEY_EXPORT_IMPORT) && (TEST_NXP_WRAPPED_KEY_EXPORT_IMPORT)
#define WRAPPED_KEY_FEATURE_ENABLED 1
#else
#define WRAPPED_KEY_FEATURE_ENABLED 0
#endif
...
#define ASSERT_STATUS(actual, expected)                                          \
    do                                                                           \
    {                                                                            \
        psa_status_t _actual = (psa_status_t)(actual);                           \
        psa_status_t _expected = (psa_status_t)(expected);                       \
        if (_actual != _expected)                                                \
        {                                                                        \
            printf("\tassertion failed at %s:%d - actual:%ld expected:%ld\r\n",  \
                   __FILE__, __LINE__, (long)_actual, (long)_expected);          \
            goto exit;                                                           \
        }                                                                        \
    } while (0)
                           ...
                           uint32_t key_location)
{
    ...
    if (key_lifetime == PSA_KEY_LIFETIME_PERSISTENT) {
        psa_set_key_id(&attributes, mbedtls_svc_key_id_make(0, (psa_key_id_t)persistent_key_id));
        persistent_key_id++;
    }
#if WRAPPED_KEY_FEATURE_ENABLED
    /* Addtional usage added to allow export of key*/
    usage = usage | PSA_KEY_USAGE_EXPORT;
#endif /* WRAPPED_KEY_FEATURE_ENABLED */
    ...
    {
      printf(" psa_generate_key: status: %ld\n",(long)status);
    }
    else if (status == PSA_SUCCESS)
    {
        ...
        if (status != PSA_SUCCESS)
        {
          printf(" psa_export_key: status: %ld\n",(long)status);
        }
        ASSERT_STATUS(status, PSA_SUCCESS);
        
        /* 4. Store externally (currently in ram) */
        memcpy(storage, wrapped_key, wrapped_len);
        ...
        if (status != PSA_SUCCESS)
        {
          printf(" psa_destroy_key: status: %ld\n",(long)status);
        }
        ASSERT_STATUS(status, PSA_SUCCESS);
        
        /* 6. Again Import back the wrapped key*/
        ...
        if (status != PSA_SUCCESS)
        {
          printf(" psa_import_key: status: %ld\n",(long)status);
        }
        ASSERT_STATUS(status, PSA_SUCCESS);

#endif /* WRAPPED_KEY_FEATURE_ENABLED */
        
        /* Reset the key attributes, then get again the attributes via key-id*/
        psa_reset_key_attributes(&attributes);
...
int main(int argc, char *argv[])
{
    ...
    printf("Location is %lx\n", (unsigned long)TEST_KEY_LOCATION);
