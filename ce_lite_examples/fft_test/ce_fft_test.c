...
// Test 3
int ifft256_cf16_input[512] = {
#include "ifft256_cf16_input.txt"
};

int ifft256_cf16_ref_output[512] = {
#include "ifft256_cf16_ref_output.txt"
};
...
int main(void)
{
    ...
    // Test 3: CF16 256pt IFFT Test
    N     = 256;
    log2N = 8;
    for (i = 0; i < 2 * N; i++)
    {
        temp       = (float *)&ifft256_cf16_input[i];
        srcdata[i] = *temp;
        temp       = (float *)&ifft256_cf16_ref_output[i];
        refdata[i] = *temp;
    }
    ...
    if (copyerr > 1e-5f)
        PRINTF("Test 3: IFFT 256 F16 Test Failed: Status=%8X, Reply=%8X\r\n", status, status_buffer[0]);
    else
        PRINTF("Test 3: IFFT 256 F16 Test Passed: Status=%8X, Reply=%8X\r\n", status, status_buffer[0]);

    // Test 4: CF32 256pt IFFT Test
    N     = 256;
    log2N = 8;
    for (i = 0; i < 2 * N; i++)
    {
