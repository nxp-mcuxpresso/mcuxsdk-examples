 ...
 *
 * Architecture:
 *   This app combines the edgefast_open broadcast_media_sender BT app with Wi-Fi CLI.
 *   - Wi-Fi and BT use combo firmware (CONFIG_BT_BYPASS_DOWNLOAD=y) loaded
 *     together over SDIO.
 *   - The BT task (broadcast_media_sender_task) creates its own shell via
 *     le_audio_shell_init() with prompt "BMS>>".
 ...
 *          -> xTaskCreate(broadcast_media_sender_task) [BT task]
...
void task_main(void *param)
{
    ...
    /* Create the BT LE Audio Broadcast Media Sender task */
    if (xTaskCreate(broadcast_media_sender_task, "broadcast_media_sender_task",
...
int main(void)
{
    ...
    printSeparator();
    PRINTF("     Coex Wi-Fi Broadcast Media Sender Open APP\r\n");
    printSeparator();
