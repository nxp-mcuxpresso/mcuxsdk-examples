...
/* Runtime CIS count: determined by WAV file channels (1=mono/1CIS, 2=stereo/2CIS) */
static int le_conn_count = LE_CONN_COUNT;  /* default to compile-time max */
...
StaticSemaphore_t sem_scan_buff;
SemaphoreHandle_t sem_scan;
static StaticSemaphore_t sem_device_selected_buff;
static SemaphoreHandle_t sem_device_selected;
static StaticSemaphore_t sem_connected_buff;
static SemaphoreHandle_t sem_connected;
static StaticSemaphore_t sem_csip_discovered_buff;
static SemaphoreHandle_t sem_csip_discovered;
static StaticSemaphore_t sem_member_discovered_buff;
static SemaphoreHandle_t sem_member_discovered;
static StaticSemaphore_t sem_disconnected_buff;
static SemaphoreHandle_t sem_disconnected;
static StaticSemaphore_t sem_mtu_exchanged_buff;
static SemaphoreHandle_t sem_mtu_exchanged;
static StaticSemaphore_t sem_security_updated_buff;
static SemaphoreHandle_t sem_security_updated;
static StaticSemaphore_t sem_sinks_discovered_buff;
static SemaphoreHandle_t sem_sinks_discovered;
static StaticSemaphore_t sem_vcs_discovered_buff;
static SemaphoreHandle_t sem_vcs_discovered;
static StaticSemaphore_t sem_sources_discovered_buff;
static SemaphoreHandle_t sem_sources_discovered;
static StaticSemaphore_t sem_stream_configured_buff;
static SemaphoreHandle_t sem_stream_configured;
static StaticSemaphore_t sem_stream_qos_buff;
static SemaphoreHandle_t sem_stream_qos;
static StaticSemaphore_t sem_stream_enabled_buff;
static SemaphoreHandle_t sem_stream_enabled;
static StaticSemaphore_t sem_stream_disabled_buff;
static SemaphoreHandle_t sem_stream_disabled;
static StaticSemaphore_t sem_stream_started_buff;
static SemaphoreHandle_t sem_stream_started;
static StaticSemaphore_t sem_stream_connected_buff;
static SemaphoreHandle_t sem_stream_connected;
...
static int audio_stream_encode(bool mute)
{
	...
	else if(1 == lc3_codec_info.channels)
	{
		...
		memcpy(audio_buff[0], wav_file_buff, lc3_codec_info.samples_per_frame * (bits / 8));
	}
	...
	for(int i = 0; i < lc3_codec_info.channels; i++)
	{
		do {
			...
			buf[i] = net_buf_alloc(tx_pool[i], K_MSEC(timeout));
			if(buf[i] == NULL)
			{
	...
	for(int i = 0; i < lc3_codec_info.channels; i++)
	{
		...
		{
			/* Mono: use the first (and only) stream directly */
			stream = &streams[0];
		}
		else
		{
...
int open_wav_file(char *path)
{
	...
	/* Set runtime CIS count based on WAV channels */
	le_conn_count = wav_file.channels;
	PRINTF("\nCIS mode: %s (%d CIS)\n", (le_conn_count == 1) ? "MONO" : "STEREO", le_conn_count);
			      ...
			      const struct bt_bap_qos_cfg_pref *pref)
{
	...
	xSemaphoreGive(sem_stream_configured);
}
...
static void stream_enabled(struct bt_bap_stream *stream)
{
	...
	xSemaphoreGive(sem_stream_enabled);
}
...
static void stream_connected(struct bt_bap_stream *stream)
{
	...
	xSemaphoreGive(sem_stream_connected);
}
...
static void stream_started(struct bt_bap_stream *stream)
{
	...
	xSemaphoreGive(sem_stream_started);
}
...
static void stream_disabled(struct bt_bap_stream *stream)
{
	...
	xSemaphoreGive(sem_stream_disabled);
}
...
static void discover_sinks_cb(struct bt_conn *conn, int err, enum bt_audio_dir dir)
{
	...
	xSemaphoreGive(sem_sinks_discovered);
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
    	...
    	if(info.type == BT_CONN_TYPE_LE)
	{
		...
		xSemaphoreGive(sem_disconnected);
	}
}
				...
				enum bt_security_err err)
{
	...
	{
		if (err == 0) {
			xSemaphoreGive(sem_security_updated);
		} else {
...
static void att_mtu_updated(struct bt_conn *conn, uint16_t tx, uint16_t rx)
{
	...
	xSemaphoreGive(sem_mtu_exchanged);
}
...
static int init(void)
{
	...
	sem_scan = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_scan_buff);
	sem_device_selected = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_device_selected_buff);
	sem_connected = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_connected_buff);
	sem_csip_discovered = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_csip_discovered_buff);
	sem_member_discovered = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_member_discovered_buff);
	sem_disconnected = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_disconnected_buff);
	sem_mtu_exchanged = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_mtu_exchanged_buff);
	sem_security_updated = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_security_updated_buff);
	sem_sinks_discovered = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_sinks_discovered_buff);
	sem_vcs_discovered = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_vcs_discovered_buff);
	sem_sources_discovered = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_sources_discovered_buff);
	sem_stream_configured = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_stream_configured_buff);
	sem_stream_qos = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_stream_qos_buff);
	sem_stream_enabled = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_stream_enabled_buff);
	sem_stream_disabled = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_stream_disabled_buff);
	sem_stream_started = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_stream_started_buff);
	sem_stream_connected = xSemaphoreCreateCountingStatic(UINT32_MAX, 0, &sem_stream_connected_buff);
...
int device_select(int index)
{
	...
	(void)xSemaphoreGive(sem_device_selected);
...
static void member_scan_cb(const bt_addr_le_t *addr, int8_t rssi, uint8_t adv_type, struct net_buf_simple *ad)
{
	...
	(void)xSemaphoreGive(sem_member_discovered);
}
...
static int device_connect(int index)
{
	...
	ret = xSemaphoreTake(sem_connected, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_connected\n");
	    return -1;
	}

	ret = xSemaphoreTake(sem_mtu_exchanged, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_mtu_exchanged\n");
	    return -1;
	}
	...
	ret = xSemaphoreTake(sem_security_updated, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_security_updated\n");
	    return -1;
	}
...
static void vcs_client_discover_callback(struct bt_conn *conn, int err)
{
	...
	(void)xSemaphoreGive(sem_vcs_discovered);
}
...
static int discover_vcs(int index)
{
	...
	(void)xSemaphoreTake(sem_vcs_discovered, portMAX_DELAY);
...
static int discover_sinks(int index)
{
	...
	ret = xSemaphoreTake(sem_sinks_discovered, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_sinks_discovered\n");
	    return -1;
	}
...
static int configure_stream(struct bt_bap_stream *stream, struct bt_bap_ep *ep)
{
	...
	ret = xSemaphoreTake(sem_stream_configured, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_stream_configured\n");
	    return -1;
	}
...
{
	const size_t params_count = (lc3_codec_info.channels == 1) ? 1 : CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK_COUNT;
...
static int set_stream_qos(int index)
{
	...
	(void)xSemaphoreTake(sem_stream_qos, portMAX_DELAY);
...
static int enable_streams(int index)
{
	...
	ret = xSemaphoreTake(sem_stream_enabled, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_stream_enabled\n");
	    return -1;
	}
...
static int disable_streams(int index)
{
	...
	ret = xSemaphoreTake(sem_stream_disabled, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_stream_disabled\n");
	    return -1;
	}
...
static int connect_streams(int index)
{
	...
	ret = xSemaphoreTake(sem_stream_connected, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_stream_connected\n");
	    return -1;
	}
...
static int start_streams(int index)
{
	...
	ret = xSemaphoreTake(sem_stream_started, portMAX_DELAY);
	if (ret != pdTRUE) {
	    PRINTF("failed to take sem_stream_started\n");
	    return -1;
	}
...
{
	vSemaphoreDelete(sem_wav_opened);
	vSemaphoreDelete(sem_lc3_preset);
	vSemaphoreDelete(sem_scan);
	vSemaphoreDelete(sem_device_selected);
	vSemaphoreDelete(sem_connected);
	vSemaphoreDelete(sem_csip_discovered);
	vSemaphoreDelete(sem_member_discovered);
	vSemaphoreDelete(sem_disconnected);
	vSemaphoreDelete(sem_mtu_exchanged);
	vSemaphoreDelete(sem_security_updated);
	vSemaphoreDelete(sem_sinks_discovered);
	vSemaphoreDelete(sem_vcs_discovered);
	vSemaphoreDelete(sem_sources_discovered);
	vSemaphoreDelete(sem_stream_configured);
	vSemaphoreDelete(sem_stream_qos);
	vSemaphoreDelete(sem_stream_enabled);
	vSemaphoreDelete(sem_stream_disabled);
	vSemaphoreDelete(sem_stream_started);
	vSemaphoreDelete(sem_stream_connected);
	...
	int err, size_t set_count)
{
	...
	(void)xSemaphoreGive(sem_csip_discovered);
}
...
void unicast_media_sender_task(void *param)
{
	...
	while (true) {
		...
		ret = xSemaphoreTake(sem_scan, portMAX_DELAY);
		if (ret != pdTRUE) {
		    PRINTF("failed to take sem_scan\n");
		    break;
		}
		...
		ret = xSemaphoreTake(sem_device_selected, portMAX_DELAY);
		if (ret != pdTRUE) {
		    PRINTF("failed to take sem_device_selected\n");
		    break;
		}
		...
		for (int i = 0; i < le_conn_count; i++)
		{
			...
			xSemaphoreTake(sem_csip_discovered, portMAX_DELAY);
			PRINTF("CSIP discovered\n");

			if(i == 0 && le_conn_count > 1)
			{
				PRINTF("Scan another member\n");

				member_scan();

				xSemaphoreTake(sem_member_discovered, portMAX_DELAY);
		...
		do{
			...
			else
			{
				if(cis_stream_play_update)
				{
					...
					for(int i = le_conn_count - 1; i >= 0; i--)
#else
					for(int i = 0; i < le_conn_count; i++)
#endif
					{
		...
		/* Wait for disconnect */
		ret = xSemaphoreTake(sem_disconnected, portMAX_DELAY);
		if (ret != pdTRUE) {
		    PRINTF("failed to take sem_disconnected\n");
		    break;
		}
