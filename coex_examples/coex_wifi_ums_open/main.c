 ...
 *
 * Architecture:
 *   This app combines the edgefast_open unicast_media_sender BT app with Wi-Fi CLI.
 ...
 *   - The BT task (unicast_media_sender_task) creates its own shell via
 *     le_audio_shell_init() with prompt "UMS>>".
 ...
 *   - This gives a single shell prompt (UMS>>) with both BT commands (scan,
 *     connect, play, etc.) and Wi-Fi commands (wifi wlan-scan, wifi wlan-version).
 *
 * Flow:
 *   main() -> task_main -> coex_cli_init() + coex_controller_init() [Wi-Fi init]
 *          -> xTaskCreate(unicast_media_sender_task) [BT task]
 ...
 *   - middleware/edgefast_open/examples/unicast_media_sender (BT LE Audio UMS)
 *   - examples/coex_examples/coex_wifi_ums_4cis_open (4CIS coex reference)
 */
...
void task_main(void *param)
{
    ...
    if (xTaskCreate(unicast_media_sender_task, "unicast_media_sender_task", configMINIMAL_STACK_SIZE * 8, NULL, tskIDLE_PRIORITY + 3, NULL) != pdPASS)
    {
...
int main(void)
{
    ...
    printSeparator();
    PRINTF("        Coex Wi-Fi UMS Open APP\r\n");
    printSeparator();
