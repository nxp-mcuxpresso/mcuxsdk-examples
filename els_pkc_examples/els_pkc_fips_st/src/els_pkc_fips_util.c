...
static const ALIGNED uint8_t s_CkdfDerivationDataWrapIn[12U] = {
    0xC8U, 0xACU, 0x48U, 0x88U, 0xA6U, 0x1BU,
    0x3DU, 0x9BU, 0x56U, 0xA9U, 0x75U, 0xE7U,
};

static const ALIGNED uint8_t s_ImportDieIntEcdhSk[32] = {
    0x82U, 0x9BU, 0xB4U, 0x4AU, 0x3BU, 0x6DU, 0x73U, 0x35U, 0x09U, 0x5EU, 0xD9U,
    0x8DU, 0xF6U, 0x09U, 0x89U, 0x98U, 0xACU, 0x63U, 0xABU, 0x4EU, 0x4EU, 0x78U,
    0xF6U, 0x0AU, 0x70U, 0xEAU, 0x64U, 0x92U, 0xD4U, 0xFCU, 0xE4U, 0x92U};

static const ALIGNED uint8_t s_ImportDieIntEcdhPk[64] = {
    0x8CU, 0xE2U, 0x3AU, 0x89U, 0xE7U, 0xC5U, 0xE9U, 0xB1U, 0x3EU, 0x89U, 0xEDU,
    0xDBU, 0x69U, 0xB9U, 0x22U, 0xF8U, 0xC2U, 0x8FU, 0x5DU, 0xCCU, 0x59U, 0x3EU,
    0x5FU, 0x7BU, 0x6EU, 0x5AU, 0x6CU, 0xB3U, 0x62U, 0xC0U, 0x17U, 0x8AU, 0x2FU,
    0xDAU, 0xE8U, 0x72U, 0x67U, 0x7BU, 0xDFU, 0xFEU, 0xDBU, 0x4AU, 0x6EU, 0x39U,
    0x2AU, 0x1BU, 0xAEU, 0xF8U, 0x88U, 0x8FU, 0xC5U, 0x11U, 0xC3U, 0x67U, 0x85U,
    0x5AU, 0xC5U, 0x54U, 0xBBU, 0xEBU, 0x19U, 0xF6U, 0x52U, 0x66U};
...
/*!
 * @brief Function to write a 32-bit unsigned integer in big-endian format to a
 * byte array.
 */
static void write_uint32_msb_first(uint8_t *pos, uint32_t data) {
  /* Write the most significant byte of the data to the first position in the
   * byte array */
  pos[0U] = (uint8_t)(((data) >> 24U) & 0xFFU);

  /* Write the second most significant byte of the data to the second position
   * in the byte array. And so on..
   */
...
static bool els_is_active_keyslot(mcuxClEls_KeyIndex_t key_idx) {
  ...
  key_properties.word.value =
      ((const volatile uint32_t *)(&ELS->ELS_KS0))[key_idx];
 ...
 */
static status_t els_get_key_properties(mcuxClEls_KeyIndex_t key_index,
                                       mcuxClEls_KeyProp_t *key_properties) {
  /* Wrapper function to return the key properties of a key in ELS keystore */
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token, mcuxClEls_GetKeyProperties(key_index, key_properties));
  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_GetKeyProperties) != token) ||
...
mcuxClEls_KeyIndex_t els_get_free_keyslot(uint32_t required_keyslots) {
  ...
  for (mcuxClEls_KeyIndex_t keyIdx = 0U;
       keyIdx <= (MCUXCLELS_KEY_SLOTS - required_keyslots); keyIdx++) {
 ...
 */
static status_t els_generate_keypair(mcuxClEls_KeyIndex_t *dst_key_index,
                                     uint8_t *public_key,
  ...
  /* Initialize key gen options to do a key exchange, generating random output
   * keys */
  ...
  *dst_key_index =
      (mcuxClEls_KeyIndex_t)els_get_free_keyslot(keypair_required_keyslots);
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token,
      mcuxClEls_EccKeyGen_Async(options, (mcuxClEls_KeyIndex_t)0U,
                                *dst_key_index, s_KeypairProp, NULL,
                                public_key));
...
status_t els_keygen(mcuxClEls_KeyIndex_t key_index, uint8_t *public_key,
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token,
      mcuxClEls_EccKeyGen_Async(key_gen_options, (mcuxClEls_KeyIndex_t)0,
                                key_index, key_properties, NULL,
                                &public_key[0]));
 ...
 */
static status_t
els_perform_key_agreement(mcuxClEls_KeyIndex_t keypair_index,
  ...
  uint32_t shared_secret_required_keyslots =
      get_required_keyslots(shared_secret_prop);
  *dst_key_index = els_get_free_keyslot(shared_secret_required_keyslots);
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token,
      mcuxClEls_EccKeyExchange_Async(keypair_index, public_key, *dst_key_index,
                                     shared_secret_prop));

  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_EccKeyExchange_Async) != token) ||
      (MCUXCLELS_STATUS_OK_WAIT != result)) {
    return STATUS_ERROR_GENERIC;
  }
  MCUX_CSSL_FP_FUNCTION_CALL_END();
...
status_t els_delete_key(mcuxClEls_KeyIndex_t key_index) {
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token,
                                   mcuxClEls_KeyDelete_Async(key_index));
...
static status_t els_derive_key(mcuxClEls_KeyIndex_t src_key_index,
                               mcuxClEls_KeyProp_t key_prop, const uint8_t *dd,
                               mcuxClEls_KeyIndex_t *dst_key_index) {
  ...
  *dst_key_index = els_get_free_keyslot(required_keyslots);
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token,
                                   mcuxClEls_Ckdf_Sp800108_Async(src_key_index,
                                                                 *dst_key_index,
                                                                 key_prop, dd));

  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_Ckdf_Sp800108_Async) != token) ||
      (MCUXCLELS_STATUS_OK_WAIT != result)) {
    return STATUS_ERROR_GENERIC;
  }
  MCUX_CSSL_FP_FUNCTION_CALL_END();
...
static status_t els_import_key(const uint8_t *wrapped_key,
  ...
  *dst_key_index = els_get_free_keyslot(required_keyslots);
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token,
      mcuxClEls_KeyImport_Async(options, wrapped_key, wrapped_key_size,
                                unwrap_key_index, *dst_key_index));
  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_KeyImport_Async) != token) ||
      (MCUXCLELS_STATUS_OK_WAIT != result)) {
    return STATUS_ERROR_GENERIC;
  }
  MCUX_CSSL_FP_FUNCTION_CALL_END();
...
static status_t els_get_random(unsigned char *out, size_t out_size) {
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token, mcuxClCss_Rng_DrbgRequest_Async(out, out_size));
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      result, token, mcuxClCss_WaitForOperation(MCUXCLCSS_ERROR_FLAGS_CLEAR));
  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_WaitForOperation) != token) ||
      (MCUXCLELS_STATUS_OK != result)) {
    return STATUS_ERROR_GENERIC;
  }
...
static status_t host_derive_key(const uint8_t *input_key,
                                ...
                                uint32_t key_properties, uint8_t *output,
                                size_t *output_size) {
  ...
  *output_size = length_bytes;
   ...
   * of NIST SP 800-108, Recommendation for Key Derivation Using Pseudorandom
   * Functions Derivation data[191:0](sic!) = software_derivation_data[95:0] ||
   * 64'h0 || requested_ properties[31:0 || length[31:0] || counter[31:0]
   */
  uint8_t dd[32U] = {0U};
  (void)memcpy(&dd[0U], derivation_data, 12U);
  (void)memset(&dd[12], 0, 8U);
  write_uint32_msb_first(&dd[20], key_properties);
  write_uint32_msb_first(&dd[24],
                         length_bytes * (uint32_t)8U); /* expected in bits! */
  write_uint32_msb_first(&dd[28], counter);

  mbedtls_cipher_type_t mbedtls_cipher_type = MBEDTLS_CIPHER_AES_256_ECB;
  const mbedtls_cipher_info_t *cipher_info =
      mbedtls_cipher_info_from_type(mbedtls_cipher_type);
 ...
 */
static int get_random_mbedtls_callback(void *ctx, unsigned char *out,
                                       size_t out_size) {
...
static status_t host_perform_key_agreement(const uint8_t *public_key,
  ...
  status_t status = STATUS_SUCCESS;
  ALIGNED uint8_t public_key_compressed[65U] = {0U};
  ...
  *shared_secret_size = 32U;
  ...
  ret = mbedtls_mpi_read_binary(&dA, s_ImportDieIntEcdhSk,
                                sizeof(s_ImportDieIntEcdhSk));
  CHECK_MBEDTLS_SUCCESS();

  public_key_compressed[0U] = 0x04U;
  (void)memcpy(&public_key_compressed[1], public_key, public_key_size);

  ret = mbedtls_ecp_point_read_binary(&grp, &qB, public_key_compressed,
                                      sizeof(public_key_compressed));
  CHECK_MBEDTLS_SUCCESS();

  ret = mbedtls_ecdh_compute_shared(&grp, &zA, &qB, &dA,
                                    &get_random_mbedtls_callback, NULL);
  CHECK_MBEDTLS_SUCCESS();

  ret = mbedtls_ecp_point_write_binary(&grp, &qB, MBEDTLS_ECP_PF_UNCOMPRESSED,
                                       &strlen, &strbuf[0U], sizeof(strbuf));
  CHECK_MBEDTLS_SUCCESS();
 ...
 */
static status_t host_wrap_key(const uint8_t *data, size_t data_size,
  ...
  ret = mbedtls_nist_kw_setkey(&ctx, MBEDTLS_CIPHER_ID_AES, key, 16U * 8U,
                               (int)true);
  CHECK_MBEDTLS_SUCCESS();

  /* Key wrap using mbedTLS */
  ret = mbedtls_nist_kw_wrap(&ctx, MBEDTLS_KW_MODE_KW, data, data_size, output,
...
static status_t create_els_import_keyblob(const uint8_t *plain_key,
  ...
  ALIGNED uint8_t buffer[ELS_BLOB_METADATA_SIZE + MAX_ELS_KEY_SIZE] = {0U};
  ...
  status_t status =
      host_wrap_key(buffer, buffer_size, key_wrap_in, blob, blob_size);
  return status;
}
...
status_t import_plain_key_into_els(const uint8_t *plain_key,
  ...
  ALIGNED uint8_t shared_secret[32U] = {0U};
  ALIGNED uint8_t key_wrap_in[32U];
  ALIGNED uint8_t els_key_in_blob[ELS_BLOB_METADATA_SIZE + MAX_ELS_KEY_SIZE +
                                  ELS_WRAP_OVERHEAD];
  ...
  if (els_generate_keypair(&index_plain, &public_key[0U], &public_key_size) !=
      STATUS_SUCCESS) {
    (void)els_delete_key(index_plain);
    return STATUS_ERROR_GENERIC;
  }

  if (host_perform_key_agreement(public_key, public_key_size,
                                 &shared_secret[0U],
                                 &shared_secret_len) != STATUS_SUCCESS) {
    (void)els_delete_key(index_plain);
    return STATUS_ERROR_GENERIC;
  }

  /* CKDF with mbedTLS */
  if (host_derive_key(shared_secret, s_CkdfDerivationDataWrapIn,
                      s_WrapInKeyProp.word.value, &key_wrap_in[0],
  ...
  if (create_els_import_keyblob(plain_key, key_properties, key_wrap_in,
                                &els_key_in_blob[0],
                                &els_key_in_blob_size) != STATUS_SUCCESS) {
    (void)els_delete_key(index_plain);
    return STATUS_ERROR_GENERIC;
  }

  /* Key exchange */
  if (els_perform_key_agreement(index_plain, s_SharedSecretProp,
                                &index_shared_secret, s_ImportDieIntEcdhPk,
  ...
  /* CKDF with ELS */
  if (els_derive_key(index_shared_secret, s_WrapInKeyProp,
                     s_CkdfDerivationDataWrapIn,
                     &index_unwrap) != STATUS_SUCCESS) {
    (void)els_delete_key(index_shared_secret);
    return STATUS_ERROR_GENERIC;
  }
  ...
  if (els_import_key(els_key_in_blob, els_key_in_blob_size, key_properties,
                     index_unwrap, index_output) != STATUS_SUCCESS) {
    (void)els_delete_key(index_unwrap);
    return STATUS_ERROR_GENERIC;
  }
