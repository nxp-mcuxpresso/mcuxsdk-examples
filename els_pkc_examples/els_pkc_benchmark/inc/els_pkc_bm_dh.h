 ...
 * @param bit_length Bit length of algorithm (P and N).
 * @param a_result Struct for the algorithm result. Setting point multiplication
 * per second and secret key calculation per second.
 * @retval MCUXCLEXAMPLE_STATUS_ERROR If error in algorithm happens.
 * @retval MCUXCLEXAMPLE_STATUS_OK If algorithm succeeds.
 */
bool exec_ecdh_point_mult(char *data_from, uint32_t bit_length,
