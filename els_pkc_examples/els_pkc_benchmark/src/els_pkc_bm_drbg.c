    ...
    if ((data_from_ram)) {                                                     \
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          ...
          mcuxClRandom_generate(                                               \
              (session),                                                       \
              (block_amount) == SINGLE_BLOCK ? s_DrbgBufferSingleBlock         \
                                             : s_DrbgBuffer,                   \
              (block_amount) == SINGLE_BLOCK ? sizeof(s_DrbgBufferSingleBlock) \
                                             : sizeof(s_DrbgBuffer)));         \
      if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClRandom_generate) !=              \
           generate_token) ||                                                  \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          rg_status, generate_token,                                           \
          mcuxClRandom_generate((session),                                     \
                                (block_amount) == SINGLE_BLOCK                 \
                                    ? s_DrbgBufferSingleBlockFlash             \
                                    : s_DrbgBufferFlash,                       \
                                (block_amount) == SINGLE_BLOCK                 \
                                    ? sizeof(s_DrbgBufferSingleBlockFlash)     \
                                    : sizeof(s_DrbgBufferFlash)));             \
      if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClRandom_generate) !=              \
           generate_token) ||                                                  \
          (MCUXCLRANDOM_STATUS_OK != rg_status)) {                             \
        PRINTF("[Error] CTR random generation failed\r\n");                    \
 ...
 * Code
 ******************************************************************************/
bool exec_ctr_drbg(char *data_from, uint32_t security_strength,
  ...
  ALLOCATE_AND_INITIALIZE_SESSION(session,
                                  MCUXCLRANDOMMODES_MAX_CPU_WA_BUFFER_SIZE, 0U);

  /**************************************************************************/
  /* DRBG selftest.                                                         */
  /**************************************************************************/
  mcuxClRandom_Mode_t random_mode =
      security_strength == 128U ? mcuxClRandomModes_Mode_ELS_Drbg
                                : mcuxClRandomModes_Mode_CtrDrbg_AES256_DRG3;

  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(rs_status, selftest_token,
                                   mcuxClRandom_selftest(session, random_mode));
  ...
  uint32_t context[MCUXCLRANDOMMODES_CTR_DRBG_AES256_CONTEXT_SIZE_IN_WORDS] = {
      0U};
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      ri_status, init_token,
      mcuxClRandom_init(session, (mcuxClRandom_Context_t)context, random_mode));

  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClRandom_init) != init_token) ||
      (MCUXCLRANDOM_STATUS_OK != ri_status)) {
  ...
  a_result->cyclesPerBlock =
      COMPUTE_CYCLES(CTR_DRBG(session, block_amount, data_from_ram),
                     block_amount * 16U, iteration_amount);
  a_result->cyclesPerByte = a_result->cyclesPerBlock / 16U;
  a_result->kbPerS =
      KB_S(CTR_DRBG(session, block_amount, data_from_ram), block_amount * 16U);
  /**************************************************************************/
  /* Cleanup                                                                */
  /**************************************************************************/

  /* Random uninit. */
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(ru_status, uninit_token,
                                   mcuxClRandom_uninit(session));

  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClRandom_uninit) != uninit_token) ||
      (MCUXCLRANDOM_STATUS_OK != ru_status)) {
...
void test_drbg(char *code_from, char *data_from, uint32_t security_strength,
               uint32_t block_amount, const bool cache_enable) {
  ...
  exec_ctr_drbg(data_from, security_strength, block_amount, &a_result,
                cache_enable);
