  ...
  do {                                                                         \
    ...
    if ((data_from_ram)) {                                                     \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result, token,                                                       \
          mcuxClMac_compute(                                                   \
              (session), (key), mcuxClMac_Mode_CMAC,                           \
              (block_amount) == SINGLE_BLOCK ? s_CmacInputSingleBlock          \
                                             : s_CmacInput,                    \
              (block_amount) == SINGLE_BLOCK ? sizeof(s_CmacInputSingleBlock)  \
                                             : sizeof(s_CmacInput),            \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result, token,                                                       \
          mcuxClMac_compute((session), (key), mcuxClMac_Mode_CMAC,             \
  ...
  do {                                                                         \
    ...
    if ((data_from_ram)) {                                                     \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          ...
          mcuxClMac_compute((session), (key), mode,                            \
                            (block_amount) == SINGLE_BLOCK                     \
                            ...
                            (block_amount) == SINGLE_BLOCK                     \
                                ...
                                : sizeof(s_HmacInput),                         \
                            result_buffer, &result_size));                     \
      if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClMac_compute) !=                  \
           macCompute_token) ||                                                \
...
bool exec_cmac(uint32_t block_amount, char *data_from,
  ...
  /* Example AES-128 key */
  static uint8_t s_Key128[MCUXCLAES_AES128_KEY_SIZE] = {
      0x7CU, 0x0BU, 0x7DU, 0xB9U, 0x81U, 0x1FU, 0x10U, 0xD0U,
  ...
  static uint8_t s_CmacInputSingleBlock[MCUXCLAES_BLOCK_SIZE] = {
      0x1EU, 0xE0U, 0xECU, 0x46U, 0x6DU, 0x46U, 0xFDU, 0x84U,
      0x9BU, 0x40U, 0xC0U, 0x66U, 0xB4U, 0xFBU, 0xBDU, 0x22U};
  ...
  case MCUXCLAES_AES128_KEY_SIZE: {
    ...
    if (!cl_key_init_and_load(session, key, mcuxClKey_Type_Aes128,
                              data_from_ram ? (mcuxCl_Buffer_t)s_Key128
                                            : (mcuxCl_Buffer_t)s_Key128Flash,
                              data_from_ram ? sizeof(s_Key128)
                                            : sizeof(s_Key128Flash),
                              &cmac_key_properties, key_buffer, 0U)) {
  ...
  case MCUXCLAES_AES256_KEY_SIZE: {
    ...
    if (!cl_key_init_and_load(session, key, mcuxClKey_Type_Aes256,
                              data_from_ram ? (mcuxCl_Buffer_t)s_Key256
                                            : (mcuxCl_Buffer_t)s_Key256Flash,
                              data_from_ram ? sizeof(s_Key256)
                                            : sizeof(s_Key256Flash),
                              &cmac_key_properties, key_buffer, 0U)) {
      PRINTF("[Error] Key initialization failed\r\n");
      return BM_STATUS_ERROR;
    }
  ...
  a_result->cyclesPerBlock =
      COMPUTE_CYCLES(COMPUTE_CMAC(session, key, block_amount, data_from_ram),
                     block_amount, iteration_amount);
  a_result->cyclesPerByte = a_result->cyclesPerBlock / 16U;
  a_result->kbPerS =
      KB_S(COMPUTE_CMAC(session, key, block_amount, data_from_ram),
           block_amount * 16U);
...
bool exec_hmac(uint32_t block_amount, char *data_from,
  ...
  static uint8_t s_Key[] = {0x00U, 0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x66U,
                            0x77U, 0x88U, 0x99U, 0xAAU, 0xBBU, 0xCCU, 0xDDU,
                            0xEEU, 0xFFU, 0x00U, 0x11U, 0x22U, 0x33U, 0x44U,
                            0x55U, 0x66U, 0x77U, 0x88U, 0x99U, 0xAAU, 0xBBU,
  ...
  static uint8_t s_HmacInputSingleBlock[128U] = {
  ...
  static const uint8_t s_KeyFlash[] = {
      0x00U, 0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x66U, 0x77U,
      0x88U, 0x99U, 0xAAU, 0xBBU, 0xCCU, 0xDDU, 0xEEU, 0xFFU,
      0x00U, 0x11U, 0x22U, 0x33U, 0x44U, 0x55U, 0x66U, 0x77U,
      0x88U, 0x99U, 0xAAU, 0xBBU, 0xCCU, 0xDDU, 0xEEU, 0xFFU};
  /* Example single block input to the HMAC function stored in flash */
  static const uint8_t s_HmacInputSingleBlockFlash[128U] = {
      0x00U, 0x9FU, 0x5EU, 0x39U, 0x94U, 0x30U, 0x03U, 0x82U,
      0x50U, 0x72U, 0x1BU, 0xE1U, 0x79U, 0x65U, 0x35U, 0xFFU};
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(
      ...
      mcuxClKey_init(
          ...
          data_from_ram ? sizeof(s_Key) : sizeof(s_KeyFlash)));
  ...
  const uint32_t iteration_amount = cache_enable ? 1024U : 1U;
  a_result->cyclesPerBlock =
      COMPUTE_CYCLES(COMPUTE_HMAC(session, key, block_amount, data_from_ram),
                     block_amount, iteration_amount);
  a_result->cyclesPerByte = a_result->cyclesPerBlock / 128U;
  a_result->kbPerS =
      KB_S(COMPUTE_HMAC(session, key, block_amount, data_from_ram),
           block_amount * 128U);
...
void run_tests_mac(void) {
  ...
  PRINTF("AES-CMAC-128:\r\n");
  test_cmac(code_from, "FLASH", MULTIPLE_BLOCKS, MCUXCLAES_AES128_KEY_SIZE,
            false);
  test_cmac(code_from, "FLASH", MULTIPLE_BLOCKS, MCUXCLAES_AES128_KEY_SIZE,
            true);
  test_cmac(code_from, "RAM", MULTIPLE_BLOCKS, MCUXCLAES_AES128_KEY_SIZE,
            false);
  ...
  PRINTF("AES-CMAC-256:\r\n");
  test_cmac(code_from, "FLASH", MULTIPLE_BLOCKS, MCUXCLAES_AES256_KEY_SIZE,
            false);
  test_cmac(code_from, "FLASH", MULTIPLE_BLOCKS, MCUXCLAES_AES256_KEY_SIZE,
            true);
  test_cmac(code_from, "RAM", MULTIPLE_BLOCKS, MCUXCLAES_AES256_KEY_SIZE,
            false);
