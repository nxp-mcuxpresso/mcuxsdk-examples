...
#define CKDF_SP800108(key_properties, delete_target_index)                     \
  do {                                                                         \
    ...
    uint8_t der_data[12U] = {0x00U, 0x65U, 0x32U, 0x67U, 0x63U, 0x6FU,         \
                             0x6EU, 0x5FU, 0x73U, 0x65U, 0x00U, 0x00U};        \
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                          \
        result_ckdf, token_async,                                              \
        mcuxClEls_Ckdf_Sp800108_Async(0x06U, 0x0EU, (key_properties),          \
                                      der_data));                              \
    if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_Ckdf_Sp800108_Async) !=        \
         token_async) ||                                                       \
        (MCUXCLELS_STATUS_OK_WAIT != result_ckdf)) {                           \
      PRINTF("[Error] ckdf-sp800108 failed\r\n");                              \
      return BM_STATUS_ERROR;                                                  \
    }                                                                          \
    MCUX_CSSL_FP_FUNCTION_CALL_END();                                          \
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                          \
        result_wait, token_wait,                                               \
...
#define HKDF_SP80056C()                                                        \
  do {                                                                         \
    ...
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                          \
        result_ckdf, token_async,                                              \
        mcuxClEls_Hkdf_Sp80056c_Async(0x0EU, out, label_buf,                   \
                                      sizeof(label_buf)));                     \
...
#define HKDF_RFC5869(options, key_properties, delete_target_index)             \
  do {                                                                         \
    ...
    uint8_t der_data[MCUXCLELS_HKDF_RFC5869_DERIVATIONDATA_SIZE] = {           \
        0x00U, 0x65U, 0x32U, 0x67U, 0x63U, 0x6FU,                              \
        0x6EU, 0x5FU, 0x73U, 0x65U, 0x00U, 0x00U};                             \
    MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                          \
        result_ckdf, token_async,                                              \
        mcuxClEls_Hkdf_Rfc5869_Async((options), 0x0EU, 0x04U,                  \
                                     (key_properties), der_data));             \
...
#define DELETE_KEY(key_index)                                                  \
  do { /* Delete existing key for storing it for derivation */                 \
...
bool exec_hkdf_rfc5869(algorithm_result *a_result) {
  ...
  a_result->kbPerS = KB_S(HKDF_RFC5869(opt, key_properties, true), 32U) -
                     KB_S(DELETE_KEY(0x04U), 32U);
...
bool exec_hkdf_sp80056c(algorithm_result *a_result) {
  ...
  a_result->cyclesPerBlock =
      COMPUTE_CYCLES(HKDF_SP80056C(), 1U, 1024U); /* Cycles/Key */
  a_result->cyclesPerByte = a_result->cyclesPerBlock / 32U;
...
bool exec_ckdf_Sp800108(algorithm_result *a_result, uint16_t key_size) {
  ...
  a_result->kbPerS =
      KB_S(CKDF_SP800108(key_properties, true), (key_size / 8U)) -
      KB_S(DELETE_KEY(0x0EU), (key_size / 8U));
  /*************************************************************************/
  /* Cleanup                                                                */
  /**************************************************************************/
...
void test_ckdf_Sp800108(char *code_from, uint16_t key_size) {
  ...
  strcpy(a_result.execution,
         key_size == KEY_SIZE_256 ? "256 BIT KEY" : "128 BIT KEY");
