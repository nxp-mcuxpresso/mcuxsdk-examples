  ...
  do {                                                                         \
    ...
    if ((data_from_ram)) {                                                     \
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          ...
          mcuxClCipher_crypt(                                                  \
              (session), (key), (mode),                                        \
              (mode) == mcuxClCipher_Mode_AES_ECB_Enc_NoPadding ? NULL         \
                                                                : s_Aes128Iv,  \
              (mode) == mcuxClCipher_Mode_AES_ECB_Enc_NoPadding                \
                  ? 0U                                                         \
                  : sizeof(s_Aes128Iv),                                        \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlock           \
                                             : s_MsgPlain,                     \
              (block_amount) == SINGLE_BLOCK ? sizeof(s_MsgPlainSingleBlock)   \
                                             : sizeof(s_MsgPlain),             \
              (block_amount) == SINGLE_BLOCK ? s_MsgEncSingleBlock : s_MsgEnc, \
              &msg_enc_size));                                                 \
                                                                               \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          ...
          mcuxClCipher_crypt(                                                  \
              ...
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlockFlash      \
                                             : s_MsgPlainFlash,                \
              (block_amount) == SINGLE_BLOCK                                   \
  ...
  do {                                                                         \
    ...
    if ((data_from_ram)) {                                                     \
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result_enc, token_enc,                                               \
          mcuxClAead_crypt(                                                    \
              (session), (key), (mode), s_Nonce64, sizeof(s_Nonce64),          \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlock           \
                                             : s_MsgPlain,                     \
              (block_amount) == SINGLE_BLOCK ? sizeof(s_MsgPlainSingleBlock)   \
                                             : sizeof(s_MsgPlain),             \
              s_MsgAdata, sizeof(s_MsgAdata),                                  \
              (block_amount) == SINGLE_BLOCK ? s_MsgEncSingleBlock : s_MsgEnc, \
              &msg_enc_size, msg_tag, sizeof(msg_tag)));                       \
    ...
    {                                                                          \
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result_enc, token_enc,                                               \
          mcuxClAead_crypt(                                                    \
              (session), (key), (mode), s_Nonce64Flash,                        \
              sizeof(s_Nonce64Flash),                                          \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlockFlash      \
                                             : s_MsgPlainFlash,                \
              (block_amount) == SINGLE_BLOCK                                   \
                  ? sizeof(s_MsgPlainSingleBlockFlash)                         \
                  : sizeof(s_MsgPlainFlash),                                   \
              s_MsgAdataFlash, sizeof(s_MsgAdataFlash),                        \
              (block_amount) == SINGLE_BLOCK ? s_MsgEncSingleBlock : s_MsgEnc, \
              &msg_enc_size, msg_tag, sizeof(msg_tag)));                       \
      if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClAead_crypt) != token_enc) ||     \
          (MCUXCLAEAD_STATUS_OK != result_enc)) {                              \
  ...
  do {                                                                         \
    if ((data_from_ram)) {                                                     \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result_enc, token_enc,                                               \
          mcuxClCipher_crypt(                                                  \
              (session), (key), mcuxClCipher_Mode_AES_CBC_Enc_NoPadding,       \
              s_Aes128Iv, sizeof(s_Aes128Iv),                                  \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlock           \
                                             : s_MsgPlain,                     \
              (block_amount) == SINGLE_BLOCK ? sizeof(s_MsgPlainSingleBlock)   \
                                             : sizeof(s_MsgPlain),             \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result_enc, token_enc,                                               \
          mcuxClCipher_crypt(                                                  \
              (session), (key), mcuxClCipher_Mode_AES_CTR, s_Aes128Iv,         \
              sizeof(s_Aes128Iv),                                              \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlock           \
                                             : s_MsgPlain,                     \
              (block_amount) == SINGLE_BLOCK ? sizeof(s_MsgPlainSingleBlock)   \
    ...
    {                                                                          \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          ...
          mcuxClCipher_crypt(                                                  \
              (session), (key), mcuxClCipher_Mode_AES_CBC_Enc_NoPadding,       \
                                                                               \
              s_Aes128Iv, sizeof(s_Aes128IvFlash),                             \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlockFlash      \
                                             : s_MsgPlainFlash,                \
              (block_amount) == SINGLE_BLOCK                                   \
                  ? sizeof(s_MsgPlainSingleBlockFlash)                         \
      ...
      MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(                                        \
          result_enc, token_enc,                                               \
          mcuxClCipher_crypt(                                                  \
              (session), (key), mcuxClCipher_Mode_AES_CTR, s_Aes128IvFlash,    \
              sizeof(s_Aes128IvFlash),                                         \
              (block_amount) == SINGLE_BLOCK ? s_MsgPlainSingleBlockFlash      \
                                             : s_MsgPlainFlash,                \
              (block_amount) == SINGLE_BLOCK                                   \
...
static uint8_t s_Aes128Key[16U] = {0x2BU, 0x7EU, 0x15U, 0x16U, 0x28U, 0xAEU,
                                   0xD2U, 0xA6U, 0xABU, 0xF7U, 0x15U, 0x88U,
                                   0x09U, 0xCFU, 0x4FU, 0x3CU};

/** 192 bit key for the AES encryption */
static uint8_t s_Aes192Key[24U] = {0x2BU, 0x7EU, 0x15U, 0x16U, 0x28U, 0xAEU,
                                   0xD2U, 0xA6U, 0xABU, 0xF7U, 0x15U, 0x88U,
                                   0x09U, 0xCFU, 0x4FU, 0x3CU, 0xABU, 0xF7U,
                                   0x15U, 0x88U, 0x09U, 0xCFU, 0x4FU, 0x3CU};

/** 256 bit key for the AES encryption */
static uint8_t s_Aes256Key[32U] = {
    0x2BU, 0x7EU, 0x15U, 0x16U, 0x28U, 0xAEU, 0xD2U, 0xA6U, 0xABU, 0xF7U, 0x15U,
...
static uint8_t s_Aes128Iv[MCUXCLAES_BLOCK_SIZE] = {
...
static uint8_t s_MsgPlainSingleBlock[MCUXCLAES_BLOCK_SIZE] = {
...
static uint8_t s_MsgAdata[MCUXCLAES_BLOCK_SIZE] = {
    0x00U, 0x01U, 0x02U, 0x03U, 0x04U, 0x05U, 0x06U, 0x07U,
    0x08U, 0x09U, 0x0AU, 0x0BU, 0x0CU, 0x0DU, 0x0EU, 0x0FU};

/** Nonce for AEAD */
static uint8_t s_Nonce64[8U] = {0x10U, 0x21U, 0x32U, 0x43U,
                                0x54U, 0x65U, 0x76U, 0x87U};

/* Variables stored in flash */

/** 128 bit key for the AES encryption stored in flash */
static const uint8_t s_Aes128KeyFlash[16U] = {
    0x2BU, 0x7EU, 0x15U, 0x16U, 0x28U, 0xAEU, 0xD2U, 0xA6U,
    0xABU, 0xF7U, 0x15U, 0x88U, 0x09U, 0xCFU, 0x4FU, 0x3CU};

/** 192 bit key for the AES encryption stroed in flash */
static const uint8_t s_Aes192KeyFlash[24U] = {
    0x2BU, 0x7EU, 0x15U, 0x16U, 0x28U, 0xAEU, 0xD2U, 0xA6U,
    0xABU, 0xF7U, 0x15U, 0x88U, 0x09U, 0xCFU, 0x4FU, 0x3CU,
    0xABU, 0xF7U, 0x15U, 0x88U, 0x09U, 0xCFU, 0x4FU, 0x3CU};
...
static const uint8_t s_Aes128IvFlash[MCUXCLAES_BLOCK_SIZE] = {
    0xF8U, 0xD2U, 0x68U, 0x76U, 0x81U, 0x6FU, 0x0FU, 0xBAU,
    0x86U, 0x2BU, 0xD8U, 0xA3U, 0x2DU, 0x04U, 0x67U, 0xC3U};
...
static const uint8_t s_MsgAdataFlash[MCUXCLAES_BLOCK_SIZE] = {
    0x00U, 0x01U, 0x02U, 0x03U, 0x04U, 0x05U, 0x06U, 0x07U,
    0x08U, 0x09U, 0x0AU, 0x0BU, 0x0CU, 0x0DU, 0x0EU, 0x0FU};

/** Nonce for AEAD stored in flash */
static const uint8_t s_Nonce64Flash[8U] = {0x10U, 0x21U, 0x32U, 0x43U,
                                           0x54U, 0x65U, 0x76U, 0x87U};
...
bool exec_cl_cipher_mode(mcuxClCipher_Mode_t mode, uint32_t block_amount,
  ...
  /* Load key */
  uint32_t dst_data[8U];
  switch (key_size) {
  case AES_128_KEY: {
    key_properties.bits.ksize = MCUXCLELS_KEYPROPERTY_KEY_SIZE_128;
    if (!cl_key_init_and_load(session, key, mcuxClKey_Type_Aes128,
                              data_from_ram ? (mcuxCl_Buffer_t)s_Aes128Key
                                            : (mcuxCl_Buffer_t)s_Aes128KeyFlash,
                              key_size, &key_properties, dst_data, 0U)) {
  ...
  case AES_192_KEY: {
    if (!cl_key_init_and_load(session, key, mcuxClKey_Type_Aes192,
                              data_from_ram ? (mcuxCl_Buffer_t)s_Aes192Key
                                            : (mcuxCl_Buffer_t)s_Aes192KeyFlash,
                              key_size, &key_properties, dst_data, 0U)) {
      PRINTF("[Error] Key initialization failed\r\n");
      return BM_STATUS_ERROR;
    }
  ...
  case AES_256_KEY: {
    ...
    if (!cl_key_init_and_load(session, key, mcuxClKey_Type_Aes256,
                              data_from_ram ? (mcuxCl_Buffer_t)s_Aes256Key
                                            : (mcuxCl_Buffer_t)s_Aes256KeyFlash,
                              key_size, &key_properties, dst_data, 0U)) {
      PRINTF("[Error] Key initialization failed\r\n");
      return BM_STATUS_ERROR;
    }
  ...
  a_result->cyclesPerBlock = COMPUTE_CYCLES(
      AES_ENCRYPT(mode, data_from_ram, key, session, block_amount),
      block_amount, iteration_amount);
  a_result->cyclesPerByte = a_result->cyclesPerBlock / 16U;
  a_result->kbPerS =
      KB_S(AES_ENCRYPT(mode, data_from_ram, key, session, block_amount),
           block_amount * 16U);
...
bool exec_cl_aead_mode(mcuxClAead_Mode_t mode, uint32_t block_amount,
  ...
  ALLOCATE_AND_INITIALIZE_SESSION(session,
                                  MCUXCLAEAD_CRYPT_CPU_WA_BUFFER_SIZE +
  ...
  if (key_size == AES_192_KEY) {
    a_result->cyclesPerBlock =
        COMPUTE_CYCLES(AES_CCM_192(data_from_ram, key, session, block_amount),
                       block_amount, iteration_amount);
    a_result->cyclesPerByte = a_result->cyclesPerBlock / 16U;
    a_result->kbPerS =
        KB_S(AES_CCM_192(data_from_ram, key, session, block_amount),
             block_amount * 16U);
  } else {
    a_result->cyclesPerBlock = COMPUTE_CYCLES(
        AEAD_ENCRYPT(mode, data_from_ram, key, session, block_amount),
        block_amount, iteration_amount);
    a_result->cyclesPerByte = a_result->cyclesPerBlock / 16U;
    a_result->kbPerS =
        KB_S(AEAD_ENCRYPT(mode, data_from_ram, key, session, block_amount),
             block_amount * 16U);
  }
...
void test_aes_cbc(char *code_from, char *data_from, uint32_t block_amount,
  ...
  exec_cl_cipher_mode(mcuxClCipher_Mode_AES_CBC_Enc_NoPadding, block_amount,
                      data_from, &a_result, key_size, cache_enable);
...
void test_aes_ecb(char *code_from, char *data_from, uint32_t block_amount,
  ...
  strcpy(a_result.code, code_from);
  strcpy(a_result.data, data_from);
  exec_cl_cipher_mode(mcuxClCipher_Mode_AES_ECB_Enc_NoPadding, block_amount,
                      data_from, &a_result, key_size, cache_enable);
  strcpy(a_result.cached, cache_enable ? "YES" : "NO");
...
void test_aes_ctr(char *code_from, char *data_from, uint32_t block_amount,
  ...
  strcpy(a_result.code, code_from);
  strcpy(a_result.data, data_from);
  exec_cl_cipher_mode(mcuxClCipher_Mode_AES_CTR, block_amount, data_from,
                      &a_result, key_size, cache_enable);
  strcpy(a_result.cached, cache_enable ? "YES" : "NO");
...
void test_aes_ccm(char *code_from, char *data_from, uint32_t block_amount,
  ...
  strcpy(a_result.code, code_from);
  strcpy(a_result.data, data_from);
  exec_cl_aead_mode(mcuxClAead_Mode_AES_CCM_ENC, block_amount, data_from,
                    &a_result, key_size, cache_enable);
  strcpy(a_result.cached, cache_enable ? "YES" : "NO");
...
void test_aes_gcm(char *code_from, char *data_from, uint32_t block_amount,
  ...
  strcpy(a_result.code, code_from);
  strcpy(a_result.data, data_from);
  exec_cl_aead_mode(mcuxClAead_Mode_AES_GCM_ENC, block_amount, data_from,
                    &a_result, key_size, cache_enable);
  strcpy(a_result.cached, cache_enable ? "YES" : "NO");
