...
static void CRYPTO_InitHardware(void) {
  ...
  if (status != kStatus_Success) {
    PRINTF("[Error] ELS_PowerDownWakeupInit(ELS) failed\r\n");
  }
  /* Enable PKC related clocks and RAM zeroize */
  status = PKC_PowerDownWakeupInit(PKC);
  if (status != kStatus_Success) {
    PRINTF("[Error] ELS_PowerDownWakeupInit(PKC) failed\r\n");
  }
#if defined(FSL_FEATURE_SOC_TRNG_COUNT) && (FSL_FEATURE_SOC_TRNG_COUNT > 0)
  /* Initilize the TRNG driver */
  {
    ...
    /* Set sample mode of the TRNG ring oscillator to Von Neumann, for better
     * random data.*/
    /* Initialize TRNG */
...
int main(void) {
  ...
  PRINTF("   -FOR AES-CCM-192 DOING FIRST CBC AND THEN CTR, BECAUSE NO "
         "AES-CCM-192 SUPPORTED BY ELS PKC\r\n");
  PRINTF("\r\n\n");

  /* Enable the ELS */
  MCUX_CSSL_FP_FUNCTION_CALL_BEGIN(result, token, mcuxClEls_Enable_Async());
  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_Enable_Async) != token) ||
      (MCUXCLELS_STATUS_OK_WAIT != result)) {
    PRINTF("[Error] ELS init failed\r\n");
  }
  ...
  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClEls_Disable) != token) ||
      (MCUXCLELS_STATUS_OK != result)) {
    PRINTF("[Error] ELS de-init failed\r\n");
  }
  MCUX_CSSL_FP_FUNCTION_CALL_END();
