...
bool exec_ecc_weier_key_gen(char *data_from, uint32_t bit_length) {
  ...
  ALLOCATE_AND_INITIALIZE_SESSION(
      session, MCUXCLECC_KEYGEN_WACPU_SIZE,
      MCUXCLECC_KEYGEN_WAPKC_SIZE(p_byte_length, n_byte_length));
  ...
  MCUX_CSSL_FP_FUNCTION_CALL_PROTECTED(randomUninit_result, randomUninit_token,
                                       mcuxClRandom_uninit(session));
  if ((MCUX_CSSL_FP_FUNCTION_CALLED(mcuxClRandom_uninit) !=
       randomUninit_token) ||
...
void test_ecc_weier_key_gen(char *code_from, char *data_from,
  ...
  /* key-gen/s */
  a_result.signPerS =
      TIME_PUBLIC(exec_ecc_weier_key_gen(data_from, bit_length));
