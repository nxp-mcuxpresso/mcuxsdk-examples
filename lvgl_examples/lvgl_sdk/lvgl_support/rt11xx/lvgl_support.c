...
static void DEMO_FlushDisplay(lv_display_t *disp_drv, const lv_area_t *area, uint8_t *color_p)
{
    ...
    for (uint32_t y = 0; y < ROTATED_FB_HEIGHT; y++)
    {
        for (uint32_t x = 0; x < ROTATED_FB_WIDTH; x++)
        {
            ((lv_color_t *)inactiveFrameBuffer)[(DEMO_FB_HEIGHT - x) * DEMO_FB_WIDTH + y] =
                color_p[y * ROTATED_FB_WIDTH + x];
        }
    }
#endif
