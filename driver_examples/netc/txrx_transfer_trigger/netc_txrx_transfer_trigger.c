...
{
        netc_timer_alarm_t alarm;

        alarm.enableInterrupt = 1;
        alarm.polarity = 0;
...
void *netc_1588_init(void)
{
    ...
    timer_config.refClkHz = HAL_ClockGetRate(hal_clock_enettimer1);
...
int main(void)
{
    ...
    timer_handler = netc_1588_init();
    if (!timer_handler)
	return 0;
    NETC_TimerEnable(timer_handler, true);
    netc_fiper_init();
