 ...
 * Definitions
 ******************************************************************************/
#ifndef DEMO_CLOCK_SOURCE_COUNT
...
int main(void)
{
    ...
    (void)EnableIRQWithPriority(Freqme_IRQn, FREQME_IRQ_PRIO);
...
static void DEMO_DoFreqMeasurement(void)
{
     ...
     */
    INPUTMUX_AttachSignal(0UL, DEMO_FREQ_REF_CLK_SOURCE);

    FREQME_SetMaxExpectedValue(DEMO_FREQME, DEMO_MAXEXPECTVALUE);
    FREQME_SetMinExpectedValue(DEMO_FREQME, DEMO_MINEXPECTVALUE);

    FREQME_EnableInterrupts(DEMO_FREQME,
        kFREQME_ReadyInterruptEnable | kFREQME_UnderflowInterruptEnable | kFREQME_OverflowInterruptEnable);
    ...
    {
        targetFreqCount = FREQME_GetMeasurementResult(DEMO_FREQME);
        PRINTF("Target clock frequency count is %ld.\r\n", targetFreqCount);
    }

    FREQME_DisableInterrupts(DEMO_FREQME,
        kFREQME_ReadyInterruptEnable | kFREQME_UnderflowInterruptEnable | kFREQME_OverflowInterruptEnable);
}
...
static void DEMO_DoPulseMeasurement(void)
{
     ...
     */
    FREQME_GetDefaultConfig(&config);
    config.operateMode = kFREQME_PulseWidthMeasurementMode;
    DEMO_GetClockSelection(&config);
     ...
     */
    INPUTMUX_AttachSignal(0UL, DEMO_PULSE_REF_CLK_SOURCE);
...
static void DEMO_GetClockSelection(freq_measure_config_t *config)
{
    ...
    while (1)
    {
        ...
        if (chInput < DEMO_CLOCK_SOURCE_COUNT)
        {
            if (config->operateMode == kFREQME_FreqMeasurementMode)
            {
                INPUTMUX_AttachSignal(0UL, g_freqClockSourceSignalArray[(uint8_t)chInput]);
            }
            else
            {
                INPUTMUX_AttachSignal(0UL, g_pulseClockSourceSignalArray[(uint8_t)chInput]);
            }
            break;
        }
        else
