 ...
 * Definitions
 ******************************************************************************/
#define EXAMPLE_LPUART_BASEADDR                    LPUART
#define DEMO_LPUART_CLOCK_FREQ                     90000000U
...
static status_t UART_Receive_Blocking(uint32_t *numOfReveive);
static status_t UART_Send_Blocking(void);
...
static status_t send_boot_evt()
{
    ...
    bStatus = LPUART_PollTransmit(EXAMPLE_LPUART_BASEADDR, transfer->startAddress, transfer->bufferLength);
    return bStatus;
}
...
static uint32_t ReceiveTlvCommand(void)
{
    ...
    status = UART_Receive_Blocking(/*&g_hifUartHandle, 1,*/ &dwTotalBytesRxd);
...
static status_t UART_Receive_Blocking(uint32_t *numOfReveive)
{
    ...
    do
    {
        ...
        *numOfReveive = 0;
        ...
        status = LPUART_PollReceive(EXAMPLE_LPUART_BASEADDR, transfer->startAddress, numOfReveive);
        if (status != kStatus_Success)
        {
...
status_t UART_Send_Blocking(void)
{
    status_t status;
    do
    {
        status = LPUART_PollTransmit(EXAMPLE_LPUART_BASEADDR, transfer->startAddress, transfer->bufferLength);
        if (status != kStatus_Success)
        {
