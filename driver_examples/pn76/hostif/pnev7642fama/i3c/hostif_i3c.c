 ...
 * Prototypes
 ******************************************************************************/
void BOARD_WriteGpio(uint8_t value);
static void tlv_dispatcher(void);
static status_t send_boot_evt(void);
static uint32_t ReceiveTlvCommand(void);
static status_t I3C_Receive_Blocking(hostif_handle_t *handle, uint8_t count, uint32_t *numOfReveive);
static status_t I3C_Send_Blocking(hostif_handle_t *handle);
...
void HIF_I3C_GetDefaultConfig(hostif_config_t *hifI3cConfig);
static void Do_Printf(uint8_t *buffer, uint32_t bufferLen, printf_flag_t flag);

/*******************************************************************************
 * Variables
 ******************************************************************************/
static volatile bool hifI3cFinished                            = false;
static hostif_handle_t g_hifI3cHandle                          = {0};
...
static void tlv_dispatcher()
{
    ...
    /* Wait until dynamic address is set. */
    while(HOSTIF->I3C_DYNADDR == 0U)
    {
    }
...
static status_t send_boot_evt()
{
    ...
    hifI3cFinished = false;
    bStatus        = HOSTIF_SendNonBlocking(&g_hifI3cHandle, transfer);

    if (kStatus_Success == bStatus)
    {
        while (!hifI3cFinished)
        {
        }
    }
...
static uint32_t ReceiveTlvCommand(void)
{
    ...
    status = I3C_Receive_Blocking(&g_hifI3cHandle, 1, &dwTotalBytesRxd);
...
status_t I3C_Send_Blocking(hostif_handle_t *handle)
{
    status_t status;
    hifI3cFinished = false;
    do
...
static status_t SendTlvResponse(uint32_t dwBufferSize)
{
    ...
    wStatus = I3C_Send_Blocking(&g_hifI3cHandle);
    return wStatus;
}

void HIF_I3C_GetDefaultConfig(hostif_config_t *hifI3cConfig)
{
    HOSTIF_GetDefaultConfig(hifI3cConfig);

    hifI3cConfig->interface                          = kHOSTIF_SelectI3c;
    hifI3cConfig->interfaceConfig.i3cConfig.slvena   = 1U;
    hifI3cConfig->interfaceConfig.i3cConfig.nack     = 0U;
    hifI3cConfig->interfaceConfig.i3cConfig.ddrok    = 1U;
    hifI3cConfig->interfaceConfig.i3cConfig.tspok    = 0U;
    hifI3cConfig->interfaceConfig.i3cConfig.tslok    = 0U;
    hifI3cConfig->interfaceConfig.i3cConfig.idrand   = 0U;
    hifI3cConfig->interfaceConfig.i3cConfig.offline  = 0U;
    hifI3cConfig->interfaceConfig.i3cConfig.saddri2c = 0U;
    hifI3cConfig->interfaceConfig.i3cConfig.saddri3c = 0U;
}
...
int main(void)
{
    ...
    /*
     * hifI3cConfig.interface        = kHOSTIF_SelectI3c;
     * hifI3cConfig.timeOut          = 0U;
     * hifI3cConfig.waterMarkLevel   = 0U;
     * hifI3cConfig.enableCrc        = false;
     * hifI3cConfig.enableExtIrqCtrl = false;
     */
    HIF_I3C_GetDefaultConfig(&hifI3cConfig);
    hifI3cConfig.enableExtIrqCtrl = true;

    hifI3cConfig.interfaceConfig.i3cConfig.saddri2c = 3U;
    hifI3cConfig.interfaceConfig.i3cConfig.slvena   = 1U;

    HOSTIF_Init(HOSTIF, &hifI3cConfig);
    AD_NFC_IRQ_Init();

    HOSTIF_CreateHandle(HOSTIF, &g_hifI3cHandle, hostif_callback, NULL);
