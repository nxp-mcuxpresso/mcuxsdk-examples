...
status_t i2c_WriteSensor(uint8_t deviceAddress, uint32_t regAddress, uint8_t *regData, size_t dataSize)
{
    ...
    result                 = LPI2C_MasterTransferNonBlocking(BOARD_TEMP_SENSOR_I2C_BASEADDR, &g_i2c_m_handle, &masterXfer);
...
int main(void)
{
    ...
    LPI2C_MasterTransferCreateHandle(BOARD_TEMP_SENSOR_I2C_BASEADDR, &g_i2c_m_handle, i2c_master_callback, NULL);
