...
int main(void)
{
    ...
    uint32_t lpspiFlashSectorSize = 0;
...
#define SECTOR_INDEX_FROM_END 1U
    ...
    destAdrss = lpspiFlashBase + (lpspiFlashTotalSize - (SECTOR_INDEX_FROM_END * lpspiFlashSectorSize));
    result    = SPI_EepromErase(destAdrss, kSize_Erase4K);
    if (kStatus_Success != result)
    {
        error_trap();
    }
    PRINTF("\r\n Successfully erased sector 0x%x -> 0x%x\r\n", destAdrss, (destAdrss + lpspiFlashSectorSize));
