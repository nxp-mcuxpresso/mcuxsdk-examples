...
void APP_DPU_Display(void)
{
    ...
    BOARD_ConfigCm0Csr();
    PRINTF("Localdimming firmware is running");

    SDK_DelayAtLeastUs(100000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY);
    DPU_Init_Localdimming_Env(APP_DPU_LD);
    DPU_ClearInterruptsPendingFlags(APP_DPU_DC, 1, kDPU_Group1LocalDimming0Irq1Interrupt);
    DPU_EnableInterrupts(APP_DPU_DC, 1, kDPU_Group1LocalDimming0Irq1Interrupt);
...
void DPU_IRQHandler(void)
{
    ...
    /* Check the kDPU_Group1LocalDimming0Irq1Interrupt in status reg for Display engine */
    if ((intGroup0Flags & APP_LD_INT_GROUP1) || (intGroup1Flags & APP_LD_INT_GROUP1))
    {
        //s_isContentStreamShadowPending = false;
        DPU_ClearInterruptsPendingFlags(APP_DPU_DC, 0, intGroup0Flags);
        DPU_ClearInterruptsPendingFlags(APP_DPU_DC, 1, intGroup1Flags);
...
int main(void)
{
    ...
    PRINTF("DPU LOCALDIMMING Example:\r\n");
