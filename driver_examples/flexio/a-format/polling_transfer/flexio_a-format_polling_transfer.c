...
int main(void)
{
    ...
    encoder_single_stat_t singleStat;
    encoder_single_temp_t singleTemp;
    encoder_abs_multi_single_t enc_abs[3], abs_save;
    uint32_t enc_id = 0;
    uint32_t clock;
    ...
    SDK_DelayAtLeastUs(30000, SDK_DEVICE_MAXIMUM_CPU_CLOCK_FREQUENCY); /* 30 ms */
    ...
    if (singleData.es == A_Format_ES_FrameErr)
    {
        PRINTF("failed\r\n");
    } else {
    ...
    if (singleStat.es == A_Format_ES_FrameErr)
    {
        PRINTF("failed\r\n");
    } else {
    ...
    if (singleTemp.es == A_Format_ES_FrameErr)
    {
        PRINTF("failed\r\n");
    } else {
    ...
    if (enc_abs[0].es != A_Format_ES_FrameErr) {
