...
static flexio_spi_slave_dma_handle_t g_spiHandle;
...
int main(void)
{
    ...
    FLEXIO_SPI_SlaveTransferCreateHandleDMA(&spiDev, &g_spiHandle, spi_slave_completionCallback, NULL, &txHandle,
                                            &rxHandle);
    FLEXIO_SPI_SlaveTransferDMA(&spiDev, &g_spiHandle, &xfer);
    while (!completeFlag)
    {
    }
    completeFlag = false;
