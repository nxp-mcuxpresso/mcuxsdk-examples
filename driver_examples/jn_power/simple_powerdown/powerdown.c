...
static bool checkIrqPending()
{
    ...
    for (int IRQ_Type = WDT_BOD_IRQn; IRQ_Type <= BLE_WAKE_UP_TIMER_IRQn; IRQ_Type++)
    {
        bool irq_pending;
...
void AppMain(void)
{
    ...
    while (1)
    {
        ...
        switch (wake_source)
        {
            ...
            case TEST_TIMER_0:
                WTIMER_Init();
                WTIMER_EnableInterrupts(0);
                WTIMER_StartTimer(0, sleep_time * 32768U);
                break;

            case TEST_TIMER_1:
                WTIMER_Init();
                WTIMER_EnableInterrupts(1);
                WTIMER_StartTimer(1, sleep_time * 32768U);
                break;
        }
