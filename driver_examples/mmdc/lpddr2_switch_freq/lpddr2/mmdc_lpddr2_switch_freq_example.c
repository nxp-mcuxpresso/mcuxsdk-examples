...
extern uint32_t mmdc_lpddr2_switch_freq_start;
extern uint32_t mmdc_lpddr2_switch_freq_end;
...
int main(void)
{
    ...
    bool rwStatus     = true;
    uint32_t *srcAddr = (uint32_t *)((uint32_t)&mmdc_lpddr2_switch_freq_start & (~1U)); /* remove thumb mode flag */
    uint32_t *codeDestAddr = (uint32_t *)(EXAMPLE_DDR_CODE_RAM_ADDRESS);
    ...
    if (rwStatus)
    {
        ...
        memcpy(codeDestAddr, srcAddr, ((uint32_t)(&mmdc_lpddr2_switch_freq_end) - (uint32_t)srcAddr + 1U));
        /* jump to switch frequency api */
        MMDC_SwitchDeviceFrequency(EXAMPLE_MMDC, EXAMPLE_CCM, NULL, EXAMPLE_DDR_24M,
                                   EXAMPLE_DDR_CODE_RAM_ADDRESS | 1U); /* should add cortex-m THUMB mode flag */
    ...
    {
        /* switch to 300MHZ */
        PRINTF("\r\n Start switch DDR work frequency to 300000000HZ. \r\n");

        /* jump to switch frequency api */
        MMDC_SwitchDeviceFrequency(EXAMPLE_MMDC, EXAMPLE_CCM, NULL, EXAMPLE_DDR_300M,
                                   EXAMPLE_DDR_CODE_RAM_ADDRESS | 1U); /* should add cortex-m THUMB mode flag */
...
static uint32_t Get_MMDC_CLK(void)
{
    if (CLOCK_GetMux(kCLOCK_MmdcMux) == 0)
    {
        return CLOCK_GetFreq(kCLOCK_Usb1PllPfd0Clk) / (CLOCK_GetDiv(kCLOCK_MmdcDiv) + 1U);
    }
    else if (CLOCK_GetMux(kCLOCK_MmdcMux) == 1)
    {
        return CLOCK_GetFreq(kCLOCK_OscClk) / (CLOCK_GetDiv(kCLOCK_MmdcDiv) + 1U);
    }
    else if (CLOCK_GetMux(kCLOCK_MmdcMux) == 2)
    {
        return CLOCK_GetFreq(kCLOCK_SysPllPfd2Clk) / (CLOCK_GetDiv(kCLOCK_MmdcDiv) + 1U);
    }
...
static bool Device_ReadWrite(void)
{
    ...
    uint32_t ddrSize       = EXAMPLE_DDR_RW_SIZE / 4;                           /* Calculate DDR size in 4 bytes. */
    uint32_t *ddrStartAddr = (uint32_t *)EXAMPLE_DDR_DATA_BUFFER_START_ADDRESS; /* DDR start address. */
    bool result            = true;

    PRINTF("\r\n DDR work freq %d HZ.\r\n", Get_MMDC_CLK());

    memset(ddrStartAddr, 0U, EXAMPLE_DDR_RW_SIZE);
