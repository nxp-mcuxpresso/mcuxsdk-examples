...
void SPI_SLAVE_IRQHandler(void)
{
    ...
    if ((SPI_GetStatusFlags(EXAMPLE_SPI_SLAVE) & kSPI_RxFifoNotEmptyFlag))
    {
        rxBuffer[BUFFER_SIZE - rxIndex] = SPI_ReadData(EXAMPLE_SPI_SLAVE);
        rxIndex--;
    }
    /* Write data to TX regsiter if TX register is ready. */
    if ((SPI_GetInterruptStatusFlags(EXAMPLE_SPI_SLAVE) & kSPI_TxFifoHalfEmptyInterruptFlag) && (txIndex != 0U))
    {
    ...
    {
        slaveFinished = true;
        SPI_DisableInterrupts(EXAMPLE_SPI_SLAVE, kSPI_TxFifoHalfEmptyInterruptEnable);
    }
}
...
static void EXAMPLE_SlaveStartTransfer(void)
{
    ...
    EnableIRQ(EXAMPLE_SPI_SLAVE_IRQn);
    SPI_EnableInterrupts(EXAMPLE_SPI_SLAVE, kSPI_TxFifoHalfEmptyInterruptEnable);
}
