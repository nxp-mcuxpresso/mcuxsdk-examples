...
volatile uint8_t s_te_done  = 0;
volatile uint8_t s_mou_done = 0;
volatile uint8_t s_se_done  = 0;
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
void DEMO_FSP_IRQ_HANDLER_FUNC(void);
/*******************************************************************************
 * Variables
 ******************************************************************************/
...
void DEMO_FSP_IRQ_HANDLER_FUNC(void)
{
    uint32_t status = FSP_GetStatusFlags(DEMO_FSP_BASE);
    if (FSP_INT_TE_DONE_INT_MASK == (FSP_INT_TE_DONE_INT_MASK & status))
    {
        s_te_done = 1;
        FSP_ClearStatusFlags(DEMO_FSP_BASE, FSP_INT_TE_DONE_INT_MASK);
    }
    if (FSP_INT_MOU_DONE_INT_MASK == (FSP_INT_MOU_DONE_INT_MASK & status))
    {
        s_mou_done = 1;
        FSP_ClearStatusFlags(DEMO_FSP_BASE, FSP_INT_MOU_DONE_INT_MASK);
    }
    if (FSP_INT_SE_DONE_INT_MASK == (FSP_INT_SE_DONE_INT_MASK & status))
    {
        s_se_done = 1;
        FSP_ClearStatusFlags(DEMO_FSP_BASE, FSP_INT_SE_DONE_INT_MASK);
    }
...
void fft_example(void)
{
    ...
    s_se_done = 0;
    FSP_MaxMinIntF32(DEMO_FSP_BASE, fft_fsp_input, npts);
    while (!s_se_done)
    {
    }
    FSP_GetMaxMinIntResultF32(DEMO_FSP_BASE, fft_fsp_input, &max_value, &min_value);
...
int main(void)
{
    ...
    FSP_EnableInterrupts(DEMO_FSP_BASE, FSP_INTEN_FSP_INTEN_MASK | FSP_INTEN_MOU_DONE_INTEN_MASK |
                                            FSP_INTEN_SE_DONE_INTEN_MASK | FSP_INTEN_TE_DONE_INTEN_MASK);
    NVIC_EnableIRQ(DEMO_FSP_IRQ_ID);
