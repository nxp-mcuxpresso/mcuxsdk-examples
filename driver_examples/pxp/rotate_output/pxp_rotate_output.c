...
int main(void)
{
    ...
    PRINTF("\r\nPXP Rotate Output example start...\r\n");
...
static void APP_InitPxp(void)
{
    ...
    PXP_SetPath(APP_PXP, kPXP_Mux12SelectMux11);
    PXP_SetPath(APP_PXP, kPXP_Mux14SelectRotation0Engine);
#endif
}
...
static void APP_Rotate(void)
{
    ...
    for (;;)
    {
        for (i = 0; i < ARRAY_SIZE(degrees); i++)
        {
            ...
            pxp_rotate_degree_t currentDegree = degrees[i];
            /* Prepare next buffer for LCD. */
            PXP_SetRotateConfig(APP_PXP, kPXP_RotateOutputBuffer, currentDegree, kPXP_FlipDisable);

            /* Based on rotation angle to swap the value of OUT_LRC, OUT_PS_LRC etc */

            if ((currentDegree == kPXP_Rotate90) || (currentDegree == kPXP_Rotate270))
            {
                outputBufferConfig.width          = APP_IMG_HEIGHT;
                outputBufferConfig.height         = APP_IMG_WIDTH;
                PXP_SetProcessSurfacePosition(APP_PXP, APP_PS_ULC_Y, APP_PS_ULC_X, APP_PS_LRC_Y, APP_PS_LRC_X);
            }
            else
            {
                outputBufferConfig.width          = APP_IMG_WIDTH;
