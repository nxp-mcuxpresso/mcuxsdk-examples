...
#define SECTOR_INDEX_FROM_END 256
#endif
...
/* Erase a sector from destAdrss. */
    ...
    result = FLASH_GetSectorProtection(&s_flashDriver, destAdrss);
    if (kStatus_FLASH_SectorLocked == result)
    {
        result = FLASH_SetSectorProtection(&s_flashDriver, destAdrss, false);
        if (kStatus_FLASH_Success != result)
        {
            PRINTF("\r\n Failed to unlock sector protection!");
        }
    }
