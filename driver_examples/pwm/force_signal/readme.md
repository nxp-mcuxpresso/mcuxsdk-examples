...
|-----------|----------|-------------|
| `DTSRCSEL[SMxSEL23]` | PWM Source Select | Selects the signal fed into the deadtime generator (normal, inverted, software, or external) |
...
```
Phase 0  Normal            DTSRCSEL=UsePwm   IPOL=PwmA  30 % duty on PWM_A0
Phase 1  DTSRCSEL update   DTSRCSEL=Inverted IPOL=PwmA  70% duty (inverted source)
Phase 2  Atomic update     DTSRCSEL=UsePwm   IPOL=PwmB  30% duty (unchanged), DTSRCSEL+IPOL via 1 FORCE
Phase 3  Restore           DTSRCSEL=UsePwm   IPOL=PwmA  Back to Phase 0
```

### Waveform Diagram

The GPIO is toggled immediately before each `PWM_EnableLocalForce()` call; the GPIO
...
(1kHz)    | | | | | |   :  |       | |       |   :  | | | | | | | |  :  | | | | | |
          ...
          DTSRCSEL=UsePwm DTSRCSEL=Inverted        DTSRCSEL=UsePwm      DTSRCSEL=UsePwm
          IPOL=PwmA       IPOL=PwmA (unchanged)    IPOL=PwmB            IPOL=PwmA
