...
void QSPI_SlaveUserCallback(qspi_slave_edma_handle_t *handle, status_t status, void *pUserData);
...
AT_NONCACHEABLE_SECTION_ALIGN(qspi_slave_edma_handle_t g_sQspiHandle, sizeof(edma_channel_tcd_t));
...
void QSPI_SlaveUserCallback(qspi_slave_edma_handle_t *psHandle, status_t eStatus, void *pUserData)
{
    if (eStatus == kStatus_Success)
    {
        PRINTF("This is QUEUEDSPI slave edma transfer completed callback. \r\n\r\n");
    }
...
int main(void)
{
    ...
    QSPI_SlaveTransferCreateHandleEDMA(EXAMPLE_QSPI_SLAVE_BASEADDR, &g_sQspiHandle, QSPI_SlaveUserCallback, NULL,
    ...
    while (1)
    {
        ...
        QSPI_SlaveTransferEDMA(&g_sQspiHandle, &sSlaveXfer);
