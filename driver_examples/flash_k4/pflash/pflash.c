...
#define SECTOR_INDEX_FROM_END 1U
    ...
    result = FLASH_VerifyEraseSector(&s_flashDriver, FLASH, destAdrss, pflashSectorSize);
    if (kStatus_FLASH_Success != result)
    {
        error_trap();
    }
    ...
    result = FLASH_Program(&s_flashDriver, FLASH, destAdrss, (uint8_t *)s_buffer, sizeof(s_buffer));
    if (kStatus_FLASH_Success != result)
    {
        error_trap();
    }
    ...
    /* Restore the sector to original state */
    destAdrss = pflashBlock0Base + (pflashBlock0Size - (SECTOR_INDEX_FROM_END * pflashSectorSize));
    result    = FLASH_Erase(&s_flashDriver, FLASH, destAdrss, pflashSectorSize, kFLASH_ApiEraseKey);
    if (kStatus_FLASH_Success != result)
    {
