...
status_t flexspi_nor_enable_octal_for_adesto(FLEXSPI_Type *base)
{
    ...
    status = flexspi_nor_exec_op(base, 0, FLASH_PORT, kFLEXSPI_Read, NOR_CMD_LUT_SEQ_IDX_READSTATUS_STR_OPI, 1,
                                 &readValue, 1);
    if (status != kStatus_Success)
    {
        return status;
    ...
    if ((readValue & SR_SWP_ALL_SECTORS_PROTECTED) == SR_SWP_ALL_SECTORS_PROTECTED)
    {
        ...
        status = flexspi_nor_exec_op(base, 0, FLASH_PORT, kFLEXSPI_Read, NOR_CMD_LUT_SEQ_IDX_READSTATUS_STR_OPI,
                                     1, &readValue, 1);
        if (status != kStatus_Success)
        {
            return status;
    ...
    tempLUT[0] = FLEXSPI_LUT_SEQ(kFLEXSPI_Command_SDR, kFLEXSPI_8PAD, SPINOR_OP_WRSR2_0X31,
...
status_t flexspi_nor_flash_page_program(FLEXSPI_Type *base, uint32_t dstAddr, const uint32_t *src)
{
    ...
    status = flexspi_nor_exec_op(base, dstAddr, FLASH_PORT, kFLEXSPI_Write, NOR_CMD_LUT_SEQ_IDX_PAGEPROGRAM_DTR_OPI, 1,
