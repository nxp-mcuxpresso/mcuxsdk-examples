...
static flexspi_handle_t flexspiHandle;
/*******************************************************************************
 * Code
 ******************************************************************************/

static void flexspi_callback(FLEXSPI_Type *base, flexspi_handle_t *handle, status_t status, void *userData)
{
...
status_t flexspi_nand_write_enable(FLEXSPI_Type *base, uint32_t baseAddr)
{
    ...
    status = FLEXSPI_TransferNonBlocking(base, &flexspiHandle, &flashXfer);
...
status_t flexspi_nand_get_vendor_id(FLEXSPI_Type *base, uint8_t *vendorId)
{
    ...
    status_t status = FLEXSPI_TransferNonBlocking(base, &flexspiHandle, &flashXfer);
...
int main(void)
{
    ...
    FLEXSPI_TransferCreateHandle(EXAMPLE_FLEXSPI, &flexspiHandle, flexspi_callback, NULL);
