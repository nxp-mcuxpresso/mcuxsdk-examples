...
AT_NONCACHEABLE_SECTION(static uint8_t s_nand_program_buffer[2048]);
AT_NONCACHEABLE_SECTION(static uint8_t s_nand_read_buffer[2048]);
static volatile bool g_completionFlag = false;
static flexspi_edma_handle_t flexspiHandle;
static edma_handle_t edmaTxHandle;
static edma_handle_t edmaRxHandle;
/*******************************************************************************
 * Code
 ******************************************************************************/
...
status_t flexspi_nand_get_vendor_id(FLEXSPI_Type *base, uint8_t *vendorId)
{
    ...
    status_t status = FLEXSPI_TransferEDMA(base, &flexspiHandle, &flashXfer);
...
int main(void)
{
    ...
    /* Set channel for FLEXSPI */
    DMAMUX_SetSource(EXAMPLE_FLEXSPI_DMAMUX_BASEADDR, FLEXSPI_TX_DMA_CHANNEL, FLEXSPI_TX_DMA_REQUEST);
    DMAMUX_SetSource(EXAMPLE_FLEXSPI_DMAMUX_BASEADDR, FLEXSPI_RX_DMA_CHANNEL, FLEXSPI_RX_DMA_REQUEST);
    DMAMUX_EnableChannel(EXAMPLE_FLEXSPI_DMAMUX_BASEADDR, FLEXSPI_TX_DMA_CHANNEL);
    DMAMUX_EnableChannel(EXAMPLE_FLEXSPI_DMAMUX_BASEADDR, FLEXSPI_RX_DMA_CHANNEL);

    /* Init the EDMA module */
    edma_config_t edmaConfig;
    EDMA_GetDefaultConfig(&edmaConfig);
    EDMA_Init(EXAMPLE_FLEXSPI_DMA_BASEADDR, (const edma_config_t *)&config);
    EDMA_CreateHandle(&edmaTxHandle, EXAMPLE_FLEXSPI_DMA_BASEADDR, FLEXSPI_TX_DMA_CHANNEL);
    EDMA_CreateHandle(&edmaRxHandle, EXAMPLE_FLEXSPI_DMA_BASEADDR, FLEXSPI_RX_DMA_CHANNEL);

    /* Create handle for flexspi. */
    FLEXSPI_TransferCreateHandleEDMA(EXAMPLE_FLEXSPI, &flexspiHandle, flexspi_callback, NULL, &edmaTxHandle,
                                     &edmaRxHandle);
