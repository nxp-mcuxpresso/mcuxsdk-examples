...
int main(void)
{
    ...
    TMP117_GetDefaultConfig(&tmp117Config);
    tmp117Config.deviceAddress = SENSOR_SLAVE_ADDR;
    tmp117Config.i2cWriteFunc = i2c_WriteSensor;
    tmp117Config.i2cReadFunc  = i2c_ReadSensor;

    TMP117_Init(&tmp117Config);

    while (1)
    {
        status_t status = TMP117_StartMeasurement(&tmp117Config, kTMP117_Averaging_32);

        if (kStatus_Success == status)
        {
            result = TMP117_GetTemperature(&tmp117Config, &temperature);
            if (result != kStatus_Success)
            {
                PRINTF("\r\ntmp117 read temperature failed.\r\n");
            }
            else
            {
                PRINTF("\r\nTemperature:%.4f\r\n", (double)(temperature * 0.0078125));
            }
        }
        else
        {
            PRINTF("\r\ntmp117 start measurement failed.\r\n");
        }
