...
extern uint32_t data_inject_ctrl;
#endif
...
lpm_power_mode_t LPM_GetPowerMode(void)
{
    ...
    if ((s_curMode == LPM_PowerModeWait) && (prev_scmi_status == 0))
    {
        ...
        {
            system_sleep_state = SCMI_SYS_STATE_MODE | 0 << 16 | SSM_OSC24M_ACTIVE_MASK;
#if SRTM_PDM_EDMA_DATA_INJECTION
            /* Prevent system sleep for test mode, too many DDR accesses to fetch test pattern. */
            if (data_inject_ctrl != 0) {
                system_sleep_state |= SSF_SM_ACTIVE_MASK;
            }
#endif
             ...
             */
            status = SCMI_SystemPowerStateSet(SCMI_A2P, 0, system_sleep_state);
