...
uint8_t nt_memory_pool[4300]; /* IAR EWARM compiler */
#else
uint8_t nt_memory_pool[4300] __attribute__((aligned(4))); /* Keil, GCC compiler */
#endif
        ...
        case NT_KEYPAD_MULTI_TOUCH:
            switch (index)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    /* RED + GREEN + BLUE on, full brightness */
                    LED_RED_ON();
                    LED_GREEN_ON();
                    LED_BLUE_ON();
                    break;
