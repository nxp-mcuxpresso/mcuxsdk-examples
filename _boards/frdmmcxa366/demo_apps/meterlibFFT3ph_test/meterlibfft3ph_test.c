...
int main(void)
{
    ...
    /* GPIO1 pin 07, output mode */
    GPIO1->PDDR                     = GPIO_PDDR_PDD7(1);
#endif
...
#if 0
        /* GPIO1 pin 07, Set output value, go to "H", code performance measurement start pulse */
        GPIO1->PSOR                     = GPIO_PSOR_PTSO7(1);
#endif
        /* main calculation (FFT, scaling, averaging) */
        METERLIBFFT3PH_CalcMain(&ui); 
#if 0
        /* GPIO1 pin 07, Clear output value, go to "L", code performance measurement stop pulse */
        GPIO1->PCOR                     = GPIO_PCOR_PTCO7(1);
#endif
