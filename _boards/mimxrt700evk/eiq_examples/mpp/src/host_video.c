...
static uint32_t *s_pictureBuffer_unaligned[2];
#endif
...
static void USB_HostVideoWriteSDCard(void *param)
{
    ...
    if (videoAppInstance->pictureBufferState[videoAppInstance->sdcardWriteBufferIndex])
    {
        ...
        if (videoAppInstance->cameraDeviceFormatType == USB_HOST_DESC_SUBTYPE_VS_FORMAT_MJPEG)
        {
        	image_type = IMG_FORMAT_JPG;
        	frameDesc = (usb_host_video_stream_payload_frame_common_desc_t *)videoAppInstance->videoStreamMjpegFrameDescriptor;
        }
        else
        {
        	//TODO: not implemented yet
        	image_type = IMG_FORMAT_YUV422;
        	frameDesc = (usb_host_video_stream_payload_frame_common_desc_t *)videoAppInstance->videoStreamUncompressedFrameDescriptor;
        }
		...
		if (NULL != frameDesc)
		{
			width = (uint32_t)(USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(frameDesc->wWitd));
			height = (uint32_t)(USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS(frameDesc->wHeight));

			Process_Image_Frame((uint8_t *)&s_pictureBuffer[videoAppInstance->sdcardWriteBufferIndex][0],
	        		            videoAppInstance->sdcardPictureLength[videoAppInstance->sdcardWriteBufferIndex],
...
void USB_HostVideoTask(void *param)
{
...
#ifdef USB_IMG_BUFFER_DYNALLOC
                vPortFree((void *)s_pictureBuffer_unaligned[0]);
                vPortFree((void *)s_pictureBuffer_unaligned[1]);
#endif
    ...
    switch (videoAppInstance->runState)
    {
...
#else
                /* Try to get UNCOMPRESSED format */
                status = USB_HostVideoStreamGetFormatDescriptor(videoAppInstance->classHandle,
                ...
                for (index = 1; index <= count; index++)
                {
                    ...
                    if ((kStatus_USB_Success == status) && (NULL != frameDesc))
                    {
                        ...
                        if (videoAppInstance->cameraDeviceFormatType == USB_HOST_DESC_SUBTYPE_VS_FORMAT_MJPEG)
                        {
                            ...
                            usb_host_video_stream_payload_mjpeg_frame_desc_t * crtDesc;
                            crtDesc = (usb_host_video_stream_payload_mjpeg_frame_desc_t *)frameDesc;
                            usb_echo("\t FPS ");
                        	if (crtDesc->bFrameIntervalType > 0)
							{
                        		int i;
                        		usb_echo("FIXED (%d Values):", crtDesc->bFrameIntervalType);
                        		for (i = 0; i< crtDesc->bFrameIntervalType; i++)
                        		{
                        			frameInterval = USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 4*i));
                        			usb_echo(" %u(%u)", frameInterval, 10000000/frameInterval);
                        		}
                        		usb_echo("\r\n");
                        		frameInterval = USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(
									((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + (crtDesc->bFrameIntervalType - 1) * 4) );
							}
							else
							{
								usb_echo("Variable Min:%u(%u) Max:%u(%u) Step:%u\r\n",
										USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 0)),
										10000000/USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 0)),
										USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 4)),
										10000000/USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 4)),
										USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 8))
								);
								frameInterval = USB_LONG_FROM_LITTLE_ENDIAN_ADDRESS(
									((uint8_t *)&(crtDesc->dwMinFrameInterval[0]) + 4) );
							}

#if 0
                        	usb_echo("\t\t MaxBuffer:%u, MinInterval:%u frameint:%u\r\n",
                             		(uint32_t)(USB_SHORT_FROM_LITTLE_ENDIAN_ADDRESS (
                             				((usb_host_video_stream_payload_mjpeg_frame_desc_t *)frameDesc)->dwMaxVideoFrameBufferSize)),
                                     		(uint32_t)( ((usb_host_video_stream_payload_mjpeg_frame_desc_t *)frameDesc)->dwMinFrameInterval[0]),
                                     		frameInterval
                             );
#endif
                        }
...
#if USB_FRAME_INTERVAL_OVERRIDE
                            frameInterval = USB_FRAME_INTERVAL_VALUE;
#endif
        ...
        case kUSB_HostVideoRunGetSetProbeCommitDone:
        {
            ...
            {
                usb_echo(
                    "picture buffer malloc failed, please make sure the heap size or prealocation is enough. If it is raw data now, "
            ...
            usb_echo("Start getting pictures from device camera.\r\n");
            usb_echo("Please disconnect camera before powering off the board.\r\n");
