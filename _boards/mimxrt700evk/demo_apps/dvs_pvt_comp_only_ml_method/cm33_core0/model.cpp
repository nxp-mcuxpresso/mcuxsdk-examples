...
status_t MODEL_Init(void)
{
    ...
    static tflite::MicroInterpreter static_interpreter(s_model, micro_op_resolver, s_tensorArena, kTensorArenaSize);
    ...
    PRINTF("TensorArena Size: Total 0x%x (%d B); Used 0x%x (%d B)\r\n", kTensorArenaSize, kTensorArenaSize,
           s_tensorArenaSizeUsed, s_tensorArenaSizeUsed);
...
void MODEL_ConvertInput(uint8_t *data, tensor_dims_t *dims, tensor_type_t type)
{
    ...
    switch (type)
    {
        ...
        case kTensorType_INT8:
            ...
            {
                reinterpret_cast<int8_t *>(data)[i] = static_cast<int>(data[i]) - 127;
            }
            break;
