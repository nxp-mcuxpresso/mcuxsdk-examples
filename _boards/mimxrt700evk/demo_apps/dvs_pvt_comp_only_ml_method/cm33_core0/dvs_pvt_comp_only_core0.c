...
static void config_pvts(void)
{
    ...
    PVTS_SetDelay(kPVTS_Sensor0, delays[0]);
    ...
    /* Set the delay and start sensing */
    PVTS_SetDelay(kPVTS_Sensor1, delays[1]);
...
int main(void)
{
     ...
     */
    ret = PVTS_ReadParametersFromOTP(false, kPVTS_Vdd2Com, DEMO_MAINCLK_FREQ, &pvts_parameters);
    if ((ret == kStatus_Fail) || (pvts_parameters == 0U))
    {
        DEMO_LOG("\r\nFailed to read PVTS parameters, please check the sample!\r\n\n");
        delays[0] = DEMO_TYPICAL_DELAY_CPU0;
        delays[1] = DEMO_TYPICAL_DELAY_HIFI4;
    }
    else
    {
        ret = PVTS0DeduceDelayUsingML(false, kPVTS_CPU0, (DEMO_MAINCLK_FREQ / 1000000U), pvts_parameters, &delays[0]);
        if (ret == kStatus_Fail)
        {
            DEMO_LOG("\r\nFailed to deduce PVTS delay using the ML model, please check the sample!\r\n\n");
            delays[0] = DEMO_TYPICAL_DELAY_CPU0;
        }

#if defined(DEMO_PVT_ON_CPU_DSP) && (DEMO_PVT_ON_CPU_DSP != 0U)
        ret = PVTS0DeduceDelayUsingML(true, kPVTS_HIFI4, (DEMO_MAINCLK_FREQ / 1000000U), pvts_parameters, &delays[1]);
        if (ret == kStatus_Fail)
        {
            DEMO_LOG("\r\nFailed to deduce PVTS delay using the ML model, please check the sample!\r\n\n");
            delays[1] = DEMO_TYPICAL_DELAY_HIFI4;
    ...
    DEMO_LOG("PVTS delay CPU0 = 0x%x\r\n", delays[0]);
    DEMO_LOG("PVTS delay HIFI4 = 0x%x\r\n", delays[1]);
