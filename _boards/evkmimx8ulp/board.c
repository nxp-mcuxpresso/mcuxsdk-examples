...
void BOARD_SetTrdcGlobalConfig(void)
{
    ...
    if (releasedTrdc == true)
    {
        ...
        TRDC_SetNonProcessorDomainAssignment(TRDC, TRDC_DMA0_MDAC_INST_INDEX, &nonProcessorDomainAssignment);
        nonProcessorDomainAssignment.domainId = TRDC_POWERQUAD_DOMAIN_ID;
        TRDC_SetNonProcessorDomainAssignment(TRDC, TRDC_POWERQUAD_MDAC_INST_INDEX, &nonProcessorDomainAssignment);
        /* Assigned domain id for Fusion DSP(secure privilege can access all of resources) */
        nonProcessorDomainAssignment.domainId = TRDC_FUSION_DSP_DOMAIN_ID;
        nonProcessorDomainAssignment.secureAttr    = kTRDC_ForceSecure;
        nonProcessorDomainAssignment.privilegeAttr = kTRDC_ForcePrivilege;
        TRDC_SetNonProcessorDomainAssignment(TRDC, TRDC_FUSION_DSP_MDAC_INST_INDEX, &nonProcessorDomainAssignment);
 ...
 */
void BOARD_SelectI2cPadsFunc(int32_t i2cInstIdx, bool use_as_gpio)
{
    ...
    switch (i2cInstIdx)
    {
        case 0:
            if (use_as_gpio)
            {
                IOMUXC_SetPinMux(I2C0_RELEASE_SCL_PAD_USE_AS_GPIO_ARGS, 0U);
                IOMUXC_SetPinConfig(I2C0_RELEASE_SCL_PAD_USE_AS_GPIO_ARGS,
                                    IOMUXC_PCR_OBE_MASK);
                IOMUXC_SetPinMux(I2C0_RELEASE_SDA_PAD_USE_AS_GPIO_ARGS, 0U);
                IOMUXC_SetPinConfig(I2C0_RELEASE_SDA_PAD_USE_AS_GPIO_ARGS,
                                    IOMUXC_PCR_OBE_MASK);
            }
            else
            {
                IOMUXC_SetPinMux(I2C0_RELEASE_SCL_PAD_USE_AS_SCL_ARGS, 0U);
                IOMUXC_SetPinConfig(I2C0_RELEASE_SCL_PAD_USE_AS_SCL_ARGS,
                                    IOMUXC_PCR_ODE_MASK);
                IOMUXC_SetPinMux(I2C0_RELEASE_SDA_PAD_USE_AS_SDA_ARGS, 0U);
                IOMUXC_SetPinConfig(I2C0_RELEASE_SDA_PAD_USE_AS_SDA_ARGS,
                                    IOMUXC_PCR_ODE_MASK);
            }
            break;
        case 1:
 ...
 */
void BOARD_I2C_ReleaseBus(int32_t i2cInstIdx)
{
    uint8_t i = 0;
    ...
    BOARD_SelectI2cPadsFunc(i2cInstIdx, true);

    CLOCK_EnableClock(i2c_info->clk_name);
