...
void APP_DeinitWakeupButton(void)
{
    ...
    PORT_SetPinConfig(PORTC, 7U, &wakeupButtonDefaultConfig);
}
...
{
    if ((WUU_GetExternalWakeUpPinsFlag(APP_WUU) & 0x1000UL) != 0UL)
    {
        WUU_ClearExternalWakeUpPinsFlag(APP_WUU, 0x1000UL);
    }
}
