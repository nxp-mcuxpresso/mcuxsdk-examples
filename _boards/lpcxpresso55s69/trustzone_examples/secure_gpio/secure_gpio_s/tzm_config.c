...
- ahb:
  ...
  - peripherals: {ns_user: {id: [ADC0, AHB_SECURE_CTRL, ANACTRL, CASPER, CRC_ENGINE, CTIMER0, CTIMER1, CTIMER2, CTIMER3, CTIMER4, DBGMAILBOX, DMA0, DMA1, FLASH,
        ...
        POWERQUAD, PRINCE, PUF, RNG, RTC, SCT0, SDIF, SECPINT, SPI8, SYSCTL, USB0, USBFSH, USBHSD, USBHSH, USBPHY, UTICK0, WWDT]}, s_priv: {id: [FLEXCOMM0, IOCON,
        SECGPIO, SYSCON]}}
  - interrupts:
...
- user_memory_regions: [{id: Region_1, security: s_priv, name: Secure Code, start: '0x10000000', size: '0x00010000'}, {id: Region_2, security: s_priv, name: Secure
      ...
      so reset value (NS-User) can be used, start: '0x00010000', size: '0x00068000'}, {id: Region_4, security: ns_user, name: Non-secure Stack and Data, description: 'Privilege
      check is disabled so reset value (NS-User) can be used. ', start: '0x20008000', size: '0x0002B000'}]
- mpus:
  - mpu:
    - enabled: 'false'
    - id: 's'
...
void BOARD_InitTrustZone()
{
    ...
    */
    ...
    AHB_SECURE_CTRL->SEC_CTRL_AHB_PORT10[0].SLAVE1_RULE                    = 0xFFFFFFCFU;
