...
uint32_t g_ui32NumberOfCycles    = 0U, g_ui32M2NumberOfCycles = 0U;
uint32_t g_ui32MaxNumberOfCycles = 0U, g_ui32M2MaxNumberOfCycles = 0U;

/* Demo mode enabled/disabled */
bool_t bDemoModeSpeed = FALSE, bM2DemoModeSpeed = FALSE;
bool_t bDemoModePosition = FALSE, bM2DemoModePosition = FALSE;

/* Used for demo mode */
static uint32_t ui32SpeedStimulatorCnt    = 0U, ui32M2SpeedStimulatorCnt = 0U;
static uint32_t ui32PositionStimulatorCnt = 0U, ui32M2PositionStimulatorCnt = 0U;

/* Structure used in FM to get required ID's */
app_ver_t g_sAppIdFM = {
    "",       /* User Path 1- the highest priority. MCAT not supported in dual motor application, path can be empty */
    "",       /* User Path 2 */
    "lpcxpresso55s36",  /* board id */
    "pmsm_enc_dual",         /* example id */
...
int main(void)
{
    ...
    /* Disable M1 and M2 demo modes after reset */
    bDemoModeSpeed    = FALSE;
    bM2DemoModeSpeed    = FALSE;
    bDemoModePosition = FALSE;
    bM2DemoModePosition = FALSE;
    ui32SpeedStimulatorCnt = 0U;
    ui32M2SpeedStimulatorCnt = 0U;

    /* Init board hardware. */
    BOARD_Init();

    /* Initialize peripheral motor control driver for motor M1, M2 */
    MCDRV_Init();
 ...
 *           - motors M1 and M2 slow application machine function
 *
 * @param   void
 *
 * @return  none
...
/*!
 * @brief   ADC ISR called with 100us period processes
 *           - motor M2 fast application machine function
 *           - demo mode if enabled
 *
 * @param   void
 *
...
static void DemoSpeedStimulator(void)
{
    ...
    if (bM2DemoModeSpeed)
    {
        ui32M2SpeedStimulatorCnt++;
        switch (ui32M2SpeedStimulatorCnt)
        {
            case 10:
...
static void DemoPositionStimulator(void)
{
    ...
    /* Motor 2 position demo */
    if (bM2DemoModePosition)
    {
        ui32M2PositionStimulatorCnt++;
        switch (ui32M2PositionStimulatorCnt)
        {
            case 2:
                ...
                break;
            case 10:
                M2_SetPosition(ACC32(20));
                break;
            case 3000:
                M2_SetPosition(ACC32(10));
                break;
            case 6000:
                M2_SetPosition(ACC32(20));
                break;
