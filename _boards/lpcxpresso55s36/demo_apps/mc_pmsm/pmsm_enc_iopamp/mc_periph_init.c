...
static void InitAOI_IOpAmp(void);
static void InitHsCmp0_IOpAmp(void);
static void InitHsCmp1_IOpAmp(void);
static void InitHsCmp2_IOpAmp(void);
...
static void InitADC0(void)
{
    ...
    ADC0->CTRL |= ADC_CTRL_RSTFIFO1_MASK;
...
/*!
@brief   void InitHsCmp0_IOpAmp(void)
...
static void InitHsCmp0_IOpAmp(void)
{
    ...
    /* Input minus = internal 8-bit DAC, input plus = analog mux in4 (OPAMP0_OUT), high power/high speed mode */
    HSCMP0->CCR2 |= (uint32_t)( HSCMP_CCR2_INMSEL(0) | HSCMP_CCR2_INPSEL(1) | HSCMP_CCR2_MSEL(7U) | HSCMP_CCR2_PSEL(4U) | HSCMP_CCR2_CMP_HPMD_MASK);

    /* Setting for internal OPAMP: Set DAC_OUT to 1.25V (2.5A overcurrent through 0.02 Ohm shunt, gain 8x). Overcurrent is negative through the shunt. */
    HSCMP0->DCR = HSCMP_DCR_DAC_DATA(94U) |   /* (3.3V / 256) * (94 + 1) = 1.224 V */
...
static void InitHsCmp1_IOpAmp(void)
{
    ...
    /* Input minus = internal 8-bit DAC, input plus = analog mux in4 (OPAMP0_OUT), high power/high speed mode */
    HSCMP1->CCR2 |= (uint32_t)( HSCMP_CCR2_INMSEL(0) | HSCMP_CCR2_INPSEL(1) | HSCMP_CCR2_MSEL(7U) | HSCMP_CCR2_PSEL(4U) | HSCMP_CCR2_CMP_HPMD_MASK);

    /* Setting for internal OPAMP: Set DAC_OUT to 1.25V (2.5A overcurrent through 0.02 Ohm shunt, gain 8x). Overcurrent is negative through the shunt. */
    HSCMP1->DCR = HSCMP_DCR_DAC_DATA(94U) |   /* (3.3V / 256) * (94 + 1) = 1.224 V */
                  HSCMP_DCR_DACOE(1)      |   /* DAC output enable */
                  HSCMP_DCR_DAC_HPMD(1)   |   /* High power mode enabled */
                  HSCMP_DCR_DAC_EN(1);        /* DAC enable */
...
/*!
@brief   void InitHsCmp2_IOpAmp(void)
          - Initialization of the comparator 2 module for dc-bus over current
            detection to generate fault
...
static void InitHsCmp2_IOpAmp(void)
{
    ...
    /* Input minus = internal 8-bit DAC, input plus = analog mux in4 (OPAMP0_OUT), high power/high speed mode */
    HSCMP2->CCR2 |= (uint32_t)( HSCMP_CCR2_INMSEL(0) | HSCMP_CCR2_INPSEL(1) | HSCMP_CCR2_MSEL(7U) | HSCMP_CCR2_PSEL(4U) | HSCMP_CCR2_CMP_HPMD_MASK);

    /* Setting for internal OPAMP: Set DAC_OUT to 1.25V (2.5A overcurrent through 0.02 Ohm shunt, gain 8x). Overcurrent is negative through the shunt. */
    HSCMP2->DCR = HSCMP_DCR_DAC_DATA(94U) |   /* (3.3V / 256) * (94 + 1) = 1.224 V */
                  HSCMP_DCR_DACOE(1)      |   /* DAC output enable */
                  HSCMP_DCR_DAC_HPMD(1)   |   /* High power mode enabled */
                  HSCMP_DCR_DAC_EN(1);        /* DAC enable */

    /* HSCMP enable */
    HSCMP2->CCR0 |= HSCMP_CCR0_CMP_EN_MASK;
}
...
static void InitOpAmps_IOpAmp(void)
{
    ...
    /* Disable OPAMP0, OPAMP1 and OPAMP3 power down. */
 ...
 /*!
 * @brief   void InitAOI_IOpAmp(void)
 *           - Initialization of the And-Or-Inverter peripheral
 *
 * @param   void
 *
 * @return  none
...
static void InitAOI_IOpAmp(void)
{
    /* Enable AOI AHB clock */
    SYSCON->AHBCLKCTRLSET[3] |= (1U << 7);

    /*Configure AOI0 to perform an A & B & C function */
    AOI0->BFCRT[0].BFCRT01 = 0x5700U;
    AOI0->BFCRT[0].BFCRT23 = 0x0000U;
}
...
static void InitInputmux(void)
{
    ...
    /* Faults aggregated through AOI module */
    INPUTMUX->AOI0_IN[0] = 24U;         /* Route HSCMP0_OUT to AOI0_IN0 */
    INPUTMUX->AOI0_IN[1] = 25U;         /* Route HSCMP1_OUT to AOI0_IN1 */
    INPUTMUX->AOI0_IN[2] = 26U;         /* Route HSCMP2_OUT to AOI0_IN2 */

    /* Route AOI event 0 to PWM0 fault 0 input */
    INPUTMUX->PWM0_FAULT[0] = 34U;

    /* ADC0 trigger 0 = PWM0_SM0_MUX_TRIG0 */
    INPUTMUX->ADC0_TRIG[0] = 24U;
}
...
void MCDRV_Init_M1(void)
{
    ...
    InitAOI_IOpAmp();           /* Init AOI peripheral for overcurrent detection */

    InitHsCmp0_IOpAmp();        /* Init High Speed Comparator 0 */
    InitHsCmp1_IOpAmp();        /* Init High Speed Comparator 1 */
    InitHsCmp2_IOpAmp();        /* Init High Speed Comparator 2 */
