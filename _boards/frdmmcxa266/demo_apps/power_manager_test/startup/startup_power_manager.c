...
extern uint32_t __DATA_GENERAL_END[];
extern uint32_t __data_aon_start__[];
extern uint32_t __data_aon_end__[];
...
void Reset_Handler(void)
{
    ...
    pDataSrc  = (uint32_t *)__DATA_GENERAL_END;
    pDataDest = (uint32_t *)__data_aon_start__;
    while (pDataDest < __data_aon_end__)
    {
        *pDataDest++ = *pDataSrc++;
    }
