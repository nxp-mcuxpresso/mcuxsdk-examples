...
- The non secure binary size for this example (tfm_psa_test_ns.bin) exceeds 96K. u-boot currently allows loading only 96K OCRAM. A fix for this is being added in flash.bin being used on Cortex A core to boot u-boot and will be available in later linux factory releases. For now to run this example, see the section "What to do if your non secure binary size excceds 96K?" below
...
3.  Open a serial terminal with the following settings:
    ...
    - One stop bit
    - No flow control
4.  Download the psatest binaries to the target board.
    ...
    Reference 'Getting Started with MCUXpresso SDK for MCIMX93-EVK.pdf' to download the psatest binaries on board.
...
UID 12 set failed due to insufficient space
...
What to do if your non secure binary size excceds 96K ?
-------------------------------------------------------

1. Apply the patch below to imx-atf repository. This would now allow write upto 192K of OCRAM from u-boot.
diff --git a/plat/imx/imx93/trdc_config.h b/plat/imx/imx93/trdc_config.h
index 6c8b8f8fa..46cdff275 100644
--- a/plat/imx/imx93/trdc_config.h
+++ b/plat/imx/imx93/trdc_config.h
@@ -176,12 +176,26 @@ struct trdc_mbc_config trdc_n_mbc[] = {
...
2. Compile the imx-atf
make PLAT=imx93 SPD=opteed bl31

3. Regenerate the flash.bin. See Section "How to build imx-boot image by using imx-mkimage" in "i.MX Linux User Guide"

Please refer to the latest "UG10163_i.MX_Linux_User's_Guide" to help with steps above.
