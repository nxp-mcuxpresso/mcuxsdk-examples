...
FLASH_PARTITION {
  ...
  # FlashPart1 : ORIGIN_P = 0x0,     ORIGIN_X = 0x20000, LENGTH = 0x20000
...
MEMORY {
  ...
  .p_RamCode     (RX): ORIGIN = 0x60000, LENGTH = 0x4000           # RAM mapped into p space, code resides in flash and runs in RAM

  .p_RamData     (RX): ORIGIN = AFTER(.p_RamCode) - 0x60000, LENGTH = 0x4000 + 0x60000 - AFTER(.p_RamCode)  # .data section
  .x_Data        (RW): ORIGIN = AFTER(.p_RamCode) - 0x60000, LENGTH = 0x4000 + 0x60000 - AFTER(.p_RamCode)  # bss, heap, stack ...
...
#    * (rtlib.text)
  ...
  .RamCode : AT(_RamCode_storage_start_addr)
  {
    ...
    # RTCESL functions placed in RAM
	OBJECT(FAMCLIB_PMSMBemfObsrvDQ_F16_FAsm, AMCLIB_SDM.lib)
	OBJECT(FAMCLIB_TrackObsrv_F16_FAsm, AMCLIB_SDM.lib)
	OBJECT(FMLIB_DivSat_F32_FAsm, MLIB_SDM.lib)
	OBJECT(FGFLIB_AtanYX_F16_FAsm, GFLIB_SDM.lib)
	...
	OBJECT(FGFLIB_Lut1D_F16_FAsm, GFLIB_SDM.lib)
	OBJECT(FGFLIB_Ramp_F32_FAsm, GFLIB_SDM.lib)
	OBJECT(FGFLIB_Sqrt_F16_FAsm, GFLIB_SDM.lib)
	OBJECT(FGMCLIB_ElimDcBusRipFOC_F16_FAsm, GMCLIB_SDM.lib)
	OBJECT(FGMCLIB_SvmStd_F16_FAsm, GMCLIB_SDM.lib)
    * (codesInRam.text)
