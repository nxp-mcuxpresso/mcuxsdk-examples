...
Prepare the Demo
===============
1.  Connect 12V~20V power supply and J-Link Debug Probe to the board, switch SW11 to power on the board.
2.  Connect a micro USB cable between the host PC and the J30 USB port on the target board.
...
4.  Download the flash.bin to the target board.
    ...
    Reference 'Getting Started with MCUXpresso SDK for IMX943-EVK.pdf' to make and download flash.bin.
...
4.  Enter the 'tz_bin' directory and execute 'dd' command to combin the two files.
    dd if=sdk20-app_ns.bin of=sdk20-app_s.bin bs=1 count=`ls -l ./sdk20-app_ns.bin|awk '{print $5}'` seek=131072
5.  Rename sdk20-app_s.bin as m33s_image.bin, and copy it into imx-mkimage
