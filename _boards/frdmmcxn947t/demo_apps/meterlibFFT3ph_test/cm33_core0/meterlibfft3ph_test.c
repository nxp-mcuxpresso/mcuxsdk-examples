...
int main(void)
{
...
#if 0
    /* Clock enable fog GPIO0 */
    CLOCK_EnableClock(kCLOCK_Gpio0);
    /* GPIO0 pin 05, output mode */
    GPIO0->PDDR                     = GPIO_PDDR_PDD5(1);
#endif
    ...
    while(1)
    {
...
#if 0
        /* GPIO0 pin 05, Set output value, go to "H", code performance measurement start pulse */
        GPIO0->PSOR                     = GPIO_PSOR_PTSO5(1);
#endif
        /* main calculation (FFT, scaling, averaging) */
        METERLIBFFT3PH_CalcMain(&ui); 
#if 0
        /* GPIO0 pin 05, Clear output value, go to "L", code performance measurement stop pulse */
        GPIO0->PCOR                     = GPIO_PCOR_PTCO5(1);
#endif
        ...
        tmr_cmp = METERLIBFFT3PH_CalcWattHours(&ui, &whi, &whe, frequency); 
        /* calculates reactive energy increment and treshold value for reactive energy timer */
        tmr_cmp = METERLIBFFT3PH_CalcVarHours(&ui, &varhi, &varhe, frequency); 
