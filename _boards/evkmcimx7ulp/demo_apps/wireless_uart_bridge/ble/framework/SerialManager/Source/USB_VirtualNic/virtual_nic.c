...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @name
                                                                              *VNIC_MacAddressIsUninitialized(void)
                                                                              *
                                                                              * @brief The function
                                                                              *
                                                                              *
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @name VNIC_CompareNDISParamName(uint8_t*
                                                                              *pdata, uint)
                                                                              *
                                                                              * @brief The function
                                                                              *
                                                                              * @param pParamNameInfoBuff: pointer to
                                                                              *param name in information buffer of
                                                                              *REMOTE_NDIS_SET_MSG
                                                                              * @param pParamName:         pointer to a
                                                                              *constant string representing parameter
                                                                              *name
                                                                              * @param size:
                                                                              *
...
/**************************************************************************/ /*!
                                                                              *
                                                                              * @name
                                                                              *VNIC_ValidateNDISPacketHeader(uint8_t*
                                                                              *pdata, uint)
                                                                              *
                                                                              * @brief The function returns the Line
                                                                              *Coding/Configuration
                                                                              *
                                                                              * @param handle:        handle
                                                                              * @param interface:     interface number
                                                                              * @param coding_data:   output line coding
                                                                              *data
                                                                              *
                                                                              * @return USB_OK When Successfull
                                                                              *         USBERR_INVALID_REQ_TYPE when
                                                                              *Error
                                                                              *****************************************************************************/
static bool_t VNIC_ValidateNDISPacketHeader(uint8_t *pdata, uint32_t size)
{
...
usb_status_t USB_DeviceCdcRndisCallback(class_handle_t handle, uint32_t event, void *param)
{
    ...
    switch (event)
    {
        ...
        case kUSB_DeviceCdcEventAppConfigParameter:
        {
            ...
            paramType =
                (((rndisConfigParamInfo_t *)pInfoBuff)->paramType) ? vnicParamType_String_c : vnicParamType_Numeric_c;
            ...
            while (pParamStruct->pfVNICConfigParamCallback && pParamStruct->psParamName)
            {
                ...
                {
                    if (VNIC_CompareNDISParamName(pInfoBuff + paramNameOffset, pParamStruct->psParamName,
                                                  paramNameLength >> 1))
                    {
                        pParamStruct->pfVNICConfigParamCallback(paramType, pInfoBuff + paramValueOffset,
                                                                paramValueLength);
                    }
                }
                pParamStruct++;
    ...
    switch (event)
    {
            ...
            if (TRUE == mDeviceConfigured)
            {
                if (mRestOfBytesToSend)
                {
                    error              = USB_DeviceCdcAcmSend(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_IN_ENDPOINT,
                                                              mpToRestOfBytesToSend, mRestOfBytesToSend);
        ...
        case kUSB_DeviceCdcEventRecvResponse:
        {
            ...
            if (gVnicRecvStatus == vnicRecvStatus_WaitForHeader_c)
            {
                ...
                {
                    error = USB_DeviceCdcAcmRecv(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_OUT_ENDPOINT, maRecvDataBuff,
                                                 mRecvBuffSize_c);
                    break;
                }
                ...
                if (mpRCSAlias)
                {
                    mpReceiveBuffer = mpRCSAlias->pfEthernetHeaderReceived(
                        maRecvDataBuff + RNDIS_USB_OVERHEAD_SIZE, mRestOfBytesToReceive - RNDIS_USB_OVERHEAD_SIZE);
                }
                ...
                if (mpReceiveBuffer)
                {
                    ...
                    for (mRxPayloadLength = 0; mRxPayloadLength < mBytesAlreadyReceived - RNDIS_USB_OVERHEAD_SIZE;
                         mRxPayloadLength++)
                    {
            ...
            if (mRestOfBytesToReceive)
            {
                ...
                if (mpReceiveBuffer)
                {
                    error = USB_DeviceCdcAcmRecv(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_OUT_ENDPOINT,
                                                 mpReceiveBuffer + mRxPayloadLength, mRestOfBytesToReceive);
                }
                else
                {
                    error = USB_DeviceCdcAcmRecv(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_OUT_ENDPOINT, maRecvDataBuff,
            ...
            if (gVnicRecvStatus == vnicRecvStatus_WaitForHeader_c)
            {
                ...
                mRestOfBytesToReceive =
                    USB_LONG_TO_LITTLE_ENDIAN(((rndis_packet_msg_format_t *)maRecvDataBuff)->messageLen);
            ...
            while (mBytesToSMRxIn != mBytesToSMRxOut)
            {
                ...
                {
                    OSA_InterruptEnable();
                    mBytesToSMRxOut += SerialManager_VirtualNicRxNotify(maRecvDataBuff + mBytesToSMRxOut,
                                                                        mBytesToSMRxIn - mBytesToSMRxOut, mSerialIndex);
                }
            }
            if (mRestOfBytesToReceive)
            {
        ...
        case kUSB_DeviceEventSetConfiguration:
            if (param)
            {
                ...
                if (USB_CDC_VNIC_CONFIGURE_INDEX == (*temp8))
                {
                    ...
                    mSendInProgress   = FALSE;
                    gVnicRecvStatus   = vnicRecvStatus_WaitForHeader_c;
                    (void)USB_DeviceCdcAcmRecv(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_OUT_ENDPOINT, maRecvDataBuff,
                                               mRecvBuffSize_c);
                }
            }
            break;
...
void *VirtualNic_Init(uint8_t param)
{
    ...
    if (vNicInit == FALSE)
    {
        ...
        if (kStatus_USB_Success !=
            USB_DeviceCdcRndisInit(g_cdcVnic.cdcAcmHandle, &rndisConfig, &(g_cdcVnic.rndisHandle)))
        {
...
serialStatus_t VirtualNic_Write(void *interface, uint8_t *nic_data_ptr, uint16_t length)
{
    ...
    /* It seems that Linux RNDIS driver rejects all packets smaller than 58 bytes(44(packet header) + 14). So, for those
    packets padding bytes(zero bytes) are added */
    while (i < 58)
    {
        first_send_buff[i++] = 0;
    }
 ...
 **
 *****************************************************************************/
extern void VirtualNic_RegisterReceiveDirecltyFromUSBCallbacks(
    const vnicReceiveCallbacksStruct_t *pReceiveCallbacksStruct)
{
#if gVirtualNIC_ReceiveDirectlyFromUSB_d
...
void VirtualNic_ReceiveUnlock(void *pBuff)
{
    ...
    if ((gVnicRecvStatus == vnicRecvStatus_ReceiveLocked_c) && pBuff)
    {
        ...
        {
            gVnicRecvStatus = vnicRecvStatus_WaitForData_c;
            USB_DeviceCdcAcmRecv(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_OUT_ENDPOINT,
                                 mpReceiveBuffer + mRxPayloadLength, mRestOfBytesToReceive);
        }
        else
        {
            gVnicRecvStatus = vnicRecvStatus_WaitForHeader_c;
            mpRCSAlias->pfEthernetPacketReceived(mpReceiveBuffer, mRxPayloadLength);
            USB_DeviceCdcAcmRecv(g_cdcVnic.cdcAcmHandle, USB_CDC_VNIC_BULK_OUT_ENDPOINT, maRecvDataBuff,
                                 mRecvBuffSize_c);
        }
    }
#else
