...
Board settings
============
This demo is configured to communicate over a WiFi network.
Make sure the network SSID and password are configured in freemaster_cfg.h or directly in network_wifi.c file. The SSID and password must match the local WiFi settings.
This demo uses automatic DHCP address resolution so also make sure this service is enabled on the local network.
