 ...
 * Definitions
 ******************************************************************************/
static void BOARD_InitPCAL6408(uint32_t i2cDevId, void *base, uint32_t clkFreq, uint8_t i2cAddr);
static status_t BOARD_PCAL6408_SetPinToHigh(uint32_t i2cDevId, uint8_t pinIdx);
static status_t BOARD_PCAL6408_SetPinToLow(uint32_t i2cDevId, uint8_t pinIdx);
static status_t BOARD_PCAL6408_SetPinAsInput(uint32_t i2cDevId, uint8_t pinIdx);
static status_t BOARD_PCAL6408_SetPinAsOutput(uint32_t i2cDevId, uint8_t pinIdx);

static void BOARD_InitPCAL6524(uint32_t i2cDevId, void *base, uint32_t clkFreq, uint8_t i2cAddr);
static status_t BOARD_PCAL6524_SetPinToHigh(uint32_t i2cDevId, uint8_t pinIdx);
static status_t BOARD_PCAL6524_SetPinToLow(uint32_t i2cDevId, uint8_t pinIdx);
static status_t BOARD_PCAL6524_SetPinAsInput(uint32_t i2cDevId, uint8_t pinIdx);
static status_t BOARD_PCAL6524_SetPinAsOutput(uint32_t i2cDevId, uint8_t pinIdx);
/*******************************************************************************
 * Variables
 ******************************************************************************/
...
static pcal6408_handle_t g_pcal6408Handle[BOARD_PCAL6408_NUM];
static pcal6524_handle_t g_pcal6524Handle[BOARD_PCAL6524_NUM];

i2c_device_table_t g_i2cDevTbl[] = {
    [BOARD_PCAL6408_I2C3_ID & I2C_DEVICE_ARRAY_IDX_MASK] = {
        .handle = &g_pcal6408Handle[BOARD_PCAL6408_I2C3_HANDLE_IDX],
    },
   [BOARD_PCAL6524_I2C3_ID & I2C_DEVICE_ARRAY_IDX_MASK] = {
        .handle = &g_pcal6524Handle[BOARD_PCAL6524_I2C3_HANDLE_IDX],
    },
};

static io_expander_t g_ioExpanderData[] = {
    [PCAL6408_TYPE - PCAL6408_TYPE] = {
        .init = BOARD_InitPCAL6408,
	.set_pin_to_high = BOARD_PCAL6408_SetPinToHigh,
	.set_pin_to_low = BOARD_PCAL6408_SetPinToLow,
	.set_pin_as_input = BOARD_PCAL6408_SetPinAsInput,
	.set_pin_as_output = BOARD_PCAL6408_SetPinAsOutput,
    },
    [PCAL6524_TYPE - PCAL6408_TYPE] = {
        .init = BOARD_InitPCAL6524,
	.set_pin_to_high = BOARD_PCAL6524_SetPinToHigh,
	.set_pin_to_low = BOARD_PCAL6524_SetPinToLow,
	.set_pin_as_input = BOARD_PCAL6524_SetPinAsInput,
	.set_pin_as_output = BOARD_PCAL6524_SetPinAsOutput,
    },
};
...
void BOARD_InitI2cDevice(uint32_t i2cDevId)
{
    ...
    if (deviceType < UNKNOWN_TYPE)
    {
        io_expander_t *ioExpanderData = NULL;

        g_i2cDevTbl[i2cDevArrayIdx].data = &g_ioExpanderData[deviceType];
	ioExpanderData = g_i2cDevTbl[i2cDevArrayIdx].data;
	ioExpanderData->init(i2cDevId, base, clkFreq, i2cAddr);
    }
    else
    {
...
status_t BOARD_EXPANDER_SetPinToHigh(uint32_t i2cDevId, uint8_t pinIdx)
{
    ...
    (void)deviceType;
    assert(deviceType < UNKNOWN_TYPE);
    ...
    ioExpanderData = g_i2cDevTbl[i2cDevArrayIdx].data;
    assert(ioExpanderData != NULL);

    status = ioExpanderData->set_pin_as_output(i2cDevId, pinIdx);
    if (status == kStatus_Success)
    {
        status = ioExpanderData->set_pin_to_high(i2cDevId, pinIdx);
    }
...
status_t BOARD_EXPANDER_SetPinToLow(uint32_t i2cDevId, uint8_t pinIdx)
{
    ...
    if (status == kStatus_Success)
    {
        status = ioExpanderData->set_pin_to_low(i2cDevId, pinIdx);
    }
...
status_t BOARD_EXPANDER_SetPinAsInput(uint32_t i2cDevId, uint8_t pinIdx)
{
    ...
    assert(ioExpanderData->set_pin_as_input != NULL);

    status = ioExpanderData->set_pin_as_input(i2cDevId, pinIdx);
...
static status_t BOARD_PCAL6524_SetPinToHigh(uint32_t i2cDevId, uint8_t pinIdx)
{
   ...
   return PCAL6524_SetPins(handle, (1U << pinIdx));
}
...
static status_t BOARD_PCAL6524_SetPinToLow(uint32_t i2cDevId, uint8_t pinIdx)
{
   ...
   return PCAL6524_ClearPins(handle, (1U << pinIdx));
}
...
static status_t BOARD_PCAL6524_SetPinAsInput(uint32_t i2cDevId, uint8_t pinIdx)
{
   ...
   return PCAL6524_SetDirection(handle, (1U << pinIdx), kPCAL6524_Input);
}
...
static status_t BOARD_PCAL6524_SetPinAsOutput(uint32_t i2cDevId, uint8_t pinIdx)
{
   ...
   return PCAL6524_SetDirection(handle, (1U << pinIdx), kPCAL6524_Output);
}
