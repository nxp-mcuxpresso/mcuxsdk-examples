...
static void Timer_Init(void)
{
    ...
    /* Generate a PWM signal with 50% dutycycle */
    QTMR_SetupPwm(BOARD_QTMR_BASEADDR, BOARD_QTMR_FIRST_CHANNEL, BOARD_PWM_FREQ, 50, false, QTMR_SOURCE_CLOCK / 2);
    /* Start the counter */
...
void UpdatePwmDutycycle(TMR_Type *base, qtmr_channel_selection_t channel, uint32_t pwmFreqHz, uint8_t dutyCyclePercent)
{
    ...
    QTMR_SetupPwm(base, channel, pwmFreqHz, dutyCyclePercent, false, QTMR_SOURCE_CLOCK / 2);
...
static void Board_UpdatePwm(uint16_t x)
{
    UpdatePwmDutycycle(BOARD_QTMR_BASEADDR, (qtmr_channel_selection_t)BOARD_QTMR_FIRST_CHANNEL, BOARD_PWM_FREQ, x);
}
int main(void)
{
