@echo off
@rem Script to build K32W1xx Master Boot Image using nxpimage tool
...
pushd "%SPT_WORKSPACE%"
@echo %ifr% generate-binary -d k32w1xx -c "%SPT_WORKSPACE%\configs\romcfg.json" -o "%SPT_WORKSPACE%\configs\romcfg.bin"
"%ifr%" generate-binary -d k32w1xx -c "%SPT_WORKSPACE%\configs\romcfg.json" ^
