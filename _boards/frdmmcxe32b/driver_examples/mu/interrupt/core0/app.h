...
/*${macro:start}*/
#define APP_MU              MU2_A
/* core0 (M7_0, primary) uses the A-side of MU2, the inter-CPU messaging unit.
 * MU2_A has dedicated NVIC lines on MCXE32B (TX, RX and general-purpose).
 * core0 services the A-side vectors directly; all three collapse into a single
 * joined handler. The MU flag is cleared by the MU driver inside the handler. */
#define APP_MU_IRQHandler   MU2_A_RX_IRQHandler(void){MU_A_Joined_IRQHandler();}; \
                         void MU2_A_TX_IRQHandler(void){MU_A_Joined_IRQHandler();}; \
                         void MU2_A_IRQHandler(void){MU_A_Joined_IRQHandler();}; \
