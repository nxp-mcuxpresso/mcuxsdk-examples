...
static void init_freemaster_can(void)
{
    ...
    uint32_t canSrcClock;

    memset(&mcanConfig, 0, sizeof(mcanConfig));
    memset(&timingConfig, 0, sizeof(timingConfig));

    /* Set up dividers (it was in clock_config.c) */
    CLOCK_SetClkDiv(kCLOCK_DivCan0Clk, 0U, true);   /* Reset CAN0CLKDIV divider counter and halt it */
    CLOCK_SetClkDiv(kCLOCK_DivCan0Clk, 11U, false); /* Set CAN0CLKDIV divider to value 11 */

    canSrcClock = CLOCK_GetMCanClkFreq(0);
