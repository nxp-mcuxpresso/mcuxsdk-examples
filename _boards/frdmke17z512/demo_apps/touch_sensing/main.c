...
uint8_t nt_memory_pool[6000]; /* IAR EWARM compiler */
#else
uint8_t nt_memory_pool[6000] __attribute__((aligned(4))); /* Keil, GCC compiler */
#endif
...
static void SetHueBrightness(uint32_t hue_angle, uint32_t brightness)
{
    ...
    FTM2->CONTROLS[kFTM_Chnl_0].CnV = (duty_cycle_red * (brightness));
    FTM2->CONTROLS[kFTM_Chnl_2].CnV = (duty_cycle_blue * (brightness));
    FTM2->CONTROLS[kFTM_Chnl_3].CnV = (duty_cycle_green * (brightness));
