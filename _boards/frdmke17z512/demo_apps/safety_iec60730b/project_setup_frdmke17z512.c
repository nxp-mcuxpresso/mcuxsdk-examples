...
#include "project_setup_frdmke17z512.h"
...
void WatchdogEnable(uint32_t wd_setup_value)
{
    ...
    WDOG->CS = WDOG_CS_STOP_MASK | WDOG_CS_WAIT_MASK | WDOG_CS_UPDATE_MASK | WDOG_CS_EN_MASK | WDOG_CS_CMD32EN_MASK |
               WDOG_CS_CLK(1);
}
...
void AdcInit(void)
{
    ...
    ADC0->SC1[0] |= ADC_SC1_ADCH(0x3F); /* module disabled */
    ADC0->SC1[1] =
        0; /* conversion complete interrupt disabled, single-ended conversions anp input channels are selected */
    ADC0->SC1[1] |= ADC_SC1_ADCH(0x3F); /* module disabled */
    ADC0->CFG1 = 0;
...
void Tsi0InitSelfCap(void)
{
    ...
    TSI->MUL1 = TSI_MUL1_M_NMIRROR(3) | TSI_MUL1_M_PMIRRORR(3) | TSI_MUL1_M_PMIRRORL(3) | TSI_MUL1_M_VPRE_CHOOSE_MASK |
                TSI_MUL1_M_MODE_MASK;                                                             // 0x0005007E
    ...
    TSI->SSC2 = TSI_SSC2_MOVE_REPEAT_NUM(1) | TSI_SSC2_MOVE_STEPS_NUM(1) | TSI_SSC2_MOVE_NOCHARGE_MAX(8) |
                TSI_SSC2_MOVE_NOCHARGE_MIN(1); // 0x10080101;
