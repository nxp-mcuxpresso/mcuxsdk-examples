...
status_t FLEXSPI_NorFlash_GetConfig(FlashConfig *config)
{
    ...
    config->memConfig.serialClkFreq =
        kFLEXSPISerialClk_30MHz; /* Serial Flash FrequenceySee System Boot Chapter for more details */
    ...
    config->serialNorType           = 1U;                       /* 0-StandardSPI, 1-HyperBus, 2-XPI, 3-NoCmd */
    ...
    // NOR_CMD_INDEX values for RT1152: READ=0, READSTATUS=1, WRITEENABLE=2, ERASESECTOR=3, PAGEPROGRAM=4, CHIPERASE=5
    config->memConfig.lutCustomSeq[2].seqNum = 2U;  /* WRITEENABLE */
    config->memConfig.lutCustomSeq[2].seqId  = NOR_CMD_LUT_SEQ_IDX_WRITEENABLE;
    config->memConfig.lutCustomSeq[1].seqNum = 2U;   /* READSTATUS */
    config->memConfig.lutCustomSeq[1].seqId  = NOR_CMD_LUT_SEQ_IDX_READSTATUS;
    config->memConfig.lutCustomSeq[4].seqNum = 2U;   /* PAGEPROGRAM */
    config->memConfig.lutCustomSeq[4].seqId  = NOR_CMD_LUT_SEQ_IDX_PAGEPROGRAM;
    config->memConfig.lutCustomSeq[3].seqNum = 4U;   /* ERASESECTOR */
    config->memConfig.lutCustomSeq[3].seqId  = NOR_CMD_LUT_SEQ_IDX_ERASESECTOR;
    config->memConfig.lutCustomSeq[5].seqNum = 4U;   /* CHIPERASE */
    config->memConfig.lutCustomSeq[5].seqId  = NOR_CMD_LUT_SEQ_IDX_CHIPERASE;
