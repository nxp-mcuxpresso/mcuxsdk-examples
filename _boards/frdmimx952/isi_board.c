...
/* Refer into hardware connect, I2C3->RPi-CAM-MIPI ADP5585->AP1302 */
...
status_t BOARD_InitCameraInterface(const camera_config_t *camera_config)
{
    ...
    csi2rxConfig.pgEnable = false;
    csi2rxConfig.vcNum = camera_config->mipiChannel;

    /* MIPI D-PHY High-Speed Frequency Range Configuration
     *
     * Camera: 1080p@30fps, YUV422, 2 data lanes
     *
     * hsFreqrange = 0x29 (binary: 0101001)
     *   → Bit rate: 900 Mbps per lane (ref: i.MX952 RM Table)
     *   → Total bandwidth (2 lanes): 2000 Mbps
     *   → Meets 1080p30 YUV422 requirement
     */
    csi2rxConfig.hsFreqrange = 0x29;
    csi2rxConfig.ddl_osc_freq = 460;
    csi2rxConfig.phyMode = KCSI2RX_DPHY_NormalMode;
    csi2rxConfig.phyNumber = KCSI2RX_DPHY_Primary;

    /* round[(Fcfg_clk(MHz) - 17) × 4], 24Mhz */
    csi2rxConfig.cfgclkFreqrange = 0x1C;

    result = MIPI_CSI2RX_Init(APP_MIPI_CSI2_PRIMARY, APP_MIPI_CSI2_SECONDARY, APP_MIPI_CSI2_PHY, &csi2rxConfig);
