...
The digital input output \(IO\) example is used to demonstrate the steps to develop an Ethernet for Control Automation Technology \(EtherCAT\) subdevice based on Subdevice Stack Code \(SSC\) and ECAT peripheral integrated on FRDM-IMXRT1186.
...
The ECAT controller integrated on FRDM-IMXRT1186 takes care of the EtherCAT communication as an interface between the EtherCAT fieldbus and the sub application.
...
-   Output channel: connected with the D6 LED on the FRDM-IMXRT1186 board and used to control this LED.
