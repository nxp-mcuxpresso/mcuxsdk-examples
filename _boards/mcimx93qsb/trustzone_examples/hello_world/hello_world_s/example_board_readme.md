...
1. Build secure binary and non-secure binary. For example (if west build is used),
   west build -p -b mcimx93qsb examples/trustzone_examples/hello_world_s -Dcore_id=cm33 -d hello_world_s
   west build -p -b mcimx93qsb examples/trustzone_examples/hello_world_ns -Dcore_id=cm33 -d hello_world_ns
