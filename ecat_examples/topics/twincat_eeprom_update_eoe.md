# TwinCAT EEPROM Update

## Import the SubDevice ESI file
Copy the ESI file '`ECAT-EOE.xml`' generated by SSC tool to TwinCAT install directory: `C:\Program Files/3.1/Config/io/EtherCAT/` in general.    
## Create a new project 

1.  Select **File \> New \> Project**.

    ![](../images/new_project0.png "Create new project")

2.  Select **TwinCAT Projects** and name this project **EtherCAT_EOE**.
3.  Click **OK**.

    ![](../images/new_project1.png "Select project ")

## Update the ESI file to EEPROM 

**Note:** The EEPROM update is required if the EoE example is set up at first time on the board.
### Scan the subdevices 

1.  Right-click on **Device** \> **Scan** in the **Solution Explorer** pane.

    ![](../images/new_project4.png "Scan for the devices")

2.  Select the realtime network interface connected with the SubDevice.
3.  Click **OK**.
    ![](../images/new_project5.png "Devices scanned")
4.  Click **Yes** to scan for boxes.  
    ![](../images/new_project6.png "Scan for boxes")
5.  Click **No** to refuse to activate Free Run mode
    ![](../images/new_project7.png "Inactivate Free Run mode")

### EEPROM Update
1.  Select the subdevice under EtherCAT devices node: **Device X (EtherCAT)**
2.  Double-click the Subdevice: **Box 1 \(PFFFFFFFF RFFFFFFFF\)**.
3.  Click the **EtherCAT** tab.
4.  Click the **Advanced Settings** button.
   ![](../images/EEPROM_UPDATE0.png "update eeprom route")  
5.  Select **ESC Access \> Smart View** and click the **Write EEPROM** button.
   ![](../images/EEPROM_UPDATE1.png "update eeprom pane")  

6.  Select **NXP \> ECAT \> eoe_freertos**.
    ![](../images/EEPROM_UPDATE2.png "Select ESI file")
7.  Click **OK**.  
8.  After EEPROM update completion, click **Ok** to exit. The status of EEPROM writing is shown on the bottom of TwinCAT IDE.
    ![](../images/EEPROM_UPDATE2_1.png "Select ESI file")
9.  Right-click on **Device X (EtherCAT)** node and remove the **Device X (EtherCAT)** node.  
    ![](../images/EEPROM_UPDATE3.png "delete EtherCAT devices")