...
void SafetyWatchdogTest(safety_common_t *psSafetyCommon, wd_test_t *psSafetyWdTest)
{
    ...
    if (*(RESET_DETECT_REGISTER)&runTestConditionSRS0) /* if non WD reset --- because of debugging--- in real it must be
    ...
    if (*(RESET_DETECT_REGISTER)&checkTestCondition) /* if WD reset --- because of debugging--- in real it should be
                                                        after every non-POR reset*/
    {
        psSafetyWdTest->wdTestLptmrCnt = WATCHDOG_TEST_VARIABLES->counter;
        FS_WDOG_Check(counterLimitHigh, counterLimitLow, WATCHDOG_RESETS_LIMIT, ENDLESS_LOOP_ENABLE,
                      WATCHDOG_TEST_VARIABLES, CLEAR_FLAG, REG_WIDE);
    }
                         ...
                         fs_flash_configuration_parameters_t *psFlashConfig)
{
  ...
  psFlashConfig->endAddress = 4 + (uint32_t)&c_checksumEnd; /*We must test also last adress, due to this reason +4 in IAR*/
    #if defined(K32L3A60_CM0PLUS_H_)
        psFlashConfig->startAddress = 0x01000500;
        psFlashConfig->endAddress = 0x01002000;
    #endif
...
void SafetyFlashAfterResetTest(safety_common_t *psSafetyCommon, fs_flash_configuration_parameters_t *psFlashConfig)
{
    ...
    {
#if HW_FLASH_TEST
        psSafetyCommon->FLASH_test_result = FS_CM0_FLASH_HW16(psFlashConfig->startAddress, psFlashConfig->size,
...
#if HW_FLASH_TEST
        psFlashCrc->partCrc =
            FS_CM0_FLASH_HW16(psFlashCrc->actualAddress, psFlashCrc->blockSize, CRC_BASE, psFlashCrc->partCrc);
#else
        psFlashCrc->partCrc =
        ...
        if (c_ramEnd > (psSafetyRamTest->actualAddress + psSafetyRamTest->blockSize))
            psSafetyCommon->RAM_test_result =
                FS_CM0_RAM_Runtime(psSafetyRamTest->ramTestStartAddress, psSafetyRamTest->ramTestEndAddress,
                                   ...
                                   psSafetyRamTest->backupAddress, FS_CM0_RAM_SegmentMarchX);
    }
    else
    {
        psSafetyCommon->safetyErrors |= RAM_TEST_ERROR;
...
void SafetyAnalogTest(safety_common_t *psSafetyCommon )
{
  ...
  {
  case FS_AIO_INIT:
    FS_AIO_InputSet_A23(g_aio_safety_test_items[index], (fs_aio_a23_t *)TESTED_ADC);
    break;
  case FS_AIO_PROGRESS:
   FS_AIO_ReadResult_A23(g_aio_safety_test_items[index], (fs_aio_a23_t *)TESTED_ADC);
    break;
  ...
  case FS_AIO_INIT:
    ...
    break;
  case FS_AIO_PROGRESS:
    FS_AIO_ReadResult_A4(g_aio_safety_test_items[index], (fs_aio_a4_t *)TESTED_ADC);
    break;
  case FS_PASS: /* successfull execution of test, call the trigger function again */
    if( g_aio_safety_test_items[++index] == NULL)
    {
