...
#include "board.h"
#include "pin_mux.h"
#ifdef K32L3A60_CM4_H_
#include "fsl_mu.h"
#endif /* K32L3A60_CM4_H_ */
...
uint32_t MU_Synchronous_Start = 0;
volatile uint32_t SMC_boot_source;
volatile uint32_t ui32Message;

typedef struct _safety_common_cm0
{
...
typedef struct _flash_runtime_test_parameters_cm0
{
   ...
   uint32_t ui32PartCrc;           /* seed in begin, particular crc result in process, crc result in final*/
} flash_runtime_test_parameters_cm0_t;
    ...
    volatile safety_common_cm0_t g_sCM0Shared @ "rpmsg_sh_mem_section";
    volatile uint32_t ui32shCM0ClockTest @ "rpmsg_sh_mem_section";
    volatile unsigned short ui16CM0Checksum @ "rpmsg_sh_mem_section";
    volatile flash_runtime_test_parameters_cm0_t g_sCM0FlashTet @ "rpmsg_sh_mem_section";
    ...
    volatile safety_common_cm0_t g_sCM0Shared __attribute__((section(".rpmsg_sh_mem_section")));
    ...
    volatile flash_runtime_test_parameters_cm0_t g_sCM0FlashTet __attribute__((section(".rpmsg_sh_mem_section")));
...
volatile uint16_t ui16CrcVarInit;
volatile uint32_t ui32AdcResult;

volatile uint32_t debugWatchdogRefresh;
volatile uint32_t debugSMC0_SRS;
volatile uint8_t debugSMC0_SRS_Byte0;
volatile uint8_t debugSMC0_SRS_Byte1;
volatile uint8_t debugSMC0_SRS_Byte2;
volatile uint8_t debugSMC0_SRS_Byte3;
...
int32_t main(void)
{
...
#ifdef K32L3A60_CM4_H_
    /* Reset sources ... */
    debugSMC0_SRS = SMC0->SRS;
    debugSMC0_SRS_Byte0 = (uint8_t)(debugSMC0_SRS & 0xFF);
    debugSMC0_SRS_Byte1 = (uint8_t)((debugSMC0_SRS & 0xFF00)>>8);
    debugSMC0_SRS_Byte2 = (uint8_t)((debugSMC0_SRS & 0xFF0000)>>16);
    debugSMC0_SRS_Byte3 = (uint8_t)((debugSMC0_SRS & 0xFF000000)>>24);

    ui32shTempResult = 0;                       /* auxiliary/debug variable from shared memory */
    g_sCM0Shared.IEC60730B_aio_test_result = 0; /* initialization of shared variable, so it will not be removed as unused */
    ui16CrcVarInit = ui16CM0Checksum;           /* initialization of shared variable, so it will not be removed as unused */
    ui32shCM0ClockTest = 0;                     /* initialization of shared variable, so it will not be removed as unused */
    g_sCM0FlashTet.ui32ActualAddress = 0;       /* initialization of shared variable, so it will not be removed as unused */
    g_sCM0WatchDogTest.watchdogResets = 0;  /* initialization of shared variable, so it will not be removed as unused */
#endif /* K32L3A60_CM4_H_ */
    ...
    PCC0->CLKCFG[PCC_MUA_INDEX] |= PCC_CLKCFG_CGC_MASK;  /* enable clock to MUA */

    SMC_boot_source = SMC0->MR | SMC_MR_BOOTCFG_MASK;  /* read the boot source */
    SMC0->MR |= SMC_MR_BOOTCFG_MASK;   /* clear the register */
    MUA->CCR |= MU_CCR_HRM_MASK;       /* masks the reset from cm0 */

    /* if the define is valid, CM0+ core will start */
#if RUN_CM0PLUS_CORE  /* release CM0+ frome reset */
    if (!(MUA->SR & MU_SR_RDIP_MASK)) /* if Processor B is in reset */
    {
        uint32_t reg = MUA->CCR;
        reg = (reg & ~(MU_CCR_HR_MASK | MU_CCR_RSTH_MASK | MU_CCR_BOOT_MASK)) | MU_CCR_BOOT(MU_Synchronous_Start);
        /* HR - Hardware reset. clear to de-assert Hardware reset to the Processor B */
        /* RSTH - Processor B Reset Hold. clear to allows the Processor B to be released from reset */
        /* BOOT - Slave Processor B Boot Config. 0 - Boot from Dflash base */
        MUA->CCR = reg;
        /* Wait for coming out of reset. */
        while (!(MUA->SR & MU_SR_RDIP_MASK))
        {
        }
        /* waits for message from CM0+ */
        while (!(MUA->SR & MU_SR_RFn_MASK))
        {
            delay(10000);
            #if WATCHDOG_ENABLED
            Watchdog_refresh; /********************************************************/
        ...
        ui32Message = MUA->RR[0];
    }
#endif /* RUN_CM0PLUS_CORE */
#endif /* K32L3A60_CM4_H_ */
...
/* function for creating some delay */
void delay(uint32_t ui32Value)
{
    for (uint32_t i = 0; i < ui32Value; i++)
    {
        __NOP();
    }
}
...
void SYSTICK_Isr(void)
{
    ...
    if((counter % 1000) == 0)
    {
      LED1_TOGGLE(); //Core CM4 toggles RED LED
    }
