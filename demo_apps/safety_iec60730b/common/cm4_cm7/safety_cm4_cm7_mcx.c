...
#include "safety_cm4_cm7_mcx.h"
   ...
   #include "freemaster.h"/* Freemaster cummunication */
#endif
...
#ifdef MCXE31B_H_
    psSafetyCommon->mcgirclkFreq         = STM_CLOCK_FREQUENCY;
#endif
...
#ifdef MCXE31B_H_
    FS_CLK_STM((fs_stm_t *)STM_USED, (uint32_t *)&psSafetyClockTest->clockTestContext);
#endif
...
{
#if HW_FLASH_TEST
    uint32_t after_reset_crc = psFlashConfig->startConditionSeed;
  
    psSafetyCommon->FLASH_test_result = FS_FLASH_C_HW32_K(psFlashConfig->startAddress, psFlashConfig->size,
                                                               (FS_CRC_Type *)CRC_BASE, &after_reset_crc);
    if (after_reset_crc != psFlashConfig->checksum)
    {
        psSafetyCommon->safetyErrors |= FLASH_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
    }
#else
    psSafetyCommon->FLASH_test_result = FS_CM4_CM7_FLASH_SW32(psFlashConfig->startAddress, psFlashConfig->size,
...
#if HW_FLASH_TEST
    psSafetyCommon->FLASH_test_result = FS_FLASH_C_HW32_K(psFlashCrc->actualAddress, psFlashCrc->blockSize, (FS_CRC_Type *)CRC_BASE, &psFlashCrc->partCrc);
#else
    psFlashCrc->partCrc = FS_CM4_CM7_FLASH_SW32(psFlashCrc->actualAddress, psFlashCrc->blockSize, CRC_BASE, psFlashCrc->partCrc);
#endif
...
#ifdef MCXE31B_H_
  void SafetyDIOTestInit(safety_common_t *psSafetyCommon, fs_dio_test_siul2_t *pTestItems[])
  {
      uint32_t st = 0;
...
#ifdef MCXE31B_H_
  void SafetyDigitalInputOutput_ShortAdjTest(safety_common_t *psSafetyCommon,
                                             fs_dio_test_siul2_t *pTestedPin,
                                             fs_dio_test_siul2_t *pAdjPin,
                                             uint32_t PinValue)
  {    
      PortInit((SIUL2_Type *)pTestedPin->siul2, pTestedPin->pinNum, PIN_DIRECTION_IN);
      PortInit((SIUL2_Type *)pAdjPin->siul2, pAdjPin->pinNum, PIN_DIRECTION_OUT);

      psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToAdjSet_SIUL2(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
      if ((psSafetyCommon->DIO_short_test_result) != FS_PASS)
      {
          psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
          SafetyErrorHandling(psSafetyCommon);
      ...
      psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_SIUL2(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
      if ((psSafetyCommon->DIO_input_test_result) != FS_PASS)
      {
          psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
          SafetyErrorHandling(psSafetyCommon);
      ...
      PortInit((SIUL2_Type *)pTestedPin->siul2, pTestedPin->pinNum, pTestedPin->pinDir);
      PortInit((SIUL2_Type *)pAdjPin->siul2, pAdjPin->pinNum, pAdjPin->pinDir);
  }
#endif
 ...
 */
  ...
  {   
      ...
      psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToSupplySet_SIUL2(pTestedPin, polarity, DIO_BACKUP);
      if ((psSafetyCommon->DIO_short_test_result) != FS_PASS)
      {
          psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
          SafetyErrorHandling(psSafetyCommon);
      ...
      psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_SIUL2(pTestedPin, pTestedPin, polarity, DIO_BACKUP);
      if ((psSafetyCommon->DIO_input_test_result) != FS_PASS)
      {
          psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
          SafetyErrorHandling(psSafetyCommon);
...
void SafetyAnalogTest(safety_common_t *psSafetyCommon)
{
  ...
  {
  case FS_AIO_INIT:
    FS_AIO_InputSet_A8(g_aio_safety_test_items[index], (fs_aio_a8_t *)TESTED_ADC);
    break;
  case FS_AIO_PROGRESS:
    FS_AIO_ReadResult_A8(g_aio_safety_test_items[index], (fs_aio_a8_t *)TESTED_ADC);
    break;
