...
void SafetyFlashAfterResetTest(safety_common_t *psSafetyCommon, fs_flash_configuration_parameters_t *psFlashConfig)
{
    ...
    psSafetyCommon->FLASH_test_result = FS_FLASH_C_HW16_K(psFlashConfig->startAddress, psFlashConfig->size, (FS_CRC_Type* )CRC_BASE,
                                                          &after_reset_crc);
                            ...
                            fs_flash_configuration_parameters_t *psFlashConfig)
{
   /* CRC calculation for a given block of Flash memory */
    ...
    psSafetyCommon->FLASH_test_result = FS_FLASH_C_HW16_K(psFlashCrc->actualAddress, psFlashCrc->blockSize, (FS_CRC_Type* )CRC_BASE, (uint16_t *)&psFlashCrc->partCrc);
...

{
    ...
    psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToAdjSet_MCX(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
    ...
    for (int32_t i = 0; i < DIO_WAIT_CYCLE; i++)
    {
        __asm("nop");
    }

     psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_MCX(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
                                              ...
                                              uint8_t polarity)
{
    ...
    psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToSupplySet_MCX(pTestedPin, polarity, DIO_BACKUP);
    if ((psSafetyCommon->DIO_short_test_result) != FS_PASS)
    {
        psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
    ...
    psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_MCX(pTestedPin, pTestedPin, polarity, DIO_BACKUP);
    if ((psSafetyCommon->DIO_input_test_result) != FS_PASS)
    {
        psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
...
{
    static TSI_v6_Type TSI_regiters_backup;

    if (actions == BACKUP)
    {
        TSI_regiters_backup.CONFIG   = TSI->CONFIG;
        TSI_regiters_backup.TSHD     = TSI->TSHD;
        TSI_regiters_backup.GENCS    = TSI->GENCS;
        TSI_regiters_backup.MUL      = TSI->MUL;
        ...
        TSI_regiters_backup.BASELINE = TSI->BASELINE;
        TSI_regiters_backup.CHMERGE  = TSI->CHMERGE;
        TSI_regiters_backup.DATA     = TSI->DATA;
        TSI_regiters_backup.MISC     = TSI->MISC;
        TSI_regiters_backup.TRIG     = TSI->TRIG;
    }
    else if (actions == RESTORE)
    {
        TSI->CONFIG   = TSI_regiters_backup.CONFIG;
        TSI->TSHD     = TSI_regiters_backup.TSHD;
        TSI->GENCS    = TSI_regiters_backup.GENCS;
        TSI->MUL      = TSI_regiters_backup.MUL;
        ...
        TSI->BASELINE = TSI_regiters_backup.BASELINE;
        TSI->CHMERGE  = TSI_regiters_backup.CHMERGE;
        TSI->DATA     = TSI_regiters_backup.DATA;
        TSI->MISC     = TSI_regiters_backup.MISC;
        TSI->TRIG     = TSI_regiters_backup.TRIG;
    }
}
...
uint32_t SafetyTsiChanelTest(safety_common_t *psSafetyCommon, fs_tsi_t *pObj)
{
    ...
    {
        FS_TSI_InputCheckNONStimulated_v6(pObj, (uint32_t)TSI); /*Periodically call for result check */
    }

    if ((pObj->state == FS_TSI_PASS_NONSTIM) || (pObj->state == FS_TSI_PROGRESS_STIM))
    { /*NON stimulated input check OK */
        FS_TSI_InputCheckStimulated_v6(pObj, (uint32_t)TSI);
    }
    ...
    {                                                /*First call for this channell occur */
        ...
        FS_TSI_InputCheckNONStimulated_v6(pObj, (uint32_t)TSI);
