...
void SafetyWatchdogTest(safety_common_t *psSafetyCommon, wd_test_t *psSafetyWdTest)
{
    ...
    /* Init timer */
    GPT2_Init(WD_GPT2_SRC, WD_GPT2_CMP, WD_GPT2_DIV);

    if (*(RESET_DETECT_REGISTER)&runTestCondition) /* If non WD reset */
    {
      #if defined(MIMXRT1189_cm33_H_)
          /* Clear tested flags */
...
void SafetyDigitalInputOutput_ShortAdjTest(safety_common_t *psSafetyCommon,
                                           fs_dio_test_rgpio_t *pTestedPin,
...

{
    ...
    psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToAdjSet_RGPIO(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
    ...
    psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_RGPIO(pTestedPin, pAdjPin, PinValue, DIO_BACKUP);
                                              ...
                                              uint8_t polarity)
{
    ...
    psSafetyCommon->DIO_short_test_result = FS_DIO_ShortToSupplySet_RGPIO(pTestedPin, polarity, DIO_BACKUP);
    if ((psSafetyCommon->DIO_short_test_result) != FS_PASS)
    {
        psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
    ...
    psSafetyCommon->DIO_input_test_result = FS_DIO_InputExt_RGPIO(pTestedPin, pTestedPin, polarity, DIO_BACKUP);
    if ((psSafetyCommon->DIO_input_test_result) != FS_PASS)
    {
        psSafetyCommon->safetyErrors |= DIO_TEST_ERROR;
        SafetyErrorHandling(psSafetyCommon);
