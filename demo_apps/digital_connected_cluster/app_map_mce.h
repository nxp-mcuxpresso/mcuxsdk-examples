...
struct app_map_mce_instance
{
    ...
    bool lcl_mns_srm; /* local mns srm */
    bool lcl_mns_srmp_wait; /* local mns srmp */
};

extern struct app_map_mce_instance g_appInstance;
...
void Map_GetMsg(char *handle);
void Map_MessageListing(uint16_t length,uint8_t *value);
void Map_newMsgNotification(char *event,char *handle);
void Map_bMessage(uint16_t length,uint8_t *value);
...
void bt_map_init();
void map_connect(struct bt_conn *conn_handle);
