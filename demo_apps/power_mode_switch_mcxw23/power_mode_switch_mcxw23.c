...
#define GINT0_POL_MASK ~(1U << BOARD_INITPINS_BOARD_BUTTON1_GPIO_PIN_PIN)
...
typedef enum
{
    ...
    kWAKEUP_VECTOR_WWDT,
    kWAKEUP_VECTOR_BOD1,
    kWAKEUP_VECTOR_GINT_BOD1_ENABLED,
    kWAKEUP_VECTOR_BOD2,
...
static void InitWwdt(bool init);
static void InitBOD1(bool init);
static void InitBOD1Gint(bool init);
static void InitBOD2(bool init);
...
static void ConfigureWwdtWakeUp(int ms);
static void ConfigureBOD1WakeUp(int ms);
static void ConfigureBOD2Reset(int ms);
...
static const menu_item_t s_menu[] = {
    ...
    {.title = "Get chip version", .func = GetChipVersion, .param = 0},
    ...
    {.title = "Go to sleep. Wakeup from WWDT", .func = Sleep, .param = kWAKEUP_VECTOR_WWDT},
    ...
    {.title = "Go to deep sleep. Wakeup from WWDT", .func = DeepSleep, .param = kWAKEUP_VECTOR_WWDT},
    ...
    {.title = "Go to power down. Wakeup from pin toggle (GINT)", .func = PowerDown, .param = kWAKEUP_VECTOR_GINT},
    ...
    {.title = "Calibrate FROs", .func = Calibrate, .param = 0},
    {.title = "BOD demo", .func = DemoBODs, .param = 0},
    {.title = "Go to sleep. Wakeup from BOD1 interrupt", .func = Sleep, .param = kWAKEUP_VECTOR_BOD1},
    {.title = "Go to deep sleep. Wakeup from BOD1 interrupt", .func = DeepSleep, .param = kWAKEUP_VECTOR_BOD1},
    {.title = "Go to deep sleep. Wakeup from pin toggle (GINT) with BOD1 interrupt enabled",
     .func  = DeepSleep,
     .param = kWAKEUP_VECTOR_GINT_BOD1_ENABLED},
    {.title = "Go to power down. Wakeup from pin toggle (GINT) with BOD1 interrupt enabled",
     .func  = PowerDown,
     .param = kWAKEUP_VECTOR_GINT_BOD1_ENABLED},
    {.title = "Go to sleep. Reset on BOD2", .func = Sleep, .param = kWAKEUP_VECTOR_BOD2},
    {.title = "Go to deep sleep. Reset on BOD2", .func = DeepSleep, .param = kWAKEUP_VECTOR_BOD2},
    {.title = "Go to power down. Reset on BOD2", .func = PowerDown, .param = kWAKEUP_VECTOR_BOD2},
    {.title = "Go to deep power down. Reset on BOD2", .func = DeepPowerDown, .param = kWAKEUP_VECTOR_BOD2},
    {.title = "Voltage measurement", .func = VoltageMeasurement, .param = 0},
    {.title = NULL, .func = NULL}};

/** Array of all possible wakeup vectors */
static wakeup_vector_t s_wakeupVector[] = {{InitRTC, ConfigureRTCWakeUp, kWAKEUP_RTC, kLOWPOWERCFG_FRO32K},
                                           {InitOSTimer, ConfigureOSTimerWakeUp, kWAKEUP_OS_EVENT, kLOWPOWERCFG_FRO32K},
                                           {InitCTimer, ConfigureCTimerWakeUp, kWAKEUP_CTIMER0, kLOWPOWERCFG_FRO32K},
                                           ...
                                           {InitWwdt, ConfigureWwdtWakeUp, kWAKEUP_WDT, kLOWPOWERCFG_FRO1M},
                                           {InitBOD1, ConfigureBOD1WakeUp, kWAKEUP_BOD1, kLOWPOWERCFG_BOD1},
                                           {InitBOD1Gint, ConfigureGintWakeUp, kWAKEUP_GINT0, kLOWPOWERCFG_BOD1},
                                           {InitBOD2, ConfigureBOD2Reset, 0, kLOWPOWERCFG_BOD2}};
...
/** Variable to keep whether the BOD1 interrupt was handled. */
static volatile bool BOD1InterruptHandled = false;
...
static void InitCTimer(bool init)
{
    if (init)
    {
        ...
        /* Enable 32KHz FRO, used as ctimer clock*/
        POWER_PeripheralPowerOn(kPOWERCFG_FRO32K);
...
void WDT_IRQHandler(void)
{
    ...
    /* Refresh WWDT to avoid timeout */
    WWDT_Refresh(WWDT);
}

/* WDT_BOD_IRQHandler also exists. This is a combined interrupt that is triggered when WDT, BOD1 or BOD2 interrupt is
   triggered.
   To simplify the handling, we only use the single-source interrupts for WDT and BOD1. BOD2 interrupt is not used in
   this demo. */
void BOD1_IRQHandler(void)
{
    PRINTF("BOD1 IRQ\n");

    /* Re-configure BOD1 to not generate an interrupt anymore. Otherwise, the BOD1 interrupt will be triggered again
       immediately as long as the voltage level stays below the threshold.*/
    POWER_ConfigureBOD1(kBOD_NO_ACTION);

    /* Clear the interrupt in the BOD status registers. */
    POWER_ClearBOD1Interrupt();

    /* Signal to the main loop that a BOD interrupt was handled and the BOD1 interrupt is disabled.
        The main loop will enable the BOD1 interrupt again when the voltage level is above the threshold again. */
    BOD1InterruptHandled = true;

    PRINTF("BOD1 IRQ disabled. Raise voltage above 2.2V to re-enable BOD1\n");
}
...
static void InitBOD1(bool init)
{
    if (init)
    {
        PRINTF("BOD1 configured to generate an interrupt at Vbat 2.2V\n");
        POWER_ClearBOD1Interrupt();
        BOD1InterruptHandled = false;
    ...
    {
        PRINTF("De-Init BOD1 interrupt\n");
        POWER_ConfigureBOD1(kBOD_NO_ACTION);
        NVIC_DisableIRQ(BOD1_IRQn);
    }
}

static void ConfigureBOD1WakeUp(int ms)
{
    PRINTF("Lower voltage below 2.2V, to wake me up\n");
}
...
static void InitBOD2(bool init)
{
    if (init)
    {
        PRINTF("BOD2 configured to reset at Vbat 1.8V\n");
        POWER_SetBod2Level(kBOD_LEVEL_1800mv);
        POWER_ConfigureBOD2(kBOD_RESET);
    }
    else
    {
        PRINTF("De-Init BOD2 reset\n");
...
static void ConfigureBOD2Reset(int ms)
{
    PRINTF("Lower voltage below 1.8V, to reset\n");
}
...
static void InitWwdt(bool init)
{
    if (init)
    {
        ...
        /* Clock WWDT at 100KHz, this will give us 40ms between warning and timeout */
...
static void ConfigureWwdtWakeUp(int ms)
{
    ...
    /* Configure WWDT such that warning happens after ms milliseconds. */
    WWDT_GetDefaultConfig(&config);
    config.warningValue        = 0x3ff;
    config.timeoutValue        = wdtFreq * ms / 1000 + config.warningValue;
...
static void ConfigurePintWakeUp(int ms)
{
    ...
    WaitPinStableHigh(BOARD_INITPINS_BOARD_BUTTON1_GPIO_PIN_PIN);

    PRINTF("Push USER button, to wake me up\n");
}
...
{
    /* Make sure pin is debounced before using it (from potential previous usage)*/
    WaitPinStableHigh(BOARD_INITPINS_BOARD_WAKEUP_GPIO_PIN_PIN);

    PRINTF("Push WAKEUP button, to wake me up\n");
}
...
static status_t Calibrate(wakeup_vector_index_t param)
{
    ...
    measuredFreq = SYSTEM_MeasureFrequency(kFREQMEAS_target_clock_32kHz_FRO, kFREQMEAS_ref_time_exp_26);
    PRINTF("Measured frequency before calibration: %d Hz\n", measuredFreq);
    PRINTF("Calibrating ...\n");
    SYSTEM_CalibrateClock(kFREQMEAS_target_clock_32kHz_FRO);
    SYSTEM_CalibrateClockFine(kFREQMEAS_target_clock_32kHz_FRO);
    PRINTF("Measured frequency after calibration: %d Hz\n", measuredFreq);
    PRINTF("\n");

    PRINTF("Calibrate 1MHZ FRO:\n");
    measuredFreq = SYSTEM_MeasureFrequency(kFREQMEAS_target_clock_1MHz_FRO, kFREQMEAS_ref_time_exp_26);
    PRINTF("Measured frequency before calibration: %d Hz\n", measuredFreq);
    PRINTF("Calibrating ...\n");
    SYSTEM_CalibrateClock(kFREQMEAS_target_clock_1MHz_FRO);
    SYSTEM_CalibrateClockFine(kFREQMEAS_target_clock_1MHz_FRO);
    PRINTF("Measured frequency after calibration: %d Hz\n", measuredFreq);
    PRINTF("\n");

    PRINTF("Calibrate 12MHZ FRO:\n");
    measuredFreq = SYSTEM_MeasureFrequency(kFREQMEAS_target_clock_12_24_32MHz_FRO, kFREQMEAS_ref_time_exp_26);
    PRINTF("Measured frequency before calibration: %d Hz\n", measuredFreq);
    PRINTF("Calibrating ...\n");
    SYSTEM_CalibrateClock(kFREQMEAS_target_clock_12_24_32MHz_FRO);
    SYSTEM_CalibrateClockFine(kFREQMEAS_target_clock_12_24_32MHz_FRO);
    PRINTF("Measured frequency after calibration: %d Hz\n", measuredFreq);
...
static status_t DemoBODs(wakeup_vector_index_t param)
{
    ...
    PRINTF("\nLower voltage between 1.8V and 2.2V to trigger BOD1 interrupt\n");
    PRINTF("Lower voltage below 1.8V to trigger BOD2 reset\n\n");

    while (demoLoopTimeout--)
    {
        PRINTF("%d\n", demoLoopTimeout);
        PRINTF("BOD1: %s\n", (POWER_IsBOD1Active() ? "active" : "not active"));
        PRINTF("BOD2: %s\n", (POWER_IsBOD2Active() ? "active" : "not active"));

        if (BOD1InterruptHandled)
        {
            /* BOD 1 interrupt triggered and was handled. If BOD1 is no longer active, we can re-enable the BOD1
             * interrupt. */
            if (!POWER_IsBOD1Active())
            {
                PRINTF("BOD1 configured to generate an interrupt again\n");
                BOD1InterruptHandled = false;
                POWER_ClearBOD1Interrupt();
                POWER_ConfigureBOD1(kBOD_INTERRUPT);
            }
        }
        /* Delay 1 second to avoid spamming the UART with output. */
        SDK_DelayAtLeastUs(1000 * 1000, SystemCoreClock);
    }

    PRINTF("Disabling BODs\n");
    InitBOD1(false);
    InitBOD2(false);
...
static status_t Sleep(wakeup_vector_index_t wakeupVectorIndex)
{
    ...
    POWER_EnterSleep();
    PRINTF("Woken up from sleep. Time for coffee.\n");
...
static status_t DeepSleep(wakeup_vector_index_t wakeupVectorIndex)
{
    ...
    PRINTF("Woken up from deep sleep. Had a great nap.\n");
...
static status_t PowerDown(wakeup_vector_index_t wakeupVectorIndex)
{
    ...
    PRINTF("Woken up from power down. Still a bit fuzzy.\n");
...
static status_t DeepPowerDown(wakeup_vector_index_t wakeupVectorIndex)
{
    ...
    {
        /* FRDM and EVK boards have pull up on wakeup pin, so configure wakeup pin for pull up and trigger wakeup
         * when it goes low*/
...
static void DrawPlot(uint32_t measurementSize)
{
    for (uint32_t i = 0; i < PLOTHEIGHT; i++)
    {
        if (i == 0 || i == PLOTHEIGHT - 1)
        {
            PRINTF("%d mv |\n", i == 0 ? VOLTAGEMAX : VOLTAGEMIN);
        }
        else
        {
            PRINTF("        |\n");
    ...
    char line[measurementSize + 1];
    memset(line, '-', measurementSize);
    line[measurementSize] = '\0';
    PRINTF("        -%s\n", line);
}

static void DrawBar(uint32_t col, uint32_t voltage)
{
    uint32_t range = VOLTAGEMAX - VOLTAGEMIN;
    /* Move cursor to right location */
    PRINTF("\033[%dA\r\033[%dC", PLOTHEIGHT + 1, col + 9);
    for (uint32_t i = 0; i < PLOTHEIGHT; i++)
    {
        PRINTF("%c \033[2D\033[1B", voltage >= VOLTAGEMAX - range * i / (PLOTHEIGHT - 1) ? '*' : ' ');
    }
    PRINTF("\n");
}
...
static status_t VoltageMeasurement(wakeup_vector_index_t param)
{
    ...
    PRINTF("Press user button to stop demo\n");
    DrawPlot(ARRAY_SIZE(measurements));

    while (active)
    {
        SDK_DelayAtLeastUs(50000, SystemCoreClock);
        counter++;
        /* Toggle LED every 20 * 50ms = 1s */
        if (counter == 20)
        {
            LED_TOGGLE();
            counter = 0;
        }
        POWER_MeasureVoltageWithBod1(measurements + measurementPos);
        DrawBar(measurementPos, measurements[measurementPos]);
        PRINTF("\r%d mv", measurements[measurementPos]);
        measurementPos = (measurementPos + 1) % ARRAY_SIZE(measurements);
        active         = GPIO_PinRead(BOARD_INITPINS_BOARD_BUTTON1_GPIO_PIN_GPIO, 0U, BOARD_INITPINS_BOARD_BUTTON1_GPIO_PIN_PIN);
    }
...
int main(void)
{
    ...
    PRINTF("\n\n====================================================\n");
    PRINTF("!!!MCXW23 BOOTED!!!\n");
    PRINTF("Boot reason: ");

    /* Inform the user why the application was rebooted */
    switch (resetCause)
    {
        case kRESET_CAUSE_PAD:
        {
            PRINTF("hardware reset");
            break;
        }
        case kRESET_CAUSE_SWR:
        {
            PRINTF("software reset");
            break;
        }
        case kRESET_CAUSE_BOD:
        {
            PRINTF("BOD reset");
            break;
        }
        case kRESET_CAUSE_RTC:
        {
            PRINTF("RTC wakeup");
            break;
        }
        case kRESET_CAUSE_OST:
        {
            PRINTF("OS timer wakeup");
            break;
        }
        case kRESET_CAUSE_DPD_IO:
        {
        ...
        default:
        {
            PRINTF("0x%x not used in this demo", (uint32_t)resetCause);
            break;
        }
    }
    ...
    switch (s_supplyMode)
    {
        case kDCDC_MODE_HV_SM:
        {
            PRINTF("buck mode (HV)");
            break;
        }
        case kDCDC_MODE_XR_SM_SS:
        {
            PRINTF("XR_SM_SS mode");
            break;
        }
        case kDCDC_MODE_XR_SM_DS:
        {
            PRINTF("XR_SM_DS mode");
            break;
        }
        default:
        {
            PRINTF("not used in this demo");
            break;
        }
    }
