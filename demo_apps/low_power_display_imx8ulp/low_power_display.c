...
static void DEMO_DisableLPAV()
{
    /* Disable PLL4 PFD1DIV2 */
    CGC_LPAV->PLL4DIV_PFD_0 |= CGC_LPAV_PLL4DIV_PFD_0_DIV4HALT(1);

    /* LPAV alloc to APD */
    SIM_SEC->SYSCTRL0 |= 0x00000080;

    /* DSI/LCDIF alloc to APD */
    SIM_SEC->LPAV_MASTER_ALLOC_CTRL |= 0x00000018;

    /* Power off LPAV domain */
    UPOWER_PowerOffSwitches(kUPOWER_PS_AV_NIC);
    UPOWER_PowerOffMemPart(kUPOWER_MP0_DCNANO_A | kUPOWER_MP0_DCNANO_B, 0U);
}
...
void DEMO_LCDIF_Color_Change(void)
{
    ...
    while (1)
    {
        ...
        if (cycles > 10)
        {
            PRINTF("Press C to continnue Low Power Display cycle\r\n");
            PRINTF("Press X to exit Low Power Display cycle\r\n");
            do
            {
                ch = GETCHAR();
            } while ((ch == '\r') || (ch == '\n'));
