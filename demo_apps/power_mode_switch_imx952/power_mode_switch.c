 ...
 * Variables
 ******************************************************************************/
static IRQn_Type irqTbl[] = {MU5_A_IRQn, RPMSG_LITE_M7_A55_MU_IRQn};
...
static void APP_SetWakeupConfig(lpm_power_mode_t targetMode)
{
    ...
    uint32_t wakeMaskIdx, wakeMaskBitPos;

     /* Default to no wakeup IRQs */
     uint32_t wakeMask[GPC_CMC_IRQ_WAKEUP_MASK_COUNT] =
     {
	...
	DisableIRQ(RPMSG_LITE_M7_A55_MU_IRQn);
    ...
    if (s_wakeupSource == kAPP_WakeupSourceSM)
    {
        /* Ensure not to be woken up by A55 messages */
        DisableIRQ(RPMSG_LITE_M7_A55_MU_IRQn);
        wakeMaskIdx = MU5_A_IRQn / 32;
        wakeMaskBitPos = MU5_A_IRQn % 32;
        /* deal with MU interrupts, configure IRQ wake sources. */
        wakeMask[wakeMaskIdx] = ~(1 << wakeMaskBitPos);
    }

     status = SCMI_CpuIrqWakeSet(SCMI_A2P, CPU_IDX_M7P, 0U, GPC_CMC_IRQ_WAKEUP_MASK_COUNT, wakeMask);
     if (status != SCMI_ERR_SUCCESS)
     {
...
void APP_PowerPreSwitchHook(lpm_power_mode_t targetMode)
{
    ...
    lpmConfig[0].retMask = 1U << PWR_MEM_SLICE_IDX_M7;
     ...
     if (s_wakeupSource == kAPP_WakeupSourceSM)
     {
        ...
        SCMI_LmmNotify(SCMI_A2P, SYSTEM_PLATFORM_LMID_A55, SCMI_LMM_NOTIFY_BOOT(0U) |
...
{
    int32_t status = SCMI_ERR_SUCCESS;
    uint32_t wakeMask[GPC_CMC_IRQ_WAKEUP_MASK_COUNT] = {0x0};
    ...
    if (SM_Platform_GetSystemState() == SCMI_SYS_STATE_FULL_SUSPEND || SM_Platform_GetSystemState() == SCMI_SYS_STATE_SUSPEND || s_wakeupSource == kAPP_WakeupSourceSM)
    {
        APP_ResumePeriperal();
        /* Re-enable LMM notification from system manager after M7 exit low power state. */
	SCMI_LmmNotify(SCMI_A2P, SYSTEM_PLATFORM_LMID_A55, SCMI_LMM_NOTIFY_BOOT(1U) |
    ...
    if (s_wakeupSource == kAPP_WakeupSourceSM)
    {
        ...
        EnableIRQ(RPMSG_LITE_M7_A55_MU_IRQn);
        xSemaphoreGiveFromISR(s_wakeupSig, NULL);
        portYIELD_FROM_ISR(pdTRUE);
    }
    else
...
static void APP_SuspendTimerCallback(TimerHandle_t xTimer)
{
	...
	uint32_t wakeMaskIdx, wakeMaskBitPos;
        ...
        wakeMaskIdx = RPMSG_LITE_M7_A55_MU_IRQn / 32;
        wakeMaskBitPos = RPMSG_LITE_M7_A55_MU_IRQn % 32;
        wakeMask[wakeMaskIdx] |= (1 << wakeMaskBitPos);

        status = SCMI_CpuIrqWakeSet(SCMI_A2P, CPU_IDX_M7P, 0, GPC_CMC_IRQ_WAKEUP_MASK_COUNT, wakeMask);
        if (status != SCMI_ERR_SUCCESS)
        {
            PRINTF("SCMI_CpuIrqWakeSet set fail\r\n");
        }
