    ...
    switch (audioCommand)
    {
        ...
        case USB_DEVICE_AUDIO_TE_GET_CUR_CONNECTOR_CONTROL:
            if (USB_AUDIO_RECORDER_CONTROL_INPUT_TERMINAL_ID == entityOrEndpoint)
            {
                ...
                g_deviceAudioComposite->audioUnified.recorderConnectorStatus.iChannelNames        = 0x0U;			
                *buffer = (uint8_t *)&g_deviceAudioComposite->audioUnified.recorderConnectorStatus;
                *length = sizeof(g_deviceAudioComposite->audioUnified.recorderConnectorStatus);
            }
            break;
#else
...
usb_status_t USB_DeviceAudioUnifiedInit(usb_device_composite_struct_t *deviceComposite)
{
    ...
    g_deviceAudioComposite->audioUnified.speakerFreqControlRange.wMIN          = AUDIO_OUT_SAMPLING_RATE;
    g_deviceAudioComposite->audioUnified.speakerFreqControlRange.wMAX          = AUDIO_OUT_SAMPLING_RATE;
    ...
    g_deviceAudioComposite->audioUnified.recorderFreqControlRange.wMIN          = AUDIO_IN_SAMPLING_RATE;
    g_deviceAudioComposite->audioUnified.recorderFreqControlRange.wMAX          = AUDIO_IN_SAMPLING_RATE;
